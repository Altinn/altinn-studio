...
namespace Altinn.Studio.Cli.v7Tov8.CodeRewriters;

public class UsingRewriter: CSharpSyntaxRewriter
{
    private const string CommonInterfaceNamespace = "Altinn.App.Core.Interface";

    private readonly Dictionary<string, string> usingMappings = new Dictionary<string, string>()
    {
    ...
    private bool ImplementsIProcessExclusiveGateway(CompilationUnitSyntax node)
    {
        ...
        return classDecendants.Any(c => c.BaseList?.Types.Any(t => t.Type.ToString() == "IProcessExclusiveGateway") == true);
    }
    ...
    private CompilationUnitSyntax AddUsing(CompilationUnitSyntax node, string usingString)
    {
        ...
        var usingDirective = SyntaxFactory.UsingDirective(usingName).NormalizeWhitespace().WithTrailingTrivia(SyntaxFactory.ElasticCarriageReturnLineFeed);
    ...
    {
        var usingDirectiveSyntaxes = node.DescendantNodes().OfType<UsingDirectiveSyntax>();
        var usingDirectiveSyntax = usingDirectiveSyntaxes.FirstOrDefault(u => u.Name?.ToString() == CommonInterfaceNamespace);
