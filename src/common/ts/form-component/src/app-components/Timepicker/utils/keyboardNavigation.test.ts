...
describe('Keyboard Navigation Logic', () => {
  describe('handleSegmentKeyDown', () => {
    it('should handle Arrow Up key', () => {
      const mockEvent = { key: 'ArrowUp', preventDefault: vi.fn() } as unknown as MockKeyboardEvent;
    ...
    it('should not handle other keys', () => {
      const mockEvent = { key: 'Enter', preventDefault: vi.fn() } as unknown as MockKeyboardEvent;
      const result = handleSegmentKeyDown(mockEvent);
  ...
  describe('getNextSegmentIndex', () => {
    ...
    it('should navigate between segments correctly', () => {
      ...
      expect(getNextSegmentIndex(3, 'right', segments)).toBe(0);
      expect(getNextSegmentIndex(0, 'left', segments)).toBe(3);
    });
  });
  ...
  describe('handleValueIncrement', () => {
    it('should increment hours correctly', () => {
      ...
      expect(handleValueIncrement(23, 'hours', 'HH:mm')).toBe(0);
      expect(handleValueIncrement(12, 'hours', 'hh:mm a')).toBe(1);
    });

    it('should increment minutes and seconds', () => {
      expect(handleValueIncrement(30, 'minutes', 'HH:mm')).toBe(31);
      expect(handleValueIncrement(59, 'minutes', 'HH:mm')).toBe(0);
      expect(handleValueIncrement(59, 'seconds', 'HH:mm:ss')).toBe(0);
    });
  ...
  describe('handleValueDecrement', () => {
    it('should decrement hours correctly', () => {
      ...
      expect(handleValueDecrement(0, 'hours', 'HH:mm')).toBe(23);
      expect(handleValueDecrement(1, 'hours', 'hh:mm a')).toBe(12);
    });

    it('should decrement minutes and seconds', () => {
      expect(handleValueDecrement(30, 'minutes', 'HH:mm')).toBe(29);
      expect(handleValueDecrement(0, 'minutes', 'HH:mm')).toBe(59);
      expect(handleValueDecrement(0, 'seconds', 'HH:mm:ss')).toBe(59);
    });
