...
describe('Heading', () => {
  it.each<[HeadingSize | undefined, number]>([
    ['L', 2],
    ['h2', 2],
    ['M', 3],
    ['h3', 3],
    ...
    renderWithTranslations(<Heading componentId='h1' title='my.title' size={size} />, {
  ...
  it('resolves the title key via the context', () => {
    renderWithTranslations(<Heading componentId='h1' title='my.title' />, {
      overrides: { 'my.title': 'resolved title' },
    });
  ...
  it('does not render a help button when no help key is supplied', () => {
    renderWithTranslations(<Heading componentId='h1' title='x' />);
    expect(screen.queryByRole('button')).not.toBeInTheDocument();
  });

  it('renders a help button when a help key is supplied', () => {
    renderWithTranslations(<Heading componentId='h1' title='x' help='my.help' />, {
  ...
  it('renders the form-content wrapper for the given componentId', () => {
    renderWithTranslations(<Heading componentId='heading-1' title='x' />);
    expect(document.getElementById('form-content-heading-1')).toBeInTheDocument();
  });
});
