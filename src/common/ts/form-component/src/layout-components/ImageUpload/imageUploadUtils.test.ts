...
describe('constrainToArea', () => {
  ...
  it('should clamp position to max offset for image larger than crop area', () => {
    const zoom = 1;
    const constrainedPosition = constrainToArea({
      image,
      zoom,
...
describe('normalToLogZoom', () => {
  it('calculates zoom correctly within min and max bounds', () => {
    ...
    expect(normalToLogZoom({ value: 50, minZoom, maxZoom })).toBeCloseTo(
      Math.sqrt(minZoom * maxZoom),
    );
  });
});
...
describe('logToNormalZoom', () => {
  it('calculates normal zoom correctly within min and max bounds', () => {
    ...
    expect(logToNormalZoom({ value: Math.sqrt(minZoom * maxZoom), minZoom, maxZoom })).toBeCloseTo(
      50,
    );
  });

  it('returns 0 if log scale is zero', () => {
    const minZoom = 2;
    const maxZoom = 2;
