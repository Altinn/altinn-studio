...
| `type` | `"Checkboxes"` | Yes |  | Identifies which component type this configuration represents. |
...
| `layout` | `"column" \| "row" \| "table"` | No |  | Define the layout style for the options Allowed values: "column", "row", "table". |
| `showLabelsInTable` | `boolean` | No | `false` | Boolean value indicating if the label should be visible when only one option exists in table |
