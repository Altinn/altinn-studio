...
| `type` | `"TimePicker"` | Ja |  | Angir hvilken komponenttype konfigurasjonen gjelder. |
...
| `autocomplete` | `"time"` | Nei |  |  |
| `format` | `"HH:mm" \| "HH:mm:ss" \| "hh:mm a" \| "hh:mm:ss a"` | Nei | `"HH:mm"` | Tidsformatet for visning og inndata. |
| `minTime` | `string \| expression<string> \| string` | Nei |  | Angir tidligste tillatte klokkeslett i formatet HH:mm. |
| `maxTime` | `string \| expression<string> \| string` | Nei |  | Angir seneste tillatte klokkeslett i formatet HH:mm. |
