...
| `type` | `"SimpleTable"` | Ja |  | Angir hvilken komponenttype konfigurasjonen gjelder. |
...
| `dataModelBindings.tableData` | `object` | Ja |  | Liste over objektene der dataene lagres. |
| `dataModelBindings.tableData.dataType` | `string` | Ja |  | Navnet på datamodelltypen det skal refereres til. |
| `dataModelBindings.tableData.field` | `string` | Ja |  | Stien til egenskapen i punktnotasjon. |
| `columns` | `object[]` | Ja |  |  |
| `columns[].header` | `string` | Ja |  |  |
| `columns[].accessors` | `string[]` | Ja |  | Liste over feltene som skal tas med i cellen. |
| `columns[].component` | `object \| object \| object` | Nei |  |  |
| `columns[].component[type=link].type` | `"link"` | Ja |  |  |
| `columns[].component[type=link].hrefPath` | `string` | Ja |  |  |
| `columns[].component[type=link].textPath` | `string` | Ja |  |  |
| `columns[].component[type=link].openInNewTab` | `boolean` | Nei |  |  |
| `columns[].component[type=date].type` | `"date"` | Ja |  |  |
| `columns[].component[type=date].format` | `string` | Nei |  | Datoformatet som brukes når datoen vises til brukeren. |
| `columns[].component[type=radio].type` | `"radio"` | Ja |  |  |
| `columns[].component[type=radio].options` | `object[]` | Nei |  |  |
| `columns[].component[type=radio].options[].label` | `string` | Ja |  |  |
| `columns[].component[type=radio].options[].value` | `string` | Ja |  |  |
| `zebra` | `boolean` | Nei |  | Viser tabellen med stripete rader. |
| `enableDelete` | `boolean` | Nei |  | Lar brukeren slette raden. |
| `enableEdit` | `boolean` | Nei |  | Lar brukeren redigere raden. |
| `size` | `"sm" \| "md" \| "lg"` | Nei |  | Tabellens størrelse. Tillatte verdier: "sm", "md", "lg". |
| `externalApi` | `object` | Nei |  |  |
| `externalApi.id` | `string` | Ja |  |  |
| `externalApi.path` | `string` | Ja |  |  |
