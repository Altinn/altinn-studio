...
| `type` | `"Datepicker"` | Yes |  | Identifies which component type this configuration represents. |
...
| `autocomplete` | `"bday"` | No |  |  |
| `minDate` | `string \| expression<string> \| "today" \| "yesterday" \| "tomorrow" \| "oneYearAgo" \| "oneYearFromNow"` | No | `"1900-01-01T12:00:00.000Z"` | Sets the earliest allowed date. Can also use keyword 'today' to disable all past dates dynamically based on the current date. Defaults to 1900-01-01T12:00:00.000Z. |
| `maxDate` | `string \| expression<string> \| "today" \| "yesterday" \| "tomorrow" \| "oneYearAgo" \| "oneYearFromNow"` | No | `"2100-01-01T12:00:00.000Z"` | Sets the latest allowed date. Can also use keyword 'today' to disable all future dates dynamically based on the current date. Defaults to 2100-01-01T12:00:00.000Z. |
| `timeStamp` | `boolean` | No | `false` | Boolean value indicating if the date time should be stored as a timeStamp. Defaults to false. If true: 'yyyy-MM-ddThh:mm:ss.sssZ', if false 'yyyy-MM-dd'; |
| `format` | `string` | No | `"dd.MM.yyyy"` | Date format used when filling out and displaying the date to the user. If not set the format will be based on the user's selected language. |
