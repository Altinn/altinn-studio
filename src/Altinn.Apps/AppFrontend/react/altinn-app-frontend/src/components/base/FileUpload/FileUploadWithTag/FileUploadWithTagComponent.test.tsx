...
describe('FileUploadWithTagComponent', () => {
  ...
  it('should show spinner when file is uploading or updating ', () => {
    ...
    expect(
      wrapper.find(`#attachment-loader-upload-${mockAttachments[1].id}`),
    ).toHaveLength(2); // div and react node
    expect(
      wrapper.find(`#attachment-loader-update-${mockAttachments[4].id}`),
    ).toHaveLength(2); // div and react node
  });
  ...
  it('should disable dropdown when updating', () => {
    ...
    expect(
      wrapper.find(`#attachment-tag-dropdown-${mockAttachments[4].id}`).props()
        .disabled,
    ).toBeTruthy();
  });
  ...
  it('should display validation error when saving when no tag is selected', () => {
    ...
    const button = wrapper.find(
      `#attachment-save-tag-button-${mockAttachments[5].id}`,
    );
    button.at(0).simulate('click');
    expect(
      wrapper.find(`#attachment-error-${mockAttachments[5].id}`),
    ).toHaveLength(2); // div and react node
  });
  ...
  it('should have open edit windows for attachments without tags.', () => {
    ...
    expect(
      wrapper.find(`#attachment-edit-window-${mockAttachments[3].id}`),
    ).toHaveLength(1);
    expect(
      wrapper.find(`#attachment-edit-window-${mockAttachments[4].id}`),
    ).toHaveLength(1);
  });
