import { initialState, QueueActions, queueSlice } from 'src/features/queue/queueSlice';
import type { IQueueState } from 'src/features/queue/index';

describe('queueSlice', () => {
  const slice = queueSlice();
  ...
  it('handles startInitialDataTaskQueue action', () => {
    ...
    nextState = slice.reducer(nextState, QueueActions.startInitialDataTaskQueueFulfilled);
    expect(nextState.dataTask.isDone).toBeTruthy();
  });

  it('handles startInitialInfoTaskQueue action', () => {
    let nextState = slice.reducer(state, QueueActions.startInitialInfoTaskQueue);
    expect(nextState.infoTask.isDone).toBeFalsy();
    nextState = slice.reducer(nextState, QueueActions.startInitialInfoTaskQueueFulfilled);
    expect(nextState.infoTask.isDone).toBeTruthy();
  });

  it('handles error on app task queue', () => {
    const errorMessage = 'app task queue error';
    const nextState = slice.reducer(state, QueueActions.appTaskQueueError({ error: new Error(errorMessage) }));
  ...
  it('handles error on data task queue', () => {
    ...
    const nextState = slice.reducer(state, QueueActions.dataTaskQueueError({ error: new Error(errorMessage) }));
  ...
  it('handles error on info task queue', () => {
    ...
    const nextState = slice.reducer(state, QueueActions.infoTaskQueueError({ error: new Error(errorMessage) }));
  ...
  it('handles error on user task queue', () => {
    ...
    const nextState = slice.reducer(state, QueueActions.userTaskQueueError({ error: new Error(errorMessage) }));
