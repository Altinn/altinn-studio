...
function setupMocks(test: FunctionTest): void {
  jest.mocked(fetchApplicationMetadata).mockResolvedValue(createApplicationMetadata(test));
  jest.mocked(useExternalApis).mockReturnValue(test.externalApis as ExternalApisResult);
  jest.mocked(fetchProcessState).mockImplementation(async () => createProcess(test) ?? getProcessDataMock());
  jest.mocked(fetchUserProfile).mockImplementation(async () => createProfile(test.profileSettings));
...
function createApplicationMetadata({
  ...
  dataModels,
}: FunctionTest): IncomingApplicationMetadata {
...
function createQueries(test: FunctionTest, expression: ExprValToActualOrExpr<ExprVal.Any>): Partial<AppQueries> {
  const { frontendSettings, textResources, codeLists } = test;
  return {
