...
namespace AltinnCore.Designer.Controllers
{
    ...
    public class ManualTestingController : Controller
    {
        ...
        public async Task<IActionResult> Index(string org, string service, int reporteeId)
        {
            ...
            StartServiceModel startServiceModel = new StartServiceModel
            {
                ...
                ReporteeList = _authorization.GetPartyList(requestContext.UserContext.UserId)
                    .Select(x => new SelectListItem { Text = (x.PartyTypeName == PartyType.Person) ? x.SSN + " " + x.Person.Name : x.OrgNumber + " " + x.Organization.Name, Value = x.PartyId.ToString() })
                    .ToList(),
        ...
        public async Task<IActionResult> LoginTestUser(string org, string service, int id, string reportee)
        {
            ...
            if (!string.IsNullOrEmpty(reportee) && reporteeList.Any(r => r.SSN.Equals(reportee)))
            {
                reporteeBE = reporteeList.FirstOrDefault(r => r.SSN.Equals(reportee));
