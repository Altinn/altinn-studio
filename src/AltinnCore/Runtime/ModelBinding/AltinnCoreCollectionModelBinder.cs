...
namespace AltinnCore.Runtime.ModelBinding
{
    ...
    public class AltinnCoreCollectionModelBinder<TElement> : ICollectionModelBinder
    {
        private Func<object> _modelCreator;

        /// <summary>
        /// Initializes a new instance of the <see cref="AltinnCoreCollectionModelBinder{TElement}"/> class.
        /// </summary>
        /// <param name="elementBinder">The <see cref="IModelBinder"/> for binding elements.</param>
        public AltinnCoreCollectionModelBinder(IModelBinder elementBinder)
        {
        ...
        public virtual async Task BindModelAsync(ModelBindingContext bindingContext)
        {
            ...
            else
            {
                ...
                // Modification from standard. Not needed since it is done above.
        ...
        /// <summary>
        /// Method that based on the key and the available form collection find all indexes for a given collection posted in form
        /// </summary>
        /// <param name="formCollection"></param>
        /// <param name="startPath"></param>
        /// <returns></returns>
        private List<int> GetIndexesForCollection(IFormCollection formCollection, string startPath)
        {
            List<int> indexValues = new List<int>();
            int indexBracketStart = startPath.Length + 1;
            foreach (string key in formCollection.Keys)
            {
                if (key.StartsWith(startPath + "["))
                {
                    int indexBracketEnd = key.IndexOf("]", indexBracketStart);
                    string value = key.Substring(indexBracketStart, indexBracketEnd - indexBracketStart);
                    indexValues.Add(Convert.ToInt32(value));
                }
            }
            indexValues = indexValues.Distinct().ToList();
            indexValues.Sort();
            return indexValues;
        }

        private TModel CastOrDefault<TModel>(object model)
        {
