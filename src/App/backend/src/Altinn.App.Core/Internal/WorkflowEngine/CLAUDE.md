...
├── ProcessNextRequestFactory.cs              - Maps ProcessStateChange → WorkflowEnqueueEnvelope (request body + namespace/idempotency/collection metadata + process-next labels)
├── WorkflowCommandSet.cs                     - Builds command sequences (as StepRequest lists) per event type
├── WorkflowCommandSetContexts.cs             - TaskStartContext / ProcessEndContext records passed into WorkflowCommandSet
├── WorkflowCallbackStateService.cs           - Captures/restores workflow callback state to/from the signed state blob
├── WorkflowStateSigner.cs                    - Detached HMAC-SHA256 sign/verify of the state blob (SignedWorkflowState envelope)
├── WorkflowCallbackStateException.cs         - Thrown on invalid/tampered/mismatched callback state (→ 422)
├── WorkflowSubmissionFailedException.cs      - Thrown when the engine did not accept a submission (NotAccepted/Unknown)
├── Authentication/
│   ├── WorkflowCallbackAuthentication.cs      - Scheme name constant ("WorkflowEngineCallback")
│   ├── WorkflowCallbackTokenGenerator.cs      - Mints the callback JWT (bound to instance guid, signed with newest app-code)
│   ├── WorkflowCallbackTokenValidator.cs      - Validates the replayed callback JWT
│   ├── WorkflowCallbackSecretProvider.cs      - Supplies signing/validation AppCodes from AppCodesSettings (rotation-aware)
│   ├── WorkflowCallbackSecretNotFoundException.cs - Thrown when no usable WorkflowEngineCallback code is configured
│   └── WorkflowCallbackAppCodesValidator.cs   - Startup IValidateOptions check: a usable code must be configured
├── Commands/
│   ├── _Base/
│   │   ├── IWorkflowEngineCommand.cs          - Command interface (plain + generic IWorkflowEngineCommand<TPayload>)
│   │   ├── WorkflowEngineCommandBase.cs       - Base class for typed-payload commands (deserializes then delegates)
│   │   ├── ProcessEngineCommandContext.cs     - Context struct (AppId, InstanceId, InstanceDataMutator, Payload, CancellationToken)
│   │   ├── ProcessEngineCommandResult.cs      - SuccessfulProcessEngineCommandResult / FailedProcessEngineCommandResult
│   │   ├── CommandPayload.cs                  - CommandRequestPayload polymorphic base + CommandPayloadSerializer + source-gen context
...
│   │   │   ├── CommonTaskInitialization.cs    - Auto-create data elements, prefill (CommonTaskInitializationPayload)
...
│   │       ├── DeleteDataElements.cs         - Key "DeleteDataElementsIfConfigured": auto-delete AutoDeleteOnProcessEnd data types (post-commit)
│   │       └── DeleteInstance.cs             - Key "DeleteInstanceIfConfigured": hard-delete instance if ApplicationMetadata.AutoDeleteOnProcessEnd (post-commit)
│   ├── AltinnEvents/
...
│   ├── ExecuteServiceTask.cs                 - Runs IServiceTask.Execute() (post-commit); can request auto-advance (ExecuteServiceTaskPayload)
│   ├── NotifyInstanceOwnerOnInstantiation.cs - Sends instantiation notification (post-commit, first task only; NotifyInstanceOwnerOnInstantiationPayload)
...
├── DependencyInjection/
│   ├── ServiceCollectionExtensions.cs        - AddWorkflowEngineIntegration(): registers service, client, factory, signer, auth helpers, and all commands
│   └── WorkflowEngineCommandValidator.cs     - Startup check: all keys in WorkflowCommandSet are registered
├── Http/
│   ├── IWorkflowEngineClient.cs              - Enqueue, get-collection, list, cancel, resume, abandon operations
│   └── WorkflowEngineClient.cs              - HTTP impl (typed HttpClient; paginates ListWorkflows)
└── Models/
    ├── WorkflowCallbackState.cs              - Internal DTO for transported instance + form data callback state
    ├── SignedWorkflowState.cs                - Signed envelope { Payload, Signature, SecretId } around the callback state
    ├── AppCommand/
    │   ├── Actor.cs                          - User/org/system-user identity for the request
    ...
    │   ├── AppCallbackPayload.cs             - Payload the engine sends back per callback (commandKey, actor, lockToken, payload, workflowId, state)
    │   └── AppCallbackResponse.cs            - Success response with updated (re-signed) state blob
    └── Engine/
        ...
        ├── WorkflowEnqueueResponse.cs        - Response: Accepted (with WorkflowResult[], each carrying DatabaseId + Namespace)
        ├── StepRequest.cs                    - Single step (operationId + command + retryStrategy + metadata)
        ├── StepStatusResponse.cs             - Per-step status (operationId, command, status, retryCount, errorHistory)
        ├── CommandDefinition.cs              - Flat record with type + data (JsonElement)
        ...
        ├── ErrorEntry.cs                     - A single engine-recorded error (timestamp, message, httpStatusCode, wasRetryable)
        ├── PaginatedResponse.cs              - Paged engine list response (Data + NextCursor)
        ├── WorkflowStatusResponse.cs         - Full status (databaseId, operationId, collectionKey, overallStatus, steps, labels, timestamps)
        ├── WorkflowCollectionDetailResponse.cs - Collection status response with active heads (CollectionHeadStatus)
        ├── WorkflowRef.cs                     - Dependency reference (ref string or database id) + JsonConverter
        ├── CancelWorkflowResponse.cs         - Response from cancel
        └── ResumeWorkflowResponse.cs         - Response from resume
```
