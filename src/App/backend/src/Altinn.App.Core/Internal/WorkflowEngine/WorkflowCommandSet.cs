...
internal sealed class WorkflowCommandSet
{
    ...
    /// <summary>
    /// Step label carrying the BPMN task a task-phase step belongs to. Every step a task-phase builder adds
    /// gets it, the lifecycle steps and the task type's own declared commands alike, so the engine's dashboard
    /// can bracket them by task without knowing any command names.
    /// </summary>
    internal const string ProcessTaskLabel = "processTask";

    /// <summary>Step label carrying the task phase of a task-phase step: <c>start</c>, <c>end</c> or <c>abandon</c>.</summary>
    internal const string ProcessTaskPhaseLabel = "processTaskPhase";

    private readonly Dictionary<string, string>? _stepLabels;

    public WorkflowCommandSet() { }

    private WorkflowCommandSet(string taskId, string phase) =>
        _stepLabels = new Dictionary<string, string> { [ProcessTaskLabel] = taskId, [ProcessTaskPhaseLabel] = phase };
    ...
    public static WorkflowCommandSet GetTaskStartSteps(TaskStartContext context)
    {
        ...
        // app-supplied IOnTaskStartingHandler implementations and the task type's own start commands)
        // reads a clean slate, free of stale elements generated by previous visits to the entering task.
        ...
        var group = new WorkflowCommandSet(context.TaskId, "start")
    ...
    {
        _commands.Add(WithStepLabels(CreateCommand(commandKey, payload)));
        return this;
    }
    ...
    private StepRequest WithStepLabels(StepRequest step) =>
        _stepLabels is null ? step : step with { Labels = _stepLabels };
