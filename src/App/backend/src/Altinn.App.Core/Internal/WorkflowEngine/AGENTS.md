...
- Commands return `SuccessfulProcessEngineCommandResult`, `DeferredProcessEngineCommandResult` or `FailedProcessEngineCommandResult` (never throw from Execute). `FailedProcessEngineCommandResult` distinguishes `Retryable` (→ 500, engine retries) from `Permanent`/`NonRetryable` (→ 422, engine gives up). `DeferredProcessEngineCommandResult` is neither: it returns 200 with a `defer` block, so the engine parks the workflow in `Waiting` and re-executes the step later — no error recorded, retry counter reset. Today only `ExecuteServiceTask` produces one (from `ServiceTaskResult.Defer`); the app-facing contract, the wait budget (`ProcessStepOptions.WaitBudget`) and the fields a task reads to pace itself (`RetryCount`/`ExecutionDeadline` for this attempt, `DeferCount`/`WaitStartedAt`/`WaitDeadline` — plus the derived `RemainingWait`/`IsFinalCheck` — for the whole wait) are documented on those types. For send-then-poll tasks: the send guard is a **checkpoint** (`ServiceTaskContext.SetCheckpoint`/`GetCheckpoint` — instance data values keyed `serviceTask:{Type}:{key}`, written to Storage immediately and read through to Storage, so a crashed attempt's evidence survives to its retry), never engine bookkeeping like `DeferCount`; `StepId` is the outbound idempotency key covering the send→checkpoint crash window (see the `Defer` xmldocs)
