...
WorkflowEngineCallbackController.ExecuteCommand()   (Altinn.App.Api)
  ...
  → Capture + re-sign updated state blob, return to engine
...
- **Command phases**: task-end/abandon commands → `MutateProcessState` (in-memory state transition) → task-start / process-end commands → `CommitProcessState` (stages the process-state change; the controller save that commits it is the commit boundary) → `EnqueueSideEffectsWorkflow` (schedules the non-critical side-effect commands as separate `IsHead=false` single-step workflows, one per side effect, when the transition has any) → critical post-commit commands (see Command Sequences)
...
4. `CommitProcessState` (always inserted by the factory) — stages the `ProcessStateChange`; the
   controller save commits it
...
`DeleteInstanceIfConfigured` stages a hard deletion on the unit of work. If it stages a deletion, the controller save in that callback commits the delete mutation through Storage's aggregate mutation endpoint.
...
- Each service task gets one `ExecuteServiceTask` callback after `CommitProcessState`. `ServiceTaskContext.IdempotencyKey` is the retry-stable workflow step database id from the callback header; the required `ExecutionReferenceTime` is `Workflow.StartAt ?? Step.CreatedAt`. The engine reuses both values on retries of that persisted step.
