...
4. **Per-callback lifecycle**: The controller restores `InstanceDataUnitOfWork` from the signed state blob, resolves the `IWorkflowEngineCommand` by key, executes it, saves data changes on success, captures the updated (re-signed) state, and returns it to the engine. A **deferral** is deliberately different: it is stateless — nothing is saved, the incoming state is echoed back unchanged (the re-run starts exactly where this attempt did), and a deferring handler that made data changes is rejected with a non-retryable 422. A step that checks-and-waits is not a step that records; work that produces something durable belongs in its own step.
...
EnqueueSideEffectsWorkflow → [ExecuteServiceTask if service task — one engine step per declared step, then the concluding one]
...
- Commands return `SuccessfulProcessEngineCommandResult`, `DeferredProcessEngineCommandResult` or `FailedProcessEngineCommandResult` (never throw from Execute). `FailedProcessEngineCommandResult` distinguishes `Retryable` (→ 500, engine retries) from `Permanent`/`NonRetryable` (→ 422, engine gives up). `DeferredProcessEngineCommandResult` is neither: it returns 200 with a `defer` block, so the engine parks the workflow in `Waiting` and re-executes the step later — no error recorded, retry counter reset. Today only `ExecuteServiceTask` produces one (from `ServiceTaskResult.Defer`); the app-facing contract, the wait budget (`ProcessStepOptions.WaitBudget`) and the two clock groups a task reads to pace itself (`context.Attempt` for this execution, `context.Wait` — incl. the derived `Remaining`/`IsFinalCheck` — for the whole wait) are documented on those types. For send-then-poll (or any multi-phase) tasks: declare each earlier phase as its own step via `IServiceTask.Steps` and let the task's `Execute` conclude — the engine's step ledger is the durable send guard (a completed step never re-runs; a retry or resume re-enters at the failed step), never anything under `Attempt`/`Wait`; `StepId` (per step) is the outbound idempotency key covering the in-attempt crash window (see the `Defer` xmldocs)
- **Multi-step service tasks**: every service task is a pipeline; most have zero declared steps. `IServiceTask.Steps` (default empty) declares durable steps executed before the task's own `Execute`, each an `IServiceTaskStep` returning `ServiceTaskStepResult` — Next/Defer/Failed, deliberately unable to conclude the task (concluding is `Execute`'s alone, always last). The factory expands one `ExecuteServiceTask` engine step per declared step at enqueue time (payload carries the step name; OperationId is `ExecuteServiceTask · {stepName}`), then the concluding step with a null step name and the bare `ExecuteServiceTask` OperationId — for a task without declared steps that concluding step is the whole expansion, byte-identical to the pre-steps wire shape. The expansion fixes the task's step shape for the workflow's lifetime — step names are therefore a compatibility surface for in-flight workflows (default = class name; pin via `Name` when renaming; unknown name → permanent `ServiceTaskStepNotFound`). Steps share state through the ordinary mechanism only: a completed step's data changes are saved into the state blob the engine hands the next step — there is no separate handoff channel, and a deferring attempt saves nothing. Options: a declared step's `StepOptions` wins field-wise over the task's own, which apply directly to the concluding step. `ServiceTaskRegistrationValidator` fails startup on invalid step declarations (throwing/null `Steps`, duplicate/empty names, invalid per-step options). Internal consumers enumerate steps via `ServiceTaskLookupExtensions.GetSteps` (null-tolerant: Moq mocks bypass the interface default and return null)
