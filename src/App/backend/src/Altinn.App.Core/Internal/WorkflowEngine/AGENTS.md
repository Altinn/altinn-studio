...
`ResolveWorkflowTaskStatus` is its read-path sibling: a presentation projection (idle / processing / failed + target task + failure kind, plus the failed workflow id/timestamp as a support reference) used by `ProcessStateEnricher` to annotate every process/instance read (see `docs/adr/2026-07-08-process-live-workflow-status.md` at the repo root). The frontend renders a terminal failure as an error page carrying that support reference — deliberately **no user Retry** (the engine already exhausted its retry budget); recovery is ops calling `POST process/resume`, which the frontend's failed-state slow poll converges on. **Known gap:** for an ended process (`CurrentTask == null`) it reports idle without consulting the engine, so a terminal failure in the final transition's post-commit steps (which run after `ended` commits) never surfaces on a read — detection is ops alerting on the engine's failed workflows.
