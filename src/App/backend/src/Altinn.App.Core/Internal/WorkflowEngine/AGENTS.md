...
- **Command phases**: task-end/abandon commands → `MutateProcessState` (in-memory state transition) → task-start / process-end commands → `SaveProcessStateToStorage` (persist to Storage, the commit boundary) → `EnqueueSideEffectsWorkflow` (schedules the non-critical side-effect commands as a separate `IsHead=false` workflow, when the transition has any) → critical post-commit commands (see Command Sequences)
...
them to the engine as a separate **side-effects workflow**: an independent root
(`DependsOn = null`, `DependsOnHeads = false`), invisible to the collection heads frontier
(`IsHead = false`), carrying the commit-time state blob as its own state (see State Passthrough),
...
pipeline, and the side effects start at the commit rather than after e.g. a service task. When
there are no side-effect commands, a single workflow without the enqueue step is emitted —
identical to the pre-split behaviour.

**Ordering is per-transition only.** Side-effects workflows are independent of each other, so
cross-transition event order is not guaranteed: a retrying `movedTo.Task_2` registration can land
after `movedTo.Task_3` (or after `completed`). This matches the platform: the Altinn Events service
itself delivers at-least-once and unordered (competing consumers, no Service Bus sessions, and
per-event retry parking up to 12h — see the ordering section of the ADR), so ordered registration
was never observable by subscribers. Consumers that mirror process state from Altinn events must
tolerate out-of-order (and duplicate) delivery. Note also that process-end events may register
before the process-end cleanup commands (deletes) finish, since the side-effects workflow starts at
the commit.
...
Side-effects workflow (independent root, IsHead=false, own commit-time state, links→Main):
MovedToAltinnEvent
```
...
4. **Add to sequence**: add to the appropriate method in `WorkflowCommandSet.cs` (use `AddCommand` for pre-commit, `AddCriticalPostCommitCommand` for post-commit work the transition must wait on, `AddSideEffectCommand` for fire-and-forget post-commit work that runs in the non-gating side-effects workflow). Side-effect commands see the commit-time state (the exact state `SaveProcessStateToStorage` persisted) — but remember they run after the response has been sent, possibly in parallel with Main's critical post-commit commands, and must stay idempotent and non-blocking.
