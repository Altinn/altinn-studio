...
4. **Per-callback lifecycle**: The controller restores `InstanceDataUnitOfWork` from the signed state blob, resolves the `IWorkflowEngineCommand` by key, executes it, saves data changes on success, captures the updated (re-signed) state, and returns it to the engine. A **deferral** is deliberately different: it is stateless — nothing is saved, the incoming state is echoed back unchanged (the re-run starts exactly where this attempt did), and a deferring handler that made data changes is rejected with a non-retryable 422. A step that checks-and-waits is not a step that records; work that produces something durable belongs in its own pipeline step.
...
- **Staged service tasks** (`IStagedServiceTask`, sibling of `IServiceTask` under the root `IServiceTaskBase`): the task declares work steps (`Steps`, each an `IServiceTaskStep` returning `ServiceTaskStepResult` — Next/Defer/Failed, deliberately unable to conclude the task) and the concluding step (`FinalStep`, an `IFinalServiceTaskStep` returning the ordinary `ServiceTaskResult`). The factory expands the pipeline to one `ExecuteServiceTask` engine step per pipeline step at enqueue time (payload carries the step name; OperationId is `ExecuteServiceTask · {stepName}`), fixing the pipeline's shape for the workflow's lifetime — step names are therefore a compatibility surface for in-flight workflows (default = class name; pin via `Name` when renaming). Steps share state through the ordinary mechanism only: a completed step's data changes are saved into the state blob the engine hands the next step — there is no separate handoff channel, and a deferring attempt saves nothing. `ServiceTaskRegistrationValidator` fails startup on invalid registrations (root-registered tasks, dual-kind classes/steps) and invalid pipeline shapes (empty `Steps`, duplicate/empty names, invalid per-step options)
