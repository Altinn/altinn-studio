...
public interface IProcessNavigator
{
    ...
    /// <summary>
    /// Gets the next task using already restored instance data for gateway decisions.
    /// </summary>
    /// <param name="instance">Instance data</param>
    /// <param name="currentElement">Current process element id</param>
    /// <param name="action">Action performed</param>
    /// <param name="dataAccessor">Restored callback data, or null to load data for the current user.</param>
    /// <returns>The next process task</returns>
    internal Task<ProcessElement?> GetNextTask(
        Instance instance,
