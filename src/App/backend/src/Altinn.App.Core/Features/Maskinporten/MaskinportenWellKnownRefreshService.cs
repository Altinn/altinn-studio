...
/// from the settings read inside <see cref="MaskinportenClient.RefreshWellKnownIssuer"/> and are thereby skipped
/// silently (per iteration). A failed refresh keeps the last-known-good issuer and never stamps the client's
/// fail-fast window — so a failed initial warm-up means the first real request performs its own blocking fetch
/// (with its own Error logging) instead of being fast-failed onto the fallback.</para>
/// </summary>
internal sealed class MaskinportenWellKnownRefreshService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            // Host shutdown — end the loop cleanly.
        }
    }
    ...
    private async Task Refresh(IMaskinportenClient? service, CancellationToken stoppingToken)
    {
        ...
        {
            // Host shutdown must propagate and end the loop — never swallow it into a hot loop.
            throw;
        }
        catch (Exception ex)
        {
