...
internal static class EFormidlingStatusReader
{
    ...
    /// </summary>
    internal static bool HasLeftOutbox(MessageStatuses statuses) =>
    ...
    /// </summary>
    internal static EFormidlingShipmentStatus Classify(MessageStatuses statuses)
    {
    ...
    private static EFormidlingShipmentStatus Describe(EFormidlingDeliveryState state, MessageStatuses.Entry? entry) =>
        new()
        {
            State = state,
            Status = entry?.Status,
    ...
    /// </summary>
    private static MessageStatuses.Entry? Find(MessageStatuses statuses, params ReadOnlySpan<string> wanted)
    {
        List<MessageStatuses.Entry>? content = statuses.Content;
        if (content is null)
        {
            return null;
        }

        foreach (MessageStatuses.Entry entry in content)
        {
    ...
    private static MessageStatuses.Entry? LastOrDefault(MessageStatuses statuses)
    {
        List<MessageStatuses.Entry>? content = statuses.Content;
