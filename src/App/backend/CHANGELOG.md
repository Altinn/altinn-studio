...
- Breaking: the eFormidling status check handlers are no longer available to apps — `EformidlingStatusCheckEventHandler` is removed and `EformidlingStatusCheckEventHandler2` is now internal. The obsolete `AddEFormidlingServices<TM, TR>` overload is also removed. Use `AddEFormidlingServices2<TM, TR>`, which registers the status check for you.
