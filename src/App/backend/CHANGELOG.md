...
- Breaking: the Altinn Events **receive** stack is removed entirely. The app no longer exposes the `POST /api/v1/eventsreceiver` endpoint, and the types behind it are gone: `IEventHandler` (with the built-in `SubscriptionValidationHandler` and `UnhandledEventHandler`), `IEventHandlerResolver`, `IEventsSubscription`/`EventsSubscriptionClient`, and `IEventSecretCodeProvider` with its Key Vault implementation. Its only built-in consumer was the eFormidling reminder loop above, and the subscription secret it relied on had no rotation story — an unowned security surface. If your app subscribed to its own events as a timer, move that logic into a BPMN service task, which can now wait durably and re-poll on its own schedule; genuine inbound events from other systems need an endpoint your app owns, designed for that purpose. Publishing app events (`IEventsClient.AddEvent`) is unchanged. `studioctl app upgrade v9` reports custom `IEventHandler`, `IEventsSubscription`, and `IEventSecretCodeProvider` usages.
