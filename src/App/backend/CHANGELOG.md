...
- Add analyzer rule `ALTINNAPP0701` (error): `services.AddEFormidling();` on its own is now a build error, because it registers everything except the metadata generator only your app can provide. Complete it with `WithMetadata<T>()`, or — if your app registers its own `IEFormidlingService` and needs nothing else from the builder — discard the result explicitly with `_ =`.
