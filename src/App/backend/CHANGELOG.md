...
- Add staged service tasks — a service task can now be a typed pipeline of several durable steps. Implement `IStagedServiceTask` (sibling of `IServiceTask` under the new root `IServiceTaskBase`) and declare `Steps`: one entry step (`IServiceTaskStep<TOut>`), any number of links (`IServiceTaskStep<TIn, TOut>`), one final step (`IFinalServiceTaskStep<TIn>`). Each step expands to its own workflow-engine step — own retry budget, timeout/wait budget (`IServiceTaskStep.StepOptions`), and idempotency key (`context.StepId`) — and a completed step never re-runs: a retry or resume re-enters the pipeline at the failed step. Step outputs flow to the next step's typed `context.Input` (serialized through the workflow's callback state); only the final step can conclude the task or auto-advance the process, and this is enforced by the type system. Pipeline shape (including the type handoff between adjacent steps) is validated at app startup. Step names default to the class name and are a compatibility surface for in-flight workflows — pin via `Name` when renaming a class. This replaces the send-then-poll guidance of a single re-entrant `Execute`: put the send in its own step and let the final step poll via `ServiceTaskStepResult.Defer`/`ServiceTaskResult.Defer`.
