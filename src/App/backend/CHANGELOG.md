...
- Support several custom recipients on a correspondence notification, matching the Correspondence API, which honours every entry. Note that these recipients are notified **in addition to** the correspondence recipient's registered contact information, not instead of it — despite what the `WithRecipientOverride` naming suggests. That has always been the behaviour, and is now documented on `CorrespondenceNotification.CustomRecipients` and on each builder method.
- Add `CorrespondenceNotification.OverrideRegisteredContactInformation` and `WithOverrideRegisteredContactInformation(bool)`, which make the custom recipients replace the correspondence recipient's registered contact information instead of supplementing it. Defaults to `false`, so existing apps are unaffected. The Correspondence API rejects it without at least one custom recipient, so `CorrespondenceRequest.Validate` checks that first and throws a `CorrespondenceArgumentException` naming the problem rather than letting it surface as an opaque `400`. `WithRecipientOverride(recipient)` now accumulates and can be chained — `.WithRecipientOverride(a).WithRecipientOverride(b)` adds both — alongside a new `WithRecipientOverrides(recipients)` for adding several at once, following the same singular/plural convention as `WithAttachment`/`WithAttachments`. `WithRecipientOverrideIfConfigured(recipient)`, which ignores a `null`, is now declared on `ICorrespondenceNotificationBuilder`; it previously existed only on the concrete builder and so was unreachable through the fluent chain.
