...
- eFormidling configuration is now checked when the app starts rather than when an instance first reaches the task. Startup fails, listing every problem at once, if an eFormidling task has no configuration block, is missing a required setting for the environment being started, ships a data type not declared in `applicationmetadata.json`, or is enabled without `AddEFormidlingServices2` having been called. A task disabled for the current environment does not require the services, so an app running eFormidling only in production still starts locally.
- An eFormidling service task now waits for delivery confirmation before the process moves on: it sends the shipment, then polls the integrasjonspunkt and advances only once delivery is confirmed. A shipment that is rejected, or that the integrasjonspunkt declares expired, fails the task with a message naming what happened, so it reaches your monitoring instead of a manually drained queue. The effective deadline is about two hours — shipments carry a two-hour lifetime, set from `expectedResponseDateTime` in their own envelope (a long-standing value), and the task waits slightly longer so an expired shipment fails with the integrasjonspunkt's verdict rather than a generic timeout. The last reported status is recorded on the instance as the `eFormidlingShipmentStatus` data value, and process reads show the task as waiting with that status as the reason.
...
- Breaking: spelling across the libraries is now consistently US English, which renames a large part of the public API. The dominant change is `Organisation` to `Organization`: `OrganisationNumber`, `OrganisationNumberFormat`, `OrganisationNumberJsonConverter(Attribute)`, `OrganisationNumberExtensions`, `OrganisationOrPersonIdentifier` (and its `Organisation` case), `LookupOrganisationController`, `LookupOrganisationResponse`, `OrganisationDetails` and `MaskinportenSystemUser.Organisation` all move to the `-ization` spelling, as do the parameters, properties and methods named after them. Other renames in the same pass: `IFileAnalyser`/`IFileAnalyserFactory`/`FileAnalyserFactory` to `IFileAnalyzer`/`IFileAnalyzerFactory`/`FileAnalyzerFactory`, `FileAnalysisResult.AnalyserId` to `AnalyzerId`, `IFileAnalyser.Analyse` to `Analyze`, `InstansiationInstance` to `InstantiationInstance`, `InstansiationNotification` to `InstantiationNotification`, `AppOptionsServiceExtentions` to `AppOptionsServiceExtensions`, `RemoveBomExtentions` to `RemoveBomExtensions`, and `AddIndicies` to `AddIndices`. This is a source-level rename only — no behavior changes. `studioctl app upgrade v9` is the intended route for updating an app.
- Breaking: the file analysis namespace `Altinn.App.Core.Features.FileAnalyzis` is gone; everything now lives in `Altinn.App.Core.Features.FileAnalysis`. Two of its types were already in the correctly spelled namespace, so apps referencing them may have needed both `using` directives — now there is only one.
- Breaking: the organization lookup endpoint moves from `GET {org}/{app}/api/v1/lookup/organisation/{orgNr}` to `GET {org}/{app}/api/v1/lookup/organization/{orgNr}`, and its response field `organisationDetails` is now `organizationDetails`. The app frontend ships with the backend and is updated in step; a custom frontend or an external caller of this endpoint must be updated.
- Breaking: instance responses rename the JSON field `organisationNumber` to `organizationNumber` on the instance owner and party objects (`InstanceResponse`, `EnrichedInstanceResponse`, `PartyResponse`). The Altinn Storage fields the app forwards are untouched — `instanceOwner.organisationNumber` in a Storage payload keeps its spelling.
- Breaking: the telemetry attributes `organisation.name`, `organisation.number` and `organisation.systemuser.id` are renamed to `organization.name`, `organization.number` and `organization.systemuser.id`, and the file analysis span `FileAnalysis.Analyse` is renamed to `FileAnalysis.Analyze`. **Existing Grafana dashboards, alerts and saved queries that reference these names must be updated** — traces emitted after the upgrade will not match the old attribute keys. The helper methods `Activity.SetOrganisationName`/`SetOrganisationNumber` are renamed to `SetOrganizationName`/`SetOrganizationNumber`.
- Breaking: the `ProcessDataRead` parameter `langauge` is now spelled `language`, which matters only if you call it with a named argument.
- Misspellings are corrected throughout code comments, XML documentation, log messages and exception messages. Spellings that belong to somebody else are deliberately left as they are: the Altinn Storage fields `allowedContributers`, `enabledFileAnalysers`, `partyTypesAllowed.organisation` and `instanceOwner.organisationNumber` in `applicationmetadata.json` and instance payloads, and the `organisationNumber` field of an SMS notification recipient, all keep their spelling because they are somebody else's wire contract.
