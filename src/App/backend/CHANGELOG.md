...
- Support several custom recipients on a correspondence notification, matching the Correspondence API, which honours every entry. `WithRecipientOverride(recipient)` now accumulates and can be chained — `.WithRecipientOverride(a).WithRecipientOverride(b)` adds both — with a new `WithRecipientOverrides(recipients)` for several at once, following the same singular/plural convention as `WithAttachment`/`WithAttachments`.
- Add `WithRecipientOverrideIfConfigured(recipient)`, which ignores a `null` so an optionally-configured recipient needs no null check at the call site. It previously existed only on the concrete builder and was therefore unreachable through the fluent chain.
- Add `CorrespondenceNotification.OverrideRegisteredContactInformation` and `WithOverrideRegisteredContactInformation(bool)`, which make the custom recipients replace the correspondence recipient's registered contact information instead of supplementing it. Defaults to `false`, so existing apps are unaffected. The Correspondence API rejects it without at least one custom recipient, so `CorrespondenceRequest.Validate` checks that first and throws a `CorrespondenceArgumentException` naming the problem rather than letting it surface as an opaque `400`.
- Note that custom recipients are notified **in addition to** the correspondence recipient's registered contact information, not instead of it — despite what the `WithRecipientOverride` naming suggests. That has always been the behaviour; it is now documented on `CorrespondenceNotification.CustomRecipients` and on each builder method, and `OverrideRegisteredContactInformation` above makes the replacing behaviour reachable.

### Changed

- Breaking: `CorrespondenceNotification.CustomRecipient` becomes `CustomRecipients`, an `IReadOnlyList<CorrespondenceNotificationRecipient>`. Object-initializer callers need `CustomRecipients = [recipient]`; builder callers are unaffected. Note that repeated `WithRecipientOverride` calls previously kept only the last recipient and now keep all of them — a single call behaves as before.
- Send correspondence notification recipient overrides as the Correspondence API's current `customRecipients` list instead of its deprecated singular `customRecipient`. The API deprecated `customRecipient` in favour of the plural and normalises either into the same list, so a single recipient produces an identical notification; sending the plural keeps the client off all three deprecated recipient-override tiers rather than moving it from the oldest to the middle one.
- Breaking: `ICorrespondenceNotificationBuilder` gains `WithRecipientOverrides`, `WithRecipientOverrideIfConfigured` and `WithOverrideRegisteredContactInformation`. Additive for callers, but breaking for any external implementor of the interface.
