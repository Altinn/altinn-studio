...
- Breaking: the Altinn Events **receive** stack is removed. The app no longer exposes the `POST /api/v1/eventsreceiver` endpoint, and the types behind it are gone: `IEventHandler` (with the built-in `SubscriptionValidationHandler` and `UnhandledEventHandler`), `IEventHandlerResolver`, `IEventsSubscription`/`EventsSubscriptionClient`, and `IEventSecretCodeProvider` with its Key Vault implementation. Any `IEventHandler` your app still registers is already unreachable — the endpoint was the only thing that invoked it. If your app subscribed to its own events as a timer, move that logic into a BPMN service task, which can wait durably and re-poll on its own schedule; genuine inbound events from other systems need an endpoint your app owns, designed for that purpose. Publishing app events (`IEventsClient.AddEvent`) is unchanged. `studioctl app upgrade v9` reports usages of the removed types.
