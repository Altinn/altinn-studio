...
- eFormidling configuration is now checked when the app starts rather than when an instance first reaches the task. Startup fails, listing every problem at once, if an eFormidling task has no configuration block, is missing a required setting for the environment being started, ships a data type not declared in `applicationmetadata.json`, or is enabled without eFormidling having been registered — including the case where `AddEFormidling()` was called but never completed with `WithMetadata<T>()`. A task disabled for the current environment does not require the services, so an app running eFormidling only in production still starts locally, and an app that supplies its own `IEFormidlingService` is not asked for a metadata generator only the built-in one reads.
- Add analyzer rule `ALTINNAPP0701` (error): a registration that returns a builder which cannot work on its own — currently `AddEFormidling()` — must not have its result discarded. Calling `services.AddEFormidling();` and going no further registers everything except the metadata generator only your app can provide, which used to surface as a failure at app startup.
...
- Breaking: eFormidling is registered with a builder instead of `AddEFormidlingServices<TM>(config)` / `AddEFormidlingServices2<TM, TR>(config)`, both of which are removed. Call `services.AddEFormidling()` and complete it with `.WithMetadata<T>()`, naming the `IEFormidlingMetadata` implementation that generates your arkivmelding; add `.WithReceivers<T>()` only if the app supplies its own receivers, and `.WithConfig(...)` only to read settings from somewhere other than the `EFormidlingClientSettings` section. Each type argument now sits next to the word describing it, so the two are no longer told apart by position — and the `IConfiguration` argument is gone, since the settings section is read from the app's configuration directly. `AddEFormidling()` returns a builder that offers `WithMetadata<T>()` and nothing else, so calling the optional parts before supplying metadata will not compile. `studioctl app upgrade v9` rewrites the call.
