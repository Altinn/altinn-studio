...
- Add staged service tasks — a service task can now be a pipeline of several durable steps. Implement `IStagedServiceTask` (sibling of `IServiceTask` under the new root `IServiceTaskBase`) and declare the work steps (`Steps`, each an `IServiceTaskStep` returning `ServiceTaskStepResult.Next`/`Defer`/`FailedRetryable`/`FailedPermanent`) plus the one concluding step (`FinalStep`, an `IFinalServiceTaskStep` returning the ordinary `ServiceTaskResult` — how the task concludes, including auto-advance, is structurally reserved for the pipeline's end). Each step expands to its own workflow-engine step — own retry budget, timeout/wait budget (per-step `StepOptions`), and idempotency key (`context.StepId`) — and a completed step never re-runs: a retry or resume re-enters the pipeline at the failed step. Steps share state the way service tasks already do, through `context.InstanceDataMutator`: a completed step's data changes are saved and visible to every step after it. Step names default to the class name and are a compatibility surface for in-flight workflows — pin via `Name` when renaming a class. Registrations and pipeline shape are validated at app startup. This replaces the send-then-poll guidance of a single re-entrant `Execute`: put the send in its own step and let the final step poll via `Defer`.
