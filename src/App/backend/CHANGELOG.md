...
- Breaking: `PlatformHttpException.Response` is now a `PlatformHttpResponse` — an immutable snapshot of the failed response — instead of a live `HttpResponseMessage`. Reading `Response.StatusCode` is unaffected, and the new `exception.StatusCode` is a shorthand for it. `Response.Content` is now the response body as a string (already read, capped at 16 KB, with `ContentTruncated` telling you if it was cut off), and `Response.Headers` is a read-only dictionary with `Authorization`, `Proxy-Authorization`, `Cookie` and `Set-Cookie` values redacted. The snapshot stays readable for the lifetime of the exception; previously the underlying response was often already disposed by the time it was caught, so anything beyond the status code was unreliable. `CreateAsync` is renamed to `Create` and accepts an optional message, inner exception and cancellation token; the constructor takes a `PlatformHttpResponse`. `Create` reads the response but does not dispose it — the response stays yours, and the snapshot keeps the exception readable after you dispose it. `studioctl app upgrade v9` migrates the renamed factory and the constructor automatically, and reports the usages it cannot rewrite.
