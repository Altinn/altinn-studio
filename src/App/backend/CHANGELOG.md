...
- Breaking: `ConfigureMaskinportenClient` now redirects shared infrastructure. The default `IMaskinportenClient` is what mints the service owner tokens the workflow engine's callbacks run on, and Studio provisions its credentials at deploy time, so pointing it at your own configuration section or lambda means those credentials are never read and process transitions fail once deployed. Configure a Maskinporten client for your own integration with its own settings type and `HttpClient` registration instead, and leave the default client alone.
