...
public class ProcessNextRequestFactoryTests
{
    ...
    /// <summary>
    /// A pipeline answered by a message: one sending stage that opens the mailbox, and a conclusion
    /// that is the reply handler rather than a step of the transition. The conclusion declares its own
    /// timeout so the receive workflow's step can be shown to resolve the <c>Finally</c>'s options.
    /// </summary>
    ...
    public async Task Create_MailboxPipeline_EndsMainWithTheReceiveEnqueueAndEmitsNoConclusion()
    {
        ...
        // Assert — the conclusion is the reply handler and runs on the receive workflow, so Main
        // expands to the stages and nothing else. Emitting it here too would conclude the task twice.
        var serviceTaskSteps = ExtractServiceTaskSteps(bundle);
        ...
        // And Main's very last step is the enqueue of receiver 1 — the whole point of the step being
        // in Main at all: the receiver exists before Main settles.
        var keys = ExtractCommandKeys(bundle);
    ...
    {
        // The guard on the branch above: an ordinary pipeline is untouched — it still concludes as a
        // Main step and gains no enqueue step.
        var factory = CreateFactory(serviceTasks: new SigningTask());
        var stateChange = CreateInitialTaskStart(altinnTaskType: "signing");
    ...
    public async Task Create_MailboxPipeline_PreAssemblesTheReceiveWorkflowAsAnIndependentHead()
    {
        ...
        // A head, so the instance's collection never reads all-settled while the exchange is open;
        // depending on no head, so nothing an earlier transition left behind can condemn a workflow
        // whose only release is the rendezvous.
        ...
        // The three runtime-only values are deliberately absent here: the mailbox is not minted yet,
        // the state blob is the enqueue step's own, and the callback token is minted at that step.
        Assert.Null(receiver.Mailbox);
        Assert.Null(receiver.State);
        Assert.Null(payload.EnqueueRequest.Context);

        // Its one step is the pipeline's conclusion — the same step a non-mailbox task concludes with —
        // and it resolves the Finally's options, not the send stage's and not the task's alone.
