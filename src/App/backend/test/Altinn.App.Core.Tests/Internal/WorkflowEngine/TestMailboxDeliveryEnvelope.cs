...
/// <summary>
/// Builds a <see cref="MailboxDeliveryEnvelope"/> over an in-memory app-code, for tests that need a real
/// one — either because they exercise the envelope itself, or because
...
internal static class TestMailboxDeliveryEnvelope
{
    ...
    /// <summary>
    /// An envelope signing and verifying with a single non-expired code — the ordinary case.
    /// </summary>
    ...
    /// <summary>
    /// An envelope whose app-code is unavailable — the mounting/rotation gap in which the app can sign
    /// nothing.
    /// </summary>
    ...
    /// <summary>
    /// The signer behind <see cref="Create"/>, for tests that need the <see cref="WorkflowStateSigner"/>
    /// directly (e.g. to sign under a different domain).
    /// </summary>
