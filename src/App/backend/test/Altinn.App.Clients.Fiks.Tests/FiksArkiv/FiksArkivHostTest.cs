...
public class FiksArkivHostTest
{
    ...
    [Fact]
    public async Task MessageReceivedHandler_AdvancesParkedProcess_OnSuccess()
    {
        // Arrange: the process is parked on the Fiks Arkiv service task, and a success reply arrives.
        var fiksArkivResponseHandlerMock = new Mock<IFiksArkivResponseHandler>();
        var fiksArkivInstanceClientMock = new Mock<IFiksArkivInstanceClient>();
        var mottattMeldingMock = new Mock<IMottattMelding>();
        var svarSenderMock = new Mock<ISvarSender>();
        ...
        Instance sourceInstance = new()
        {
            ...
            Process = new ProcessState
            {
                ...
                {
                    ElementId = "Task_1",
                    AltinnTaskType = Altinn.App.Core.Constants.AltinnTaskTypes.FiksArkiv,
                },
            },
        };
        ...
        fiksArkivInstanceClientMock
            .Setup(x => x.GetInstance(It.IsAny<InstanceIdentifier>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(sourceInstance);
        fiksArkivInstanceClientMock
            .Setup(x =>
                x.ProcessMoveNext(It.IsAny<InstanceIdentifier>(), It.IsAny<string?>(), It.IsAny<CancellationToken>())
            )
            .ReturnsAsync(FiksArkivProcessNextOutcome.Advanced)
            .Verifiable(Times.Once);

        mottattMeldingMock.Setup(x => x.MeldingType).Returns(FiksArkivMeldingtype.ArkivmeldingOpprettMottatt);
        mottattMeldingMock.Setup(x => x.HasPayload).Returns(true);
        mottattMeldingMock.Setup(x => x.DecryptedPayloads).ReturnsAsync([new StreamPayload(Stream.Null, "dummy.txt")]);
        mottattMeldingMock.Setup(x => x.MeldingId).Returns(messageId);
        mottattMeldingMock
        ...
        // Assert: the parked process was advanced and the message acknowledged.
        fiksArkivInstanceClientMock.Verify();
        svarSenderMock.Verify();
    }

    [Fact]
    public async Task MessageReceivedHandler_DoesNotAdvance_OnErrorWhenMoveToNextTaskDisabled()
    {
        // Arrange: parked on the Fiks Arkiv service task, an error reply arrives, and MoveToNextTask is off.
        // The process should stay parked (no process/next), with the message acknowledged.
        var fiksArkivResponseHandlerMock = new Mock<IFiksArkivResponseHandler>();
        var fiksArkivInstanceClientMock = new Mock<IFiksArkivInstanceClient>();
        var mottattMeldingMock = new Mock<IMottattMelding>();
        var svarSenderMock = new Mock<ISvarSender>();
        ...
        {
            services.AddFiksArkiv();
            services.Configure<FiksArkivSettings>(s =>
                s.ErrorHandling = new FiksArkivErrorHandlingSettings { MoveToNextTask = false }
            );
            services.AddSingleton(fiksArkivResponseHandlerMock.Object);
            services.AddSingleton(fiksArkivInstanceClientMock.Object);
        });
        ...
        mottattMeldingMock.Setup(x => x.MeldingType).Returns(FiksArkivMeldingtype.Serverfeil);
        mottattMeldingMock.Setup(x => x.HasPayload).Returns(true);
        mottattMeldingMock.Setup(x => x.DecryptedPayloads).ReturnsAsync([new StreamPayload(Stream.Null, "dummy.txt")]);
        mottattMeldingMock.Setup(x => x.MeldingId).Returns(Guid.NewGuid());
        mottattMeldingMock
            .Setup(x => x.KlientKorrelasjonsId)
            .Returns(fixture.FiksArkivConfigResolver.GetCorrelationId(sourceInstance).ToUrlSafeBase64());
        ...
        // Assert: no process/next was issued; the failure is left to be surfaced via process state.
        fiksArkivInstanceClientMock.Verify(
            x => x.ProcessMoveNext(It.IsAny<InstanceIdentifier>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()),
            Times.Never
        );
        svarSenderMock.Verify();
    }
