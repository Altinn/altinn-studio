...
public class ProcessControllerTests : ApiTestBase, IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task CompleteProcess_WhenServiceTaskValidationFails_StopsBeforeActionAndServiceEffects()
    {
        ...
        var serviceTask = new Mock<IServiceTask>(MockBehavior.Strict);
        serviceTask.SetupGet(task => task.Type).Returns(serviceType);
        serviceTask.SetupGet(task => task.StepOptions).Returns((ProcessStepOptions?)null);
        OverrideServicesForThisTest = services =>
        {
            services.RemoveAll<IWorkflowEngineService>();
            services.RemoveAll<IProcessEngineAuthorizer>();
        ...
        try
        {
            ...
            serviceTask.VerifyGet(task => task.Type, Times.Once);
            serviceTask.VerifyGet(task => task.StepOptions, Times.AtLeastOnce);
            serviceTask.Verify(task => task.Execute(It.IsAny<ServiceTaskContext>()), Times.Never);
            serviceTask.VerifyNoOtherCalls();
            workflowEngineService.Verify(
                service =>
                    service.GetCurrentTaskWorkflowState(
                        It.Is<Instance>(instance => instance.Id == _instanceId),
