...
import { useInstantiate } from 'src/http-client/api-client/mutations/instantiate';
import { useActiveInstances } from 'src/http-client/api-client/queries/activeInstances';
...
export const InstanceSelectionWrapper = () => <InstanceSelection />;
...
export function InstanceSelection() {
  ...
  const data = useActiveInstances({ partyId: Number.parseInt(`${profile.partyId}`) });
  ...
  return (
    <div>
      <button
        onClick={async () => {
          const instance = await instantiate({ partyId: Number.parseInt(`${profile.partyId}`) });
          console.log('instance', instance);
          window.location.href = `/${window.org}/${window.app}/instance/${instance.id}`;
        }}
      >
        New instance
      </button>
...
//             onClick={async () => {
//               if (selectedParty) {
//                 const data = await instantiate({ partyId: selectedParty.partyId });
