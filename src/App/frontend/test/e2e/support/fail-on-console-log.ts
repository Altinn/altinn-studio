// Exclude known errors and log messages we don't want to fail on
export const ignoredConsoleMessages = [
  /** @see window.CypressLog */
  /^CypressLog:/,

  // Webpack stuff
  /^\[webpack-dev-server]/,

  // This ia a bug we should fix, but it's not critical.
  /^FormProvider re-rendered/,

  // Warning from MUI, because we're stuck on an old version.
  // Shows up in the Confirm task (in all-process-steps.ts) and MultipleChoiceSummary.test.tsx
  // https://github.com/Altinn/app-frontend-react/issues/2902
  /Warning: findDOMNode is deprecated and will be removed in the next major release/,

  // Bug when showing a Grid component
  // https://github.com/Altinn/app-frontend-react/issues/1851
  /Warning: validateDOMNesting.*?Add a <tbody>, <thead> or <tfoot> to your code to match the DOM tree generated by the browser/,

  /AxiosError: Request failed with status code 400/,

  // Designsystemet (v1.15) logs this on every RovingFocusRoot render. Used in LanguageSelector;
  // still functional, but deprecated with no drop-in replacement yet, so silence until we migrate.
  /Designsystemet: RovingFocusRoot is deprecated\./,
];
