...
describe('Multiple select component', () => {
  ...
  it('Adds and removes data properly when using group and soft deletion', () => {
    ...
    cy.findByRole('option', { name: /Karoline,/i }).should('exist');
    cy.get(multiselectList).contains('span', checkboxText2).click();
    cy.findByRole('option', { name: /Kåre,/i }).should('exist');
    cy.get(multiselectList).contains('span', checkboxText3).click();
    cy.findByRole('option', { name: /Johanne,/i }).should('exist');
    cy.get(multiselectList).contains('span', checkboxText4).click();
    cy.findByRole('option', { name: /Kari,/i }).should('exist');
    cy.get(multiselectList).contains('span', checkboxText5).click();
    cy.findByRole('option', { name: /Petter,/i }).should('exist');

    //Uncheck
    cy.get(multiselectList).contains('span', checkboxText4).click();
    cy.findByRole('option', { name: /Kari,/i }).should('not.exist');
    cy.get(multiselectList).contains('span', checkboxText5).click();
    cy.findByRole('option', { name: /Petter,/i }).should('not.exist');
  ...
  it('displays the values of the currently selected options', () => {
    ...
    cy.findByRole('option', {
      name: /Korte strekninger med bykjøring, eller annen moro,/i,
    }).should('exist');
    cy.findByRole('option', { name: /Kjøring i skogen,/i }).should('exist');
    cy.findByRole('option', { name: /Kjøre til hytta på fjellet,/i }).should('exist');

    // The clickable element is a psuedo-element within the button
    cy.findByRole('option', { name: /Kjøring i skogen,/i }).click('right', { force: true });
