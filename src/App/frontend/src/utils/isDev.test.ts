...
describe('isDev', () => {
  ...
  orgs.forEach((org) => {
    it(`should return true if hostname is ${org}.apps.tt02.altinn.no`, () => {
      ...
      expect(globalThis.location.hostname).toBe(`${org}.apps.tt02.altinn.no`);
      expect(isDev()).toBe(true);
    });
  });
  ...
  orgs.forEach((org) => {
    it(`should return false if hostname is ${org}.apps.altinn.no`, () => {
      ...
      expect(globalThis.location.hostname).toBe(`${org}.apps.altinn.no`);
      expect(isDev()).toBe(false);
    });
  });
});
