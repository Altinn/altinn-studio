...
// TODO: Remove when both dropdown and multiple select are migrated to src/common/typescript/form-component, since this method has been ported there when moving dropdown.
export function optionFilter(
  ...
  if (!search || (selectedLabels.length > 0 && selectedLabels.some((label) => label.toLowerCase() === search))) {
    return true;
  }
