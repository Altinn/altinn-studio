...
(async () => {
  ...
  promises.push(
    saveTsFile(path.join(CONTRACT_GENERATED_ROOT, 'expressions.generated.ts'), {
      result: `// This file is generated by running \`yarn gen\`.
import { ExprVal } from '../expression-types';
import type { ExpressionDescriptor } from '../expression-descriptor';
export const Expressions = {
  ${sortedKeys.map((key) => `${JSON.stringify(componentList[key])}: ${configMap[key].generateExpressionDescriptors()}`).join(',\n')}
} as const;
export const CommonExpressions = {
  ${getCommonTypeSources()
    .map(({ key, source }) => ({ key, definition: generateExpressionDescriptors(key, source) }))
    .filter(({ definition }) => definition !== '{}')
    .map(({ key, definition }) => `${JSON.stringify(key)}: ${definition}`)
    .join(',\n')}
} as const;`,
    }),
  );
