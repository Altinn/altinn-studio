...
export class LayoutSchemaV1 extends SchemaFile {
  ...
  async getSchema(): Promise<JSONSchema7> {
    ...
    const externallyAvailableComponentKeys = this.sortedKeys.filter(
      (key) => this.configMap[key].config.availability === 'configurable',
    );
        ...
        AnyComponent: {
          ...
          properties: {
            type: {
              ...
              enum: externallyAvailableComponentKeys.map((key) => this.componentList[key]),
            },
          },
          allOf: externallyAvailableComponentKeys.map((key) => ({
    ...
    for (const key of externallyAvailableComponentKeys) {
