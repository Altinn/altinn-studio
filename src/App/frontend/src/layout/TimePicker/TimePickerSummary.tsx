...
export const TimePickerSummary = ({ targetBaseComponentId }: Summary2Props) => {
  ...
  const config = useComponentConfig(targetBaseComponentId, 'TimePicker');
  const summaryTitle = useEvalExpression(
    config.textResourceBindings?.summaryTitle,
    Expressions.TimePicker.textResourceBindings.summaryTitle,
  );
  const resolvedTitle = useEvalExpression(
    config.textResourceBindings?.title,
    Expressions.TimePicker.textResourceBindings.title,
  );
