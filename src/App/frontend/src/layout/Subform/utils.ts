...
export function useExpressionDataSourcesForSubform(dataType: string, subformData: unknown): ExpressionDataSources {
  const baseRuntime = useExpressionDataSourcesBase({
  ...
  const langToolsSelector = useLangToolsSelectorForSubform(dataType, subformData);
  const dataModelNames = useDataModelNamesForSubform(dataType);
  ...
  return useMemo(
    () => ({
      ...
      formData: {
        ...baseRuntime.formData,
        defaultDataType: () => dataType,
        hasDataType: (candidateDataType: string) =>
          candidateDataType === dataType || dataModelNames.includes(candidateDataType),
        read: (reference: IDataModelReference) => formDataSelector(reference),
      },
    }),
    [baseRuntime, dataModelNames, dataType, formDataSelector, langToolsSelector],
  );
}
