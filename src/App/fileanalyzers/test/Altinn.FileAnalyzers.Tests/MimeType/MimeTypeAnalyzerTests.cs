...
public class MimeTypeAnalyzerTests
{
    ...
    [Fact]
    public async Task Analyze_ValidPdf_ShouldReturnCorrectMimeType()
    {
        var httpContextAccessorMock = new Mock<IHttpContextAccessor>();
        var mimeTypeAnalyzer = new MimeTypeAnalyzer(
        ...
        FileAnalysisResult analysisResult = await mimeTypeAnalyzer.Analyze(stream);

        Assert.Equal("application/pdf", analysisResult.MimeType);
    }

    [Fact]
    public async Task Analyze_InvalidPdf_ShouldReturnCorrectMimeType()
    {
        var httpContextAccessorMock = new Mock<IHttpContextAccessor>();
        var mimeTypeAnalyzer = new MimeTypeAnalyzer(
            httpContextAccessorMock.Object,
