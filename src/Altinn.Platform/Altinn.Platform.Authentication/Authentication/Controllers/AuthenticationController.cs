...
namespace Altinn.Platform.Authentication.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        /// <summary>
        /// Action for converting a JWT generated by <c>ID-porten</c> with a new JWT for further use as authentication against rest of Altinn.
        /// </summary>
        /// <returns>The result of the action. Contains the new token if the old token was valid and could be converted.</returns>
        [AllowAnonymous]
        public async Task<ActionResult> AuthenticateEndUserSystem()
        {
            string originalToken = string.Empty;
            ...
            try
            {
                ...
                string pid = originalPrincipal.Claims.Where(c => c.Type.Equals("pid")).Select(c => c.Value).FirstOrDefault();

                if (string.IsNullOrWhiteSpace(pid))
                {
                    _logger.LogInformation("Invalid person identificator claim");
                    return Unauthorized();
                }
                ...
                // claims.Add(new Claim(AltinnCoreClaimTypes.AuthenticateMethod, userProfile.A.ToString(), ClaimValueTypes.String, issuer));
                // claims.Add(new Claim(AltinnCoreClaimTypes.AuthenticationLevel, ((int)userProfile.AuthenticationLevel).ToString(), ClaimValueTypes.Integer32, issuer));
