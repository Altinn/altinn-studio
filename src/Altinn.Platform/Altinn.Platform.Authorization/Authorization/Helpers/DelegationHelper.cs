...
namespace Altinn.Platform.Authorization.Helpers
{
    ...
    public static class DelegationHelper
    {
        ...
        {
            coveredByPartyId = 0;
            if (match?.Count == 1 && match.First().Id == AltinnXacmlConstants.MatchAttributeIdentifiers.PartyAttribute && int.TryParse(match.First().Value, out coveredByPartyId) && coveredByPartyId != 0)
            {
                return true;
            }
        ...
        {
            coveredByUserId = 0;
            if (match?.Count == 1 && match.First().Id == AltinnXacmlConstants.MatchAttributeIdentifiers.UserAttribute && int.TryParse(match.First().Value, out coveredByUserId) && coveredByUserId != 0)
            {
                return true;
            }
