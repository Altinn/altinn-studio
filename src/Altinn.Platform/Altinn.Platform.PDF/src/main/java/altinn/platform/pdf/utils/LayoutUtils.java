...
public class LayoutUtils {
  ...
  /**
   * Get the height needed to the element, including title, description and
   * content
   *
   ...
   * @throws IOException
   */
  public static float getElementHeight(FormLayoutElement element, PDType0Font font, float fontSize, float width,
      float leading, float textMargin, TextResources textResources, Document formData, Instance instance)
      throws IOException {
    float height = 0;
    ...
    if (element.getType().equalsIgnoreCase("paragraph") || element.getType().equalsIgnoreCase("header")
        || element.getType().equalsIgnoreCase("panel")) {
    ...
    if (element.getType().equalsIgnoreCase("fileupload")) {
      ...
      Map<String, List<String>> attachmentsAndTags = InstanceUtils.getAttachmentsAndTagsByComponentId(element.getId(),
          instance);
  ...
  public static boolean includePageInPdf(String layoutKey, LayoutSettings layoutSettings,
      List<FormLayoutElement> formLayoutElements) {
    return (layoutSettings == null || layoutSettings.getPages() == null ||
        ...
        && checkVisibleFieldInPage(layoutSettings, formLayoutElements);
  }
   ...
   * @return boolean
   */
  private static boolean checkVisibleFieldInPage(LayoutSettings layoutSettings,
      List<FormLayoutElement> formLayoutElements) {
