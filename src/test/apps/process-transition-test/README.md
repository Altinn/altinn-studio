...
| `deferrals`   | `0` / `1` / `3`                             | How many times the service task **defers** before settling — "ran fine, the outcome isn't here yet". The engine parks the workflow in `Waiting` and re-runs the task itself. Not the same as `attempts`, which forces retryable *failures*: a deferral records no error and resets the retry counter. Only meaningful on `postCommit`. |
