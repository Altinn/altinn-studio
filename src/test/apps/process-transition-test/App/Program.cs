...
)
{
    ...
    // Gateway_PostCommit gateway routes through when path == "postCommit". It declares a step —
    // PrepareScenario runs before the task's own Execute — so every postCommit e2e scenario also
    // drives the multi-step contract (dispatch by step name, per-step durability/options). The
    // engine runs its steps as critical post-commit steps, so delays/failures are
    // frontend-observable (committed = the service task).
