...
export const useReplaceDataModelXsdMutation = (modelPath: string, meta?: MutationMeta) => {
  const { replaceDataModelXsd } = useServicesContext();
  const { org, app } = useStudioEnvironmentParams();
  const queryClient = useQueryClient();

  const mutationFn = (file: File) => {
    const formData = createFormDataWithFile(file);
    return replaceDataModelXsd(org, app, modelPath, formData);
  };
