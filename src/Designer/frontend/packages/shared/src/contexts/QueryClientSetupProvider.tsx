...
export type QueryClientSetupProviderProps = {
  children?: ReactNode;
...
const handleError = (
  ...
  if (unAuthorizedErrorCode) {
    return renderToast(errorCode || 'Unauthorized', detail, {
      onClose: async () =>
        await mutations.logout().then(() => window.location.assign(userLogoutAfterPath())),
...
export const QueryClientSetupProvider = ({
  children,
  client,
  clientConfig,
}: QueryClientSetupProviderProps) => {
