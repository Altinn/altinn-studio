...
export const useCreateAndCheckoutBranchMutation = (
  org: string,
  app: string,
  options?: UseMutationOptions<RepoStatus, AxiosError, string>,
): UseMutationResult<RepoStatus, AxiosError, string> => {
  const { createAndCheckoutBranch } = useServicesContext();
  const queryClient = useQueryClient();

  return useMutation({
    ...options,
    mutationFn: (branchName: string) => createAndCheckoutBranch(org, app, branchName),
    onSuccess: (...args) => {
