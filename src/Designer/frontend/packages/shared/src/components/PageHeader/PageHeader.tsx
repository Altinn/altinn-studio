...
const CenterContent = ({ owner }: CenterContentProps): ReactElement => {
  ...
  const isAppDashboardPath =
    pathname === appDashboardBasePath || pathname.startsWith(`${appDashboardBasePath}/`);
  const isOrgLibraryPath =
    pathname === orgLibraryBasePath || pathname.startsWith(`${orgLibraryBasePath}/`);
      ...
      <StudioPageHeader.HeaderLink
        renderLink={(props) => (
          <StudioLink href={`${DASHBOARD_BASENAME}/${APP_DASHBOARD_BASENAME}/${owner}`} {...props}>
            <span className={isAppDashboardPath ? classes.active : undefined}>
              {t('dashboard.header_item_dashboard')}
            </span>
          </StudioLink>
        )}
      />
      ...
      <StudioPageHeader.HeaderLink
        ...
        renderLink={(props) => (
          <StudioLink href={`${DASHBOARD_BASENAME}/${ORG_LIBRARY_BASENAME}/${owner}`} {...props}>
            <span className={isOrgLibraryPath ? classes.active : undefined}>
              {t('dashboard.header_item_library')}
            </span>
          </StudioLink>
        )}
      />
