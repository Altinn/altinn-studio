...
export const FavoriteItems = ({ onAddItem }: FavoriteItemsProps) => {
  const { t } = useTranslation();
  const { favorites } = useFavoriteComponents();
  ...
  return (
    <>
      ...
      {favoriteComponents.length === 0 ? (
        <StudioParagraph className={classes.noFavoritesMessage}>
          {t('ux_editor.add_item.no_favorites_message')}
        </StudioParagraph>
      ) : (
