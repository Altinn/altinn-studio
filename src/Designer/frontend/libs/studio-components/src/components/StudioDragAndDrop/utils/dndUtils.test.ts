...
import type { ExistingDndItem, NewDndItem } from '../types';
...
describe('dndUtils', () => {
  describe('getDragCursorPosition', () => {
    ...
    it('Returns Outside if item is being dragged outside of the drop target', () => {
      const monitor = {
        ...dropTargetMonitor,
        isOver: (): ReturnType<typeof isOver> => false,
      };
    ...
    it('Returns UpperHalf if item is being dragged over the upper half of the drop target', () => {
      const monitor = {
        ...dropTargetMonitor,
        getClientOffset: (): { x: number; y: number } => ({ x: xyCoord.x, y: xyCoord.y - 1 }),
      };
