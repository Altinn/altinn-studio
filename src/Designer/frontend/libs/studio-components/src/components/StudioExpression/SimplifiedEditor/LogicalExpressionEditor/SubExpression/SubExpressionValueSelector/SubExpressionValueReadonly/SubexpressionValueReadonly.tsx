...
const InstanceContextValue = ({
  ...
  return <Binding name={texts.readonlyInstanceContext} binding={<Tag color='info'>{name}</Tag>} />;
};
...
const PredefinedGatewayAction = ({
  ...
  return (
    <Binding
      ...
      binding={<Tag color='info'>{name}</Tag>}
    />
  );
};
...
const BooleanValue = ({
  ...
  const { texts } = useStudioExpressionContext();
  return (
    <Tag color={value.value ? 'success' : 'danger'}>{value.value ? texts.true : texts.false}</Tag>
  );
};
