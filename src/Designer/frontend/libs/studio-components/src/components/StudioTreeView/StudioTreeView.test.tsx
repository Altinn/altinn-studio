...
const complexComposition = (testProps: Partial<TestComponentProps> = {}): ReactElement => (
  <StudioTreeView.Root {...defaultProps} {...testProps}>
    <StudioTreeView.Item nodeId={rootNodeId1} label={rootNodeLabel1}>
      <li>
...
const createRenderFunction =
  ...
  (
    ...
    rerenderWithProps: (newProps: Partial<TestComponentProps>) => void;
  } => {
    ...
    const rerenderWithProps = (newProps: Partial<TestComponentProps>): void =>
      rerender(testComponent(newProps));
    return { ...renderResult, rerender, rerenderWithProps };
  };
...
describe('StudioTreeView', () => {
  ...
  describe.each(['simple', 'complex'])('%s composition', (simpleOrComplex: SimpleOrComplex) => {
    ...
    it('Rerenders with new selected node when the `selectedId` prop changes', () => {
      const { rerenderWithProps } = renderTreeView({ selectedId: rootNodeId2 });
      rerenderWithProps({ selectedId: rootNodeId1 });
  ...
  const getTreeitem = ({
    label,
    ...options
  }: ByRoleOptions & { label?: string } = {}): HTMLElement => {
  ...
  const queryTreeitem = ({
    label,
    ...options
  }: ByRoleOptions & { label?: string } = {}): HTMLElement | null => {
    if (label) options.name = RegExp(`^${label}( |$)`);
