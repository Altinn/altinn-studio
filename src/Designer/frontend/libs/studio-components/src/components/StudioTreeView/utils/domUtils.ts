...
export const makeDomTreeItemId = (rootId: string, id: string): string => `${rootId}-${id}-treeitem`;

export const extractIdFromTreeItem = (rootId: string, id?: string): string | undefined =>
  id ? StringUtils.removeEnd(StringUtils.removeStart(id, rootId + '-'), '-treeitem') : undefined;

export const makeDomGroupId = (rootId: string, id: string): string => `${rootId}-${id}-group`;

export const extractIdFromGroup = (rootId: string, id?: string): string | undefined =>
  id ? StringUtils.removeEnd(StringUtils.removeStart(id, rootId + '-'), '-group') : undefined;

export const findTreeItem = (rootId: string, id: string): HTMLElement | null =>
  document.getElementById(makeDomTreeItemId(rootId, id));

export const findGroup = (rootId: string, id: string): HTMLElement | null =>
  document.getElementById(makeDomGroupId(rootId, id));

const findItemLevel = (rootId: string, nodeId?: string): number =>
  nodeId ? parseInt(findTreeItem(rootId, nodeId)?.getAttribute('aria-level') ?? '1', 10) : 0;
...
export const findDirectChildIds = (rootId: string, nodeId?: string): string[] => {
  ...
  return Array.from(childItems)
    .map((item) => extractIdFromTreeItem(rootId, item.id))
...
export const findParentId = (rootId: string, nodeId: string): string | null => {
  ...
  const { id } = parentItem;
  return id === rootId ? null : (extractIdFromGroup(rootId, id) ?? null);
};
...
export const findNodeIndexWithinGroup = (rootId: string, nodeId: string): number => {
  ...
  const sameLevelIds = findDirectChildIds(rootId, parentId ?? undefined);
