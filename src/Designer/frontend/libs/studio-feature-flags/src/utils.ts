...
export const shouldDisplayFeature = (featureFlag: FeatureFlag): boolean =>
  retrieveFeatureFlags().includes(featureFlag);
...
export const addFeatureFlagToLocalStorage = (featureFlag: FeatureFlag): void => {
  const featureFlagsFromStorage = typedLocalStorage.getItem<FeatureFlag[]>(FEATURE_FLAGS_KEY) || [];
  featureFlagsFromStorage.push(featureFlag);
  typedLocalStorage.setItem(FEATURE_FLAGS_KEY, featureFlagsFromStorage);
};
...
export const removeFeatureFlagFromLocalStorage = (featureFlag: FeatureFlag): void => {
  ...
  typedLocalStorage.setItem(FEATURE_FLAGS_KEY, filteredFeatureFlags);
};
