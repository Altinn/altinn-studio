...
describe('ImportFromOrgLibraryDialog', () => {
  ...
  it('renders the select with all its options and correct default option selected', () => {
    ...
    expect(
      getOption(
        textMock('app_content_library.code_lists_with_text_resources.no_code_list_selected'),
      ),
    ).toBeInTheDocument();
  ...
  it('disables the import button when no option is selected', () => {
    ...
    const button = getButton(
      textMock('app_content_library.code_lists_with_text_resources.import_modal_import_button'),
    );
...
const getSelect = (): HTMLSelectElement =>
  screen.getByRole('combobox', {
    name: textMock('app_content_library.code_lists_with_text_resources.import_modal_select_label'),
  });
