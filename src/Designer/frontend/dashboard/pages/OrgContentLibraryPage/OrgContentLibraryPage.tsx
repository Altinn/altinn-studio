...
function MergeableOrgContentLibrary({ orgName }: MergeableOrgContentLibraryProps): ReactElement {
  ...
  const { status: publishedCodeListsStatus } = usePublishedResourcesQuery(
    orgName,
    PUBLISHED_CODE_LIST_FOLDER,
  );
...
function useCodeListsProps(orgName: string): PagesConfig['codeLists']['props'] {
  ...
  const { t } = useTranslation();
  const { data: publishedCodeLists } = usePublishedResourcesQuery(orgName, 'code_lists');
