...
describe('ApiKeysList', () => {
  ...
  it('shows expired tag for api keys past their expiry date', () => {
    renderApiKeysList();
    expect(screen.getByText(textMock('settings.user.api_keys.expired'))).toBeInTheDocument();
  });
  ...
  it('does not show expired tag for valid api keys', () => {
    ...
    expect(screen.queryByText(textMock('settings.user.api_keys.expired'))).not.toBeInTheDocument();
  });
