  ...
  if (newApiKey) {
    return (
      <StudioAlert data-color='success' className={classes.newApiKeyAlert}>
        ...
        <StudioParagraph>{t('settings.user.api_keys.new_api_key_dialog_warning')}</StudioParagraph>
        <StudioTextfield readOnly value={newApiKey} label={t('settings.user.api_keys.api_key')} />
        <StudioButton
          icon={<ClipboardIcon />}
          onClick={() => {
  ...
  return (
    <StudioCard className={classes.card}>
      <StudioCard.Block className={classes.addForm}>
        <StudioHeading level={3}>{t('settings.user.api_keys.add.header')}</StudioHeading>
        <StudioTextfield
