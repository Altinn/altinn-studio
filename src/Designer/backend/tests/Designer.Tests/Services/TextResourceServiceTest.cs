...
namespace Designer.Tests.Services
{
    public class TextResourceServiceTest
    {
        ...
        public async Task UpdateTextResourcesAsync_ShouldCreateResources(
            string org,
            string app,
            string commitId,
            int numberOfUpsertedResources
        )
        {
            // Arrange
            Mock<IAltinnStorageTextResourceClient> storageClientMock = new();
            storageClientMock
                .Setup(s => s.Upsert(org, app, It.IsAny<StorageInterface.TextResource>(), It.IsAny<string>()))
            ...
            storageClientMock.Verify(
                s =>
                    s.Upsert(
                        It.IsAny<string>(),
                        It.IsAny<string>(),
                        It.IsAny<StorageInterface.TextResource>(),
                        It.IsAny<string>()
                    ),
        ...
        private static TextResourceService GetServiceForTest(
            Mock<IAltinnStorageTextResourceClient> storageClientMock = null
        )
        {
