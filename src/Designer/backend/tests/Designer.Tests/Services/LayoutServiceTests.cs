...
public class LayoutServiceTests
{
    ...
    public async Task UsesGroup_ShouldReturnTrue_IfUsingGroups()
    {
        ...
        (AltinnRepoEditingContext editingContext, LayoutService layoutService, _) = await PrepareTestForRepo(Repo);

        bool isLayoutUsingGroups = await layoutService.IsLayoutUsingPageGroups(editingContext, "form");
    ...
    public async Task UsesGroup_ShouldReturnFalse_IfNotUsingGroups()
    {
        ...
        bool isLayoutUsingGroups = await layoutService.IsLayoutUsingPageGroups(editingContext, "layoutSet1");
    ...
    public async Task OrderToPageGroupConversion_ShouldIncludeAllPages()
    {
        ...
        PagesDto pages = PagesDto.From(await layoutService.GetLayoutSettings(editingContext, "layoutSet1"));
    ...
    public async Task PageGroupToOrderConversion_ShouldIncludeAllPages()
    {
        ...
        PagesDto pagesBeforeConvert = PagesDto.From(await layoutService.GetLayoutSettings(editingContext, "form"));
        ...
        PagesDto pages = PagesDto.From(await layoutService.GetLayoutSettings(editingContext, "form"));
        Assert.Equal(
            pages.Pages.Count,
            pagesBeforeConvert.Groups.Select((group) => group.Pages).SelectMany(pages => pages).Count()
        );
    ...
    public async Task DeletingPageGroup_ShouldDeletePagesInGroup()
    {
        ...
        await layoutService.UpdatePageGroups(editingContext, "form", (PagesWithGroups)pagesDto.ToBusiness());
        LayoutSettings updatedLayoutSettings = await layoutService.GetLayoutSettings(editingContext, "form");
        ...
        Assert.Equal(
            ...
            mediatr.Invocations.Count(i =>
                i.Method.Name == nameof(IPublisher.Publish) && i.Arguments[0] is LayoutPageDeletedEvent
            )
        );
    }
    ...
    public async Task RenamePage_ShouldNotDeletePagesInGroup()
    {
        ...
        await layoutService.RenamePage(editingContext, "form", oldPageName, newPageName);
    ...
    public async Task PageGroupAddingPages_ShouldCreateLayouts()
    {
        ...
        List<PageDto> allPages = [.. pagesDto.Groups.SelectMany((group) => group.Pages)];
        pagesDto.Groups.Add(
            new GroupDto { Name = "newGroup", Pages = [new() { Id = "newPage" }, new() { Id = "newPage2" }] }
        );
        await layoutService.UpdatePageGroups(editingContext, "form", (PagesWithGroups)pagesDto.ToBusiness());
        LayoutSettings updatedLayoutSettings = await layoutService.GetLayoutSettings(editingContext, "form");
