...
public class SchemaModelServiceTests
{
    ...
    public async Task UpdateSchema_RecreatedV9DataType_DoesNotRestoreTaskFromUiFolder(string elementName)
    {
        ...
        try
        {
            ...
            Assert.Null(Assert.Single(metadata.DataTypes, type => type.Id == "restored-model").TaskId);
        }
        finally
        {
            TestDataHelper.DeleteAppRepository(org, targetRepository, developer);
