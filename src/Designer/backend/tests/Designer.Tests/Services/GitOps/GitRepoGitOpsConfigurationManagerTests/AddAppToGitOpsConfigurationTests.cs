...
public class AddAppToGitOpsConfigurationTests
    : GitRepoGitOpsConfigurationManagerTestsBase<AddAppToGitOpsConfigurationTests>
{
    [Theory]
    [InlineData("new-app", "tt02", "existing-app1", "existing-app2")]
    [InlineData("test-app", "production", "app1", "app2", "app3")]
    public async Task WhenAppDirectoryDoesNotExist_ShouldCreateAppManifests_AndAddToEnvironment(
        string app,
        string environment,
    ...
    )
    {
        ...
        await And.When.AddAppToGitOpsConfigurationCalled(app, environment);

        await Then.AppDirectoryShouldExist(app).And.EnvironmentKustomizationManifestShouldContainApp(environment, app);
    ...
    public async Task WhenAppDirectoryExists_ShouldNotRecreateManifests_ButShouldAddToEnvironment(
        string app,
        string environment,
        params string[] existingAppsInEnvironment
    )
    ...
    public async Task WhenEnvironmentHasNoApps_ShouldCreateAppManifests_AndCreateEnvironmentWithSingleApp(
        string app,
        string environment
    )
    {
        ...
        await Then.AppDirectoryShouldExist(app).And.EnvironmentKustomizationManifestShouldContainApps(environment, app);
    }

    [Theory]
    [InlineData("duplicate-app", "tt02", "duplicate-app")]
    [InlineData("test-app", "production", "other-app", "test-app")]
    public async Task WhenAppAlreadyExistsInEnvironment_ShouldNotDuplicateApp(
        string app,
        string environment,
        params string[] existingAppsInEnvironment
    )
    {
        ...
        await But.EnvironmentShouldNotContainDuplicateApp(environment, app);
    }
    ...
    private async Task AddAppToGitOpsConfigurationCalled(string app, string environment)
    {
        ...
        await GitOpsConfigurationManager.AddAppToGitOpsConfigurationAsync(
            context,
