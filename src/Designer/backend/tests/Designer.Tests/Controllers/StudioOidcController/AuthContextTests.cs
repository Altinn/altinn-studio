...
{
    [Fact]
    public void AuthContext_WhenUnauthenticated_ReturnsNoContent()
    {
        var controller = CreateController(new ClaimsPrincipal(new ClaimsIdentity()));

        ActionResult<UserInfoResponse> result = controller.AuthContext();

        Assert.IsType<NoContentResult>(result.Result);
    }

    [Fact]
    public void AuthContext_WhenAuthenticated_ReturnsUserInformation()
    {
    ...
    [Fact]
    public void AuthContext_WhenAuthenticatedIdentityHasNoUsername_ReturnsServerError()
    {
