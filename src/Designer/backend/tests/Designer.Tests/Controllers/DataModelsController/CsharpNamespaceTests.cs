        ...
        IClassFixture<WebApplicationFactory<Program>>
{
    ...
    )
    {
        ...
        Assert.Contains(
            ...
            x => x.AppLogic != null && x.AppLogic.ClassRef == $"{expectedNamespace}.{expectedTypeName}"
        );
        Assert.DoesNotContain(
            applicationMetadata.DataTypes,
            x => x.AppLogic != null && x.AppLogic.ClassRef == $"{notExpectedNamespace}.{expectedTypeName}"
        );
    }
    ...
    )
    {
        ...
        Assert.Contains(
            applicationMetadata.DataTypes,
            x => x.AppLogic != null && x.AppLogic.ClassRef == $"{expectedNamespace}.{expectedModelName}"
        );
    ...
    )
    {
        ...
        var response = await GenerateNewJsonSchema(modelName, $"{VersionPrefix(org, targetRepository)}/new");
        Assert.Equal(HttpStatusCode.UnprocessableEntity, response.StatusCode);
