...
public class CreateAndCheckoutBranchTests
    : DesignerEndpointsTestsBase<CreateAndCheckoutBranchTests>,
        IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public CreateAndCheckoutBranchTests(WebApplicationFactory<Program> factory)
        : base(factory) { }
    ...
    public async Task CreateAndCheckoutBranch_ValidBranch_ReturnsRepoStatus(string org, string repo, string branchName)
    {
        // Arrange
        string uri = $"{VersionPrefix}/{org}/{repo}/branches/create-and-checkout";
        ...
        _branchServiceMock
            .Setup(x => x.CreateAndCheckoutBranch(authenticatedContext, branchName))
        ...
        _branchServiceMock.Verify(x => x.CreateAndCheckoutBranch(authenticatedContext, branchName), Times.Once);
    }

    [Theory]
    [InlineData("ttd", "apps-test", "feature/new-branch")]
    public async Task CreateAndCheckoutBranch_WithUncommittedChanges_ReturnsConflict(
        string org,
        string repo,
        string branchName
    )
    {
        ...
        _branchServiceMock
            .Setup(x => x.CreateAndCheckoutBranch(It.IsAny<AltinnAuthenticatedRepoEditingContext>(), branchName))
            .ThrowsAsync(new UncommittedChangesException(errorDetails));
    ...
    public async Task CreateAndCheckoutBranch_EmptyBranchName_ReturnsBadRequest(
        string org,
        string repo,
        string branchName
    )
    {
        ...
        _branchServiceMock.Verify(
            x => x.CreateAndCheckoutBranch(It.IsAny<AltinnAuthenticatedRepoEditingContext>(), It.IsAny<string>()),
            Times.Never
        );
    }
}
