...
namespace Designer.Tests.Fixtures
{
    ...
    internal class GiteaAuthDelegatingHandler : DelegatingHandler
    {
        ...
        {
            string authorizePageContent = await loginToAuthorizeRedirectedResponse.Content.ReadAsStringAsync(
                cancellationToken
            );
        ...
        )
        {
            ...
            giteaPostLoginMessage.AddCookies(
                loginRedirectResponse.GetGiteaAuthCookies().Union(redirectResponse.GetCookies("redirect_to"))
            );
