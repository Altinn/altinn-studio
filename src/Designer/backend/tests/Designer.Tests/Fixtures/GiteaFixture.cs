...
namespace Designer.Tests.Fixtures
{
    ...
    public class GiteaFixture : IAsyncLifetime
    {
        ...
        private async Task GenerateApplicationClientIdAndClientSecretInGitea()
        {
            ...
            using HttpResponseMessage addApplicationResponse = await GiteaClientRetryPolicy.ExecuteAsync(
                async _ => await GiteaClient.Value.PostAsync("user/applications/oauth2", applicationContent, _),
                CancellationToken.None
            );
