...
public class TestDbProvider
{
    ...
    public PostgreSqlContainer PostgreSqlContainer =>
        _postgreSqlContainer ?? throw new InvalidOperationException("You need to create a Postgres container first");

    private DesignerdbContext _dbContext;
    public DesignerdbContext DbContext =>
        _dbContext ?? throw new InvalidOperationException("You need to start and migrate the database first");
    ...
    private DbContextOptions<DesignerdbContext> CreatePostgresDbContextOptions() =>
        new DbContextOptionsBuilder<DesignerdbContext>().UseNpgsql(PostgreSqlContainer.GetConnectionString()).Options;
}
