...
public class CsharpEnd2EndGenerationTests : CsharpModelConversionTestsBase<CsharpEnd2EndGenerationTests>
{
    ...
    // The header marks the file as generated code, so IDEs and code-style analyzers do not flag the
    // using directives an app with implicit usings no longer needs. The directives themselves stay,
    // since the file must also compile in an app without implicit usings.
    [Theory]
    [InlineData("Model/JsonSchema/General/NonXsdContextSchema.json")]
    public void GeneratedClassesStartWithAnAutoGeneratedHeader_AndKeepTheirUsings(string jsonSchemaPath)
    {
        Given
            .That.JsonSchemaLoaded(jsonSchemaPath)
            .When.LoadedJsonSchemaConvertedToModelMetadata()
        ...
        Assert.StartsWith("// <auto-generated>", CSharpClasses);
        Assert.Contains("// </auto-generated>\n#nullable", CSharpClasses.Replace("\r\n", "\n"));
        Assert.Contains("using System;", CSharpClasses);
        Assert.Contains("using System.Collections.Generic;", CSharpClasses);
    }
