...
namespace Altinn.Studio.Designer.Controllers.Preview.V3
{
    ...
    [Route(
        "{org:regex(^(?!designer|editor|dashboard|preview|admin|resourceadm|info|start|settings))}/{app:regex(^(?!datamodels$)[[a-z]][[a-z0-9-]]{{1,28}}[[a-z0-9]]$)}/v3/instances/{partyId}/{instanceGuid}/data"
    )]
