...
namespace Altinn.Studio.Designer.Controllers.Preview
{
    ...
    [Route(
        "{org:regex(^(?!designer|editor|dashboard|preview|admin|resourceadm|info|profile))}/{app:regex(^(?!datamodels$)[[a-z]][[a-z0-9-]]{{1,28}}[[a-z0-9]]$)}/instances/{partyId}/{instanceGuid}/data"
    )]
