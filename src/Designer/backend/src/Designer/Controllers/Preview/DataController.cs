...
namespace Altinn.Studio.Designer.Controllers.Preview
{
    ...
    [Route(
        "{org:regex(^(?!(designer|editor|dashboard|preview|admin|resourceadm|info|org-settings|settings)$).+$)}/{app:regex(^(?!datamodels$)[[a-z]][[a-z0-9-]]{{1,28}}[[a-z0-9]]$)}/instances/{partyId}/{instanceGuid}/data"
    )]
    ...
    ) : Controller
    {
        ...
        [HttpPatch("{dataGuid}")]
        [UseSystemTextJson]
        public ActionResult<DataPatchResponse> Patch([FromRoute] Guid dataGuid, [FromBody] DataPatchRequest dataPatch)
        {
            JsonNode dataItem = dataService.PatchDataElement(dataGuid, dataPatch.Patch);
            return Ok(new DataPatchResponse() { ValidationIssues = [], NewDataModel = dataItem });
        }

        [HttpDelete("{dataGuid}")]
        public ActionResult<DataPostResponse> Delete([FromRoute] Guid instanceGuid, [FromRoute] Guid dataGuid)
        {
