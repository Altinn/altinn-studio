...
public class LanguageTextsIdChanged(IHubContext<SyncHub, ISyncClient> syncHub, ITextsService textsService)
    : INotificationHandler<LanguageTextsKeyChangedEvent>
{
    public async Task Handle(LanguageTextsKeyChangedEvent notification, CancellationToken cancellationToken)
    {
        List<string> updatedFiles = await textsService.UpdateRelatedFiles(
            notification.EditingContext.Org,
            notification.EditingContext.Repo,
            notification.EditingContext.Developer,
            notification.IdMutations
        );
