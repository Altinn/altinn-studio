...
public class AppScopesRepository : IAppScopesRepository
{
    ...
    )
    {
        ...
        var appScope = await _dbContext
            ...
            .SingleOrDefaultAsync(a => a.Org == repoContext.Org && a.App == repoContext.Repo, cancellationToken);
    ...
    )
    {
        AppScopesDbModel existing = await _dbContext
            .AppScopes.AsNoTracking()
            .SingleOrDefaultAsync(a => a.Org == appScopesEntity.Org && a.App == appScopesEntity.App, cancellationToken);
