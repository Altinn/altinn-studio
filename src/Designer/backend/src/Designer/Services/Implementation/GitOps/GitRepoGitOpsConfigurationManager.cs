...
public class GitRepoGitOpsConfigurationManager(
    ...
    ILogger<GitRepoGitOpsConfigurationManager> logger
) : IGitOpsConfigurationManager
{
    ...
    public async Task EnsureGitOpsConfigurationExistsAsync(
        AltinnOrgEditingContext context,
        AltinnEnvironment environment
    )
    {
        AltinnAuthenticatedRepoEditingContext gitOpsAuthenticatedContext =
    ...
    {
        var repository = gitRepositoryFactory.GetAltinnGitRepository(
            gitOpsSettings.GitOpsOrg,
            GitOpsRepoName(context.Org),
            context.Developer
        );
    ...
    private async Task EnsureEnvironmentManifests(AltinnOrgEditingContext context, AltinnEnvironment environment)
    {
        ...
        if (
            !repository.FileExistsByRelativePath(
                ManifestsPathHelper.EnvironmentManifests.KustomizationPath(environment.Name)
            )
        )
        {
            var envManifests = gitOpsManifestsRenderer.GetEnvironmentOverlayManifests(
                environment,
    ...
    private void DeleteLocalRepositoryIfExists(AltinnOrgEditingContext context)
    {
        ...
        string repoPath = serviceRepositorySettings.GetServicePath(
            gitOpsSettings.GitOpsOrg,
            GitOpsRepoName(context.Org),
            context.Developer
        );
    ...
    public async Task<bool> AppExistsInGitOpsConfigurationAsync(
        AltinnOrgEditingContext context,
        AltinnRepoName app,
        AltinnEnvironment environment
    )
    {
        ...
        if (
            !repository.DirectoryExistsByRelativePath(ManifestsPathHelper.AppManifests.AppDirectoryPath(app.Name))
    ...
    public async Task AddAppToGitOpsConfigurationAsync(AltinnRepoEditingContext context, AltinnEnvironment environment)
    {
        ...
        if (
            !repository.DirectoryExistsByRelativePath(ManifestsPathHelper.AppManifests.AppDirectoryPath(context.Repo))
        ...
        )
        {
            ...
            await WriteManifestsToFiles(
                AltinnOrgEditingContext.FromOrgDeveloper(context.Org, context.Developer),
                appManifests
            );
        }
    ...
    private static async Task<HashSet<AltinnRepoName>> GetExistingAppsFromEnvironmentKustomization(
        AltinnGitRepository repository,
        AltinnEnvironment environment
    )
    {
    ...
    public async Task RemoveAppFromGitOpsEnvironmentConfigurationAsync(
    ...
    public void PersistGitOpsConfiguration(AltinnOrgEditingContext context, AltinnEnvironment environment)
    {
        ...
        sourceControl.CommitAndPushChanges(
            ...
            $"Update GitOps configuration for environment {environment}"
        );
    }
    ...
    )
    {
        ...
        if (
            kustomization?[ManifestsPathHelper.EnvironmentManifests.KustomizationResourcesSection]
            is List<object> resources
        )
        {
