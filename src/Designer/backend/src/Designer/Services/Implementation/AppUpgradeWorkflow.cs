            ...
            steps:
              ...
              - name: Upgrade the app
                run: |
                  ...
                  case "$help_text" in
                    *--json*)
                      (cd app && studioctl app upgrade v9 --json > "$report.stdout") || exit_code=$?
                      tail -n 1 "$report.stdout" > "$report"
                      ;;
                    *)
                      echo "This studioctl cannot print the result as JSON; the result will only carry the exit code."
