...
export class ExternalApiComponent extends React.Component<
  IExternalApiComponentProps,
  IExternalApiComponentState
> {
  constructor(
    _props: IExternalApiComponentProps,
    _state: IExternalApiComponentState,
  ) {
    super(_props, _state);
    this.state = {
      apiResponse: null,
  ...
  public fetchFromApiButton = (): JSX.Element => {
    let checkForFetch = false;
    let readyToFetch = false;
    const { clientParams, metaParams } = this.props.selectedApiDef;
    const mandatoryClientParams: any = Object.keys(clientParams).filter(
      (key) => clientParams[key].required === true,
    );
    const mandatoryMetaParams: any = Object.keys(metaParams).filter(
      (key) => metaParams[key].required === true,
    );
    const mandatoryParams: any =
      mandatoryClientParams.concat(mandatoryMetaParams);
    ...
    if (checkForFetch === true) {
      const insertedRequiredParams: any = mandatoryParams.filter(
        (param: any) =>
          this.state.formDataApiTest[param] !== '' &&
          Object.keys(this.state.formDataApiTest).length >=
            mandatoryParams.length,
      );
      insertedRequiredParams.length >= mandatoryParams.length
        ? (readyToFetch = true)
        : (readyToFetch = false);
    }
      ...
      <>
        <div className='mt-2'>
          {readyToFetch === true ? (
            ...
            <button type='button' className='a-btn disabled' disabled={true}>
  ...
  public renderClientParams = (
    clientParams: any,
    clientParamValues: any,
  ): JSX.Element => {
    if (!clientParams || Object.keys(clientParams).length === 0) {
      return null;
    }
    const selectedDataModelElement = !clientParamValues
      ? ''
      : clientParamValues;
    return (
      <div>
        {Object.keys(clientParams).map((key) => {
          return (
            <div className='form-group a-form-group mt-2' key={key}>
              <h2 className='a-h4'>ClientParam: {clientParams[key].name}</h2>
              <div className='align-items-center row mt-1'>
                <div className='col-12 col'>
                  <label
                    className='a-form-label'
                    htmlFor={clientParams[key].name}
                  >
                    Enter for API test:
                  </label>
                  <input
              ...
              <div className='align-items-center row mt-1'>
                <div className='col-12 col'>
                  <SelectDataModelComponent
                    onDataModelChange={this.props.handleClientParamsChange.bind(
                      null,
                      clientParams[key].name,
                    )}
    ...
    return (
      <div>
        {Object.keys(metaParams).map((key) => {
          return (
            <div className='form-group a-form-group mt-2' key={key}>
              <h2 className='a-h4'>MetaParam: {metaParams[key].name}</h2>
              <div className='align-items-center row mt-1'>
                <div className='col-12 col'>
                  <label
                    className='a-form-label'
                    htmlFor={metaParams[key].name}
                  >
                    Enter metaparam:
                  </label>
                  <input
  ...
  public renderMappingView = (
    apiResponse: any,
    id: any,
    codelist = false,
  ): JSX.Element => {
    const resultObject =
      this.props.componentApiResponseMappingsById[id].resultObject;
      ...
      <div>
        <div className='row'>
          <div className='col-6 col'>
            ...
            <select
              name='resultObject'
              onChange={this.props.handleMappingChange.bind(
                null,
                id,
                'resultObject',
              )}
        ...
        {this.props.selectedApiDef &&
        this.props.selectedApiDef.type === 'list' ? (
          <div className='row'>
            <div className='col-6 col'>
              ...
              >
                ...
                apiResponse[resultObject].length > 0 ? (
      ...
      <div>
        ...
        {this.props.connection.externalApiId ? (
          <>
            ...
            <>
              {/* Client Parameters */}
              {this.renderClientParams(
                this.props.selectedApiDef.clientParams,
                clientParams,
              )}

              {/* Meta Parameters */}
              {this.renderMetaParams(
                this.props.selectedApiDef.metaParams,
                metaParams,
              )}
              ...
              <div className='form-group a-form-group mt-2'>
                <label className='a-form-label' htmlFor='exampleText'>
                  API Response
                </label>
                <textarea
                  name='text'
              ...
              {apiResponse ? (
                <div className='form-group a-form-group mt-2'>
                  ...
                  {this.props.componentApiResponseMappingsAllIds.map(
                    (id: any) => {
                      return (
                          ...
                          <div className='align-items-center row  a-btn-action'>
                              ...
                              <div className='row'>
                                <div className='col-12'>
                                  <SelectDataModelComponent
                                    ...
                                    selectedElement={
                                      this.props
                                        .componentApiResponseMappingsById[id]
                                        .mappingObject
                                    }
                                    hideRestrictions={true}
                                    language={this.props.language}
                            ...
                            <div className='col-2'>
                              <button
                                ...
                                onClick={this.props.removeMapping.bind(
                                  null,
                                  id,
                                )}
                              >
                ...
                <div
                  id='alert'
                  className='a-message a-message--arrow-off a-message-info mb-1'
                  role='alert'
                >
