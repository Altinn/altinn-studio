import type { IApplication, IInstance } from '../types/global';

export const getCurrentTaskDataTypeId = (
  appMetadata: IApplication,
  instance: IInstance,
) => {
  const defaultDatatype = appMetadata.dataTypes.find(
    (element) => element.appLogic !== null,
  );
  const currentTaskDataType = instance.data.find(
