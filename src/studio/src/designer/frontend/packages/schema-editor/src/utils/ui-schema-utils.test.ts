...
describe('ui-schema-utils', () => {
  describe('getDomFriendlyID', () => {
    it('removes unsupported charactes', () => {
      const idWithUnsupportedCharacters = '#my\\id: with+unsupported/CHARACTERS ^ *0'
      expect(getDomFriendlyID(idWithUnsupportedCharacters, {suffix: '1/2#3 4#5/'}))
        .toMatch(/^my-id-_with-unsupported-characters_-_-0-1-23_45--\d/)
    })
