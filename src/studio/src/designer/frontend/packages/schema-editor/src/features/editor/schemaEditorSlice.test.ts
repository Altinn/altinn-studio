...
import { CombinationKind, FieldType, getNodeByPointer } from '@altinn/schema-model';
...
describe('SchemaEditorSlice', () => {
  ...
  it('handles setRestrictionKey', () => {
    ...
    let item = getNodeByPointer(nextState.uiSchema, '#/definitions/Kommentar2000Restriksjon');
    ...
    item = getNodeByPointer(nextState.uiSchema, '#/definitions/Kommentar2000Restriksjon');
  ...
  it('handles setFieldValue', () => {
    ...
    const item = getNodeByPointer(nextState.uiSchema, '#/definitions/Kommentar2000Restriksjon');
    if (!item || !item.restrictions) {
      fail('item not found');
    }
  ...
  it('handles adding child items to a combination', () => {
    ...
    let nextState = reducer(
      ...
      addCombinationItem({
        path: '#/definitions/anyOfTestSeveralItems',
        props: { fieldType: FieldType.String, displayName: '' },
      }),
    );
