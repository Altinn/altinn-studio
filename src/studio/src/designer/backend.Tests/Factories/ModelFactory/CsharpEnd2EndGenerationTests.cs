...
public class CsharpEnd2EndGenerationTests : FluentTestsBase<CsharpEnd2EndGenerationTests>
{
    ...
    // String comparison works well for given xsd schemas. Old classes are obsolete and it's planned to removed them from solution.
    // Best approach for comparison would be to create util for comparing types from assemblies generated by new and old approach. There is no point of spending time on creating such util at this point.
    // If comparison becomes too strict for new xsd schemas consider comparing types instead of plain string content.
    // Also, old classes are not maintained anymore so Namespace feature that new classes have is added before comparison.
    private CsharpEnd2EndGenerationTests GeneratedClassesShouldBeEquivalent()
    {
        ...
        expectedClasses.Should().BeEquivalentTo(CSharpClasses);
        return this;
    }
}
