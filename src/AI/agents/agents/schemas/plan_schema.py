...
class PlanStep(BaseModel):
    ...
    def validate_atomic_field_add_files(cls, v, values):
        ...
        if task_type == "atomic_field_add":  # TODO: Can we have hardcoded task types?
            ...
            if len(v) > expected_count and not constraints.allow_extras:
                raise ValueError(
                    f"ATOMIC_FIELD_SCOPE: found {len(v)} files, expected {expected_count}, extras not allowed"
                )
    ...
    def validate_no_generated_file_edits(cls, v, values):
        ...
        if constraints.forbid_generated_edits:
            ...
            generated_patterns = [r"App/models/.*\.cs$", r"App/models/.*\.xsd$"]

            for op in v:
                for pattern in generated_patterns:
                    if re.match(pattern, op.file):
                        raise ValueError(
                            f"Operation targets generated file {op.file} (source_of_truth: {context.source_of_truth})"
                        )
        return v
...
def is_numeric_ui_component(ui_hints: Optional[UIHints]) -> bool:
    ...
    numeric_indicators = [ui_hints.input_mode == "numeric", ui_hints.format == "numeric", ui_hints.format == "number"]
