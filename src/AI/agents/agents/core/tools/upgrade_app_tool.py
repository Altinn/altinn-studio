...
The upgrade logic lives in the bundled studioctl-server. This tool POSTs
to its HTTP endpoint over a unix socket; the upgrade stages its changes
on disk, and we then mark those files so the normal commit flow picks
them up.
"""
...
_SOCKET_PATH = os.getenv("STUDIOCTL_SERVER_UNIX_SOCKET_PATH", "/tmp/studioctl-server.sock")
_UPGRADE_PATH = "/api/v1/studioctl/apps/upgrades"

# studioctl-server upgrade exit codes
_EXIT_SUCCESS = 0
...
async def _post_upgrade(project_folder: str) -> httpx.Response:
    """POST the v9 upgrade to studioctl-server over its unix socket."""
    transport = httpx.AsyncHTTPTransport(uds=_SOCKET_PATH)
    async with httpx.AsyncClient(
        transport=transport, base_url="http://_", timeout=300
    ) as client:
        return await client.post(
            _UPGRADE_PATH,
            json={"projectFolder": project_folder, "kind": "v9"},
        )
...
class UpgradeAppToV9Tool(WriteToolMixin):
    ...
    async def run(self, args: UpgradeAppToV9Args, ctx: LoopContext) -> ToolResult:
        ...
        if response.status_code != httpx.codes.OK:
            return ToolResult(
                content=f"Upgrade refused: {_get_error(response)}",
                is_error=True,
            )

        return _map_exit_code_to_tool_result(response.json(), ctx)


def _get_error(response: httpx.Response) -> str:
    try:
...
def _map_exit_code_to_tool_result(result: dict, ctx: LoopContext) -> ToolResult:
    ...
    if exit_code == _EXIT_MANUAL_ACTION_REQUIRED:
        ...
        return ToolResult(
            content=(
                "Upgraded the app to v9, but some steps need manual "
                f"follow-up:\n\n{summary}"
            )
        )
    ...
    if exit_code == _EXIT_UNSUPPORTED_VERSION:
        return ToolResult(
            content=(
                "This app is not on version 8, so it cannot be upgraded "
                f"to v9.\n\n{summary}"
            ),
            is_error=True,
        )
...
def _summarize_steps(steps: list[dict]) -> str:
    summary = "\n".join(
        f"[{message['status']}] {step['name']}: {message['text']}"
        for step in steps
