...
def summarize_policy_content(xml_content: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not root.tag.endswith("Policy"):
            results.append({"type": "error", "message": f"Root element should be 'Policy', found '{root.tag}' "})

        # Check for required attributes
        required_attrs = ["PolicyId", "Version"]
        for attr in required_attrs:
            if attr not in root.attrib:
                results.append({"type": "error", "message": f"Missing required attribute '{attr}' in Policy element"})

        # Check for rules
        rules = root.findall(".//xacml:Rule", {"xacml": "urn:oasis:names:tc:xacml:3.0:core:schema:wd-17"})
        if not rules:
            results.append({"type": "warning", "message": "WARNING: No rules found in policy file"})
...
def summarize_rule(rule: Element, rule_index: int) -> List[Dict[str, Any]]:
    ...
    if "RuleId" not in rule.attrib:
        results.append({"type": "error", "message": f"Rule #{rule_index} is missing required 'RuleId' attribute"})

    # Check for Effect attribute
    if "Effect" not in rule.attrib:
        results.append({"type": "error", "message": f"Rule #{rule_index} is missing required 'Effect' attribute"})
    else:
    ...
    if target is None:
        results.append({"type": "warning", "message": f"WARNING: Rule #{rule_index} has no Target element"})
    else:
        ...
        for any_of in any_of_elements:
            ...
            for all_of in all_of_elements:
                match_elements = all_of.findall(
                    "xacml:Match", {"xacml": "urn:oasis:names:tc:xacml:3.0:core:schema:wd-17"}
                )

                for match in match_elements:
                    # Get attribute designator to determine if this is role, resource, or action
                    attr_designator = match.find(
                        "xacml:AttributeDesignator", {"xacml": "urn:oasis:names:tc:xacml:3.0:core:schema:wd-17"}
                    )
                    ...
                    if attr_designator is not None:
                        ...
                        # Get attribute value
                        attr_value_elem = match.find(
                            "xacml:AttributeValue", {"xacml": "urn:oasis:names:tc:xacml:3.0:core:schema:wd-17"}
                        )
                        if attr_value_elem is not None and attr_value_elem.text:
                            ...
                            if category == "urn:oasis:names:tc:xacml:1.0:subject-category:access-subject":
                                ...
                                role.append({"attribute_id": attr_id, "value": attr_value, "role": role_info})
                            elif category == "urn:oasis:names:tc:xacml:3.0:attribute-category:resource":
                                resource.append({"attribute_id": attr_id, "value": attr_value})
                            elif category == "urn:oasis:names:tc:xacml:3.0:attribute-category:action":
                                action.append({"attribute_id": attr_id, "value": attr_value})
