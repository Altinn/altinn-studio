    ...
    def generate_from_json_schema(
        ...
        if generate_csharp:
            csharp_content = self.metamodel_to_csharp.convert(
                metadata, separate_namespace=False, use_nullable_reference_types=False
            )
...
def datamodel_sync(user_goal: str, schema_content: str, schema_filename: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            results = _generator.generate_from_json_schema(schema_dict, generate_xsd=True, generate_csharp=True)
        ...
        if errors:
            ...
            return {"status": "ok", "generated": generated_files, "warnings": warnings, "errors": errors}
