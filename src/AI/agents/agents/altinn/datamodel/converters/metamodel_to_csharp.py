...
class ModelMetadataToCsharpConverter:
    ...
    def convert(
        self, metadata: ModelMetadata, separate_namespace: bool = False, use_nullable_reference_types: bool = False
    ) -> str:
    ...
    def _create_class_recursive(
        ...
        if element.ParentElement is None:
            if metadata.TargetNamespace:
                lines.append(
                    f'{self._indent(1)}[XmlRoot(ElementName="{element.Name}", Namespace="{metadata.TargetNamespace}")]'
                )
            else:
        ...
        # Get child elements
        child_elements = [e for e in metadata.Elements.values() if e.ParentElement == element.ID]
        ...
        for type_name in referred_types:
            referred_element = next(
                (e for e in metadata.Elements.values() if e.TypeName == type_name and e.Type == ElementType.GROUP), None
            )
            if referred_element:
        ...
        if element.MaxOccurs > 1:
            ...
            if is_value_type:
                ...
                if not element.Nillable and element.MinOccurs == 0:
                    lines.append("")
                    lines.append(
                        f"{self._indent(2)}public bool ShouldSerialize{element.Name}() => {element.Name}.HasValue;"
                    )
            else:
    ...
    def _parse_group_property(self, lines: List[str], element: ElementMetadata, order: int, use_nullable: bool):
        ...
        if element.MaxOccurs > 1:
            lines.append(
                f"{self._indent(2)}public List<{element.TypeName}>{nullable_ref} {element.Name} {{ get; set; }}"
            )
        else:
    ...
    def _parse_attribute_property(self, lines: List[str], element: ElementMetadata, use_nullable: bool):
        ...
        if element.FixedValue:
            ...
            if cs_type == "string":
                lines.append(
                    f'{self._indent(2)}public {cs_type} {element.Name} {{ get; set; }} = "{element.FixedValue}";'
                )
            else:
                lines.append(
                    f"{self._indent(2)}public {cs_type} {element.Name} {{ get; set; }} = {element.FixedValue};"
    ...
    def _write_restriction_annotations(self, lines: List[str], element: ElementMetadata):
        ...
        if element.XsdValueType and not has_range:
            ...
            if regex:
                ...
                BaseValueType.INT,
                BaseValueType.LONG,
                BaseValueType.DOUBLE,
                BaseValueType.DECIMAL,
                BaseValueType.INTEGER,
                BaseValueType.POSITIVE_INTEGER,
                BaseValueType.NEGATIVE_INTEGER,
                BaseValueType.NON_NEGATIVE_INTEGER,
                BaseValueType.NON_POSITIVE_INTEGER,
            ]:
    ...
    def _write_altinn_row_id(self, lines: List[str]):
        ...
        lines.append(
            f"{self._indent(2)}[System.Text.Json.Serialization.JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingDefault)]"
        )
