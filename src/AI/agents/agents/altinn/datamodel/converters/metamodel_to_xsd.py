...
class ModelMetadataToXsdConverter:
    ...
    def _create_root_element(self, schema: Element, root: ElementMetadata, metadata: ModelMetadata):
        ...
        # Add child elements
        children = [e for e in metadata.Elements.values() if e.ParentElement == root.ID]
    ...
    def _create_complex_types(
        ...
        children = [
            e for e in metadata.Elements.values() if e.ParentElement == parent.ID and e.Type == ElementType.GROUP
        ]
    ...
    def _create_complex_type_definition(
        ...
        # Add child elements
        children = [e for e in metadata.Elements.values() if e.ParentElement == element.ID]

        for child in children:
            self._add_element_reference(sequence, child, metadata)

    def _add_element_reference(self, parent: Element, element: ElementMetadata, metadata: ModelMetadata):
