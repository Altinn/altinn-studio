            ...
            if operation == "create":
                ...
                try:
                    ...
                    if isinstance(item, dict) and "id" in item:
                        existing_ids = {e.get("id") for e in target if isinstance(e, dict) and "id" in e}
        ...
        for file_path in repo_root.glob(resource_pattern):
            if file_path.is_file():
                try:
                    ...
                    results.append({"file": str(file_path.relative_to(repo_root)), "error": f"Failed to process: {e}"})
...
def cleanup_feature_branch(
    ...
    try:
        ...
        subprocess.run(["git", "checkout", base], cwd=repo_path, capture_output=True, text=True, check=True)

        # Delete the feature branch
        subprocess.run(
            ["git", "branch", "-D", feature_branch], cwd=repo_path, capture_output=True, text=True, check=True
        )
