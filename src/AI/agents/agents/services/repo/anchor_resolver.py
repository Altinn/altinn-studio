...
class AnchorResolver:
    ...
    def resolve_anchor(self, layout_path: str, anchor: Anchor) -> int:
        ...
        if not isinstance(arr, list):
            raise AnchorResolutionError(f"Layout data is not an array: {type(arr)}", anchor, layout_path)
        ...
        if resolved_key:
            for i, comp in enumerate(arr):
                ...
                if text_bindings.get("title") == resolved_key:
                    log.info(
                        f"Found semantic anchor text_key '{text_key}' -> '{resolved_key}' at index {i}, inserting at {i + 1}"
                    )
                    return i + 1
    ...
    def _resolve_before_text_key(self, arr: List[Dict], text_key: str, anchor: Anchor, layout_path: str) -> int:
        ...
        if resolved_key:
            for i, comp in enumerate(arr):
                ...
                if text_bindings.get("title") == resolved_key:
                    log.info(
                        f"Found semantic anchor text_key '{text_key}' -> '{resolved_key}' at index {i}, inserting at {i}"
                    )
                    return i
    ...
    def _resolve_semantic_text_key(self, semantic_text: str, layout_path: str) -> Optional[str]:
        ...
        try:
                    ...
                    try:
                        ...
                        for resource in resources:
                            ...
                            if "poststed" in resource_value.lower() or "1.5" in resource_value:
                                log.info(
                                    f"Found poststed-related resource: id='{resource_id}', value='{resource_value}'"
                                )

                            # Check if the resource value matches our semantic text
                            if resource_value == semantic_text:
                                log.info(
                                    f"Resolved semantic text '{semantic_text}' to resource key '{resource_id}' in locale '{locale}'"
                                )
                                return resource_id
            ...
            log.warning(
                f"Could not resolve semantic text '{semantic_text}' to any resource key in available locales: {context.available_locales}"
            )
            return None
...
def inject_anchor_resolution(operations: List[Dict], anchor: Anchor, repo_path: str) -> List[Dict]:
    ...
    for op in operations:
        ...
        if op.get("op") == "insert_json_array_item" and "layout" in op.get("file", "").lower():
            layout_path = op["file"]
            try:
