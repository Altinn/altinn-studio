"""Tests for the observability feedback route."""
...
FEEDBACK_PATH = f"/api/observability/feedback/{VALID_TRACE_ID}"
DEVELOPER = "ola"
...
class TestFeedbackEndpoint:
    def test_thumbs_up_writes_score_and_returns_204(self):
        with (
            patch("api.routes.observability.get_trace_developer", return_value=DEVELOPER),
            patch("api.routes.observability.score_validation") as mock_score,
        ):
    ...
    def test_missing_developer_header_returns_400(self):
        with patch("api.routes.observability.score_validation") as mock_score:
    ...
    def test_unknown_trace_owner_returns_403(self):
        with (
            patch("api.routes.observability.get_trace_developer", return_value=None),
            patch("api.routes.observability.score_validation") as mock_score,
        ):
            response = _put_feedback({"thumbs_up": True}, headers=DEVELOPER_HEADER)
    ...
    def test_owner_mismatch_returns_403(self):
        with (
            patch(
                "api.routes.observability.get_trace_developer",
