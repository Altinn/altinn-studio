"""The trace lookup runs on the v4 observations API.

`GET /api/public/traces` returns 404 once Langfuse runs write mode
`events_only`, so the benchmark runner reads observation rows instead and takes
`traceId` off the matching root span.
"""
...
class TestFindTraceForSession:
    ...
    def test_queries_the_v4_endpoint_not_the_deprecated_one(self):
