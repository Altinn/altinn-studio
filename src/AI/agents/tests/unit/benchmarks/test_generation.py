...
def _layout(*components) -> str:
    import json

    return json.dumps({"data": {"hidden": False, "layout": list(components)}})


def _datepicker(component_id: str, **props) -> dict:
    return {
        "id": component_id,
        "type": "Datepicker",
        "dataModelBindings": {"simpleBinding": {"dataType": "model", "field": component_id}},
        **props,
    }
