...
/// loaded from the agent folder. Definitions and implementations are joined by
/// tool name; an impl without a definition (or vice versa) is an error so
/// misconfiguration fails fast at agent load rather than silently dropping a tool.
/// </summary>
public sealed class ToolRegistry : IToolRegistry
{
    ...
    /// <summary>
    /// Production constructor. Pairs each supplied <see cref="ITool"/>
        ...
        foreach (var (name, _) in _byName)
        {
            if (!definitions.TryGetValue(name, out var def))
                throw new InvalidOperationException(
                    ...
                    $"Add a {name}.json file under the agent's tools/ folder.");
            defs.Add(def);
        }
    ...
    private static IReadOnlyList<ITool> BuiltInForTesting()
    {
        ...
        // real registry data should construct a RegistryProvider pointing at
        // a real registries/ folder explicitly.
        var registries = new RegistryProvider(Path.Combine(Path.GetTempPath(), "nonexistent-registries"));
    ...
    /// <summary>The full built-in tool set, wired against the supplied registry provider.</summary>
