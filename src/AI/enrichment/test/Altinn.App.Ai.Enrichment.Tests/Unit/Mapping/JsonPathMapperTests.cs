...
namespace Altinn.App.Ai.Enrichment.Tests.Unit.Mapping;
...
public class JsonPathMapperTests
{
    ...
    private static JsonElement SampleApp() => JsonDocument.Parse("""
        {
          "SoknadsType": "standard",
          "RomKode": "R101",
          "Kontakt": {
            "Primaer": { "Fornavn": "Ola", "Etternavn": "Hansen", "Foedselsnummer": "01018012345" },
            "Sekundaer": { "FulltNavn": "Per Olsen", "Foedselsnummer": "02028012345" }
          },
          "Booking": {
            "Type": "innendoers",
            "Periode": [{ "StartDato": "2026-12-19" }],
            "AntallDeltakere": "150",
            "PrisKategori": "kategoriTo"
          },
          "SkalFornyes": true,
          "VedleggsListe": { "Rader": [{ "Navn": "vedtekter.pdf" }, { "Filnavn": "program.pdf" }] }
        }
        """).RootElement;
    ...
    private static (JsonPathMapper mapper, IDisposable cleanup) Build(string spec, DateTime? today = null)
    {
        ...
        File.WriteAllText(Path.Combine(registriesDir, "rom.json"), """
            {
              "default": { "name": "Default", "kontakt_epost": "default@example.com" },
              "entries": {
                "R101": { "name": "Møterom 101", "kontakt_epost": "booking@example.com" }
              }
            }
            """);
        File.WriteAllText(Path.Combine(registriesDir, "kategorier.json"), """
            {
              "default": "–",
              "rules": [
                { "contains": ["to", "2"], "value": "Kategori 2" },
                { "fallback": true, "value": "Kategori 1" }
              ]
            }
            """);
        File.WriteAllText(Path.Combine(registriesDir, "typer.json"), """
            {
              "default": "default-value",
              "references": ["Lov A §1", "Lov B §2"],
              "mapping": { "standard": "standard booking", "utvidet": "utvidet booking" }
            }
            """);
        ...
        var registries = new RegistryProvider(registriesDir);
    ...
    {
        var (mapper, cleanup) = Build("""{"output":{"v":{"kind":"path","source":"SoknadsType","default":"ukjent"}}}""");
        using (cleanup)
        {
            mapper.Map(SampleApp()).RootElement.GetProperty("v").GetString().Should().Be("standard");
        }
    }
    ...
    {
        var (mapper, cleanup) = Build("""
            {"output":{"v":{"kind":"chain","sources":["Missing","Kontakt.Sekundaer.FulltNavn","SoknadsType"],"default":"–"}}}
            """);
        using (cleanup)
        {
    ...
    {
        var (mapper, cleanup) = Build("""
            {"output":{"v":{"kind":"switch","source":"Booking.Type","cases":{"innendoers":"Innendørs","utendoers":"Utendørs"},"default":"–"}}}
            """);
        using (cleanup)
        {
    ...
    {
        var (mapper, cleanup) = Build("""
            {"output":{"v":{"kind":"switch","source":"SoknadsType","cases":{"utvidet":"U"},"default":"x"}}}
            """);
        using (cleanup)
        {
    ...
    {
        var (mapper, cleanup) = Build("""
            {"output":{"v":{"kind":"concat","parts":["$Kontakt.Primaer.Fornavn","$Kontakt.Primaer.Etternavn"],"separator":" "}}}
            """);
        using (cleanup)
        {
    ...
    {
        var (mapper, cleanup) = Build("""{"output":{"b":{"kind":"boolean","source":"SkalFornyes"}}}""");
        using (cleanup)
        {
    ...
    {
        var (mapper, cleanup) = Build("""{"output":{"n":{"kind":"int","source":"Booking.AntallDeltakere","default":0}}}""");
        using (cleanup)
        {
    ...
    {
        var (mapper, cleanup) = Build("""
            {"output":{"k":{"kind":"registry_field","registry":"rom.json","key_sources":["RomKode"],"field":"name","default":"–"}}}
            """);
        using (cleanup)
        {
            mapper.Map(SampleApp()).RootElement.GetProperty("k").GetString().Should().Be("Møterom 101");
        }
    }
    ...
    {
        var (mapper, cleanup) = Build("""
            {"output":{"k":{"kind":"registry_field","registry":"rom.json","key_sources":["Missing"],"field":"name","use_default":true,"default":"–"}}}
            """);
        using (cleanup)
        {
    ...
    {
        var (mapper, cleanup) = Build("""
            {"output":{"k":{"kind":"registry_field","registry":"rom.json","key_sources":["Missing"],"field":"name","fallback_sources":["SoknadsType"],"default":"–"}}}
            """);
        using (cleanup)
        {
            mapper.Map(SampleApp()).RootElement.GetProperty("k").GetString().Should().Be("standard");
        }
    }
    ...
    {
        var (mapper, cleanup) = Build("""
            {"output":{"g":{"kind":"rule_match","registry":"kategorier.json","source":"Booking.PrisKategori"}}}
            """);
        using (cleanup)
        {
            mapper.Map(SampleApp()).RootElement.GetProperty("g").GetString().Should().Be("Kategori 2");
        }
    }
    ...
    {
        var (mapper, cleanup) = Build("""
            {"output":{"t":{"kind":"mapping","registry":"typer.json","source":"SoknadsType"}}}
            """);
        using (cleanup)
        {
            mapper.Map(SampleApp()).RootElement.GetProperty("t").GetString().Should().Be("standard booking");
        }
    }
    ...
    public void ListPluck_PicksFromFirstAvailableField()
    {
        ...
        using (cleanup)
        {
            ...
            arr[0].GetString().Should().Be("vedtekter.pdf");
            arr[1].GetString().Should().Be("program.pdf");
        }
    }
    ...
    {
        var (mapper, cleanup) = Build("""
            {"output":{"refs":{"kind":"list_const","registry":"typer.json","field":"references"}}}
            """);
        using (cleanup)
        {
    ...
    {
        // Innsender (fnr=01018012345) should be excluded because Primærkontakt has same fnr.
        var app = JsonDocument.Parse("""
            {
              "Kontakt": {
                "Primaer": { "FulltNavn": "Kontakt-K", "Foedselsnummer": "01018012345" }
              },
        ...
        var (mapper, cleanup) = Build("""
            {"output":{"personer":{"kind":"list_concat","parts":[
              {"kind":"object_if_present","source":"Kontakt.Primaer","fields":{
                  "rolle":{"kind":"const","value":"Primærkontakt"},
                  "navn":{"kind":"chain","sources":["FulltNavn"]},
                  "id":{"kind":"path","source":"Foedselsnummer"}}},
              {"kind":"object_if_present","source":"Innsender",
                  "exclude_when_id_matches":["Kontakt.Primaer.Foedselsnummer"],
        ...
        using (cleanup)
        {
            ...
            arr[0].GetProperty("rolle").GetString().Should().Be("Primærkontakt");
        }
    }
