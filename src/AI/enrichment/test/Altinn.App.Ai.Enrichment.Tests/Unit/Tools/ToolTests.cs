...
namespace Altinn.App.Ai.Enrichment.Tests.Unit.Tools;
...
public class ToolTests
{
    private static readonly JsonDocument SampleApplication = JsonDocument.Parse("""
        {
          "Innsender": { "Foedselsnummer": "01018012345", "Navn": "Test Person" },
          "Booking": {
            "Periode": [
              { "StartDato": "2026-12-19", "SluttDato": "2026-12-19" }
            ],
            "Vedlegg": [
              { "FileName": "vedtekter.pdf" },
    ...
    [Theory]
    [InlineData("18:00", "02:00", 8.0, true)]    // wraps midnight, classic evening slot
    [InlineData("13:00", "03:00", 14.0, true)]   // lang varighet over midnatt
    [InlineData("09:00", "17:00", 8.0, false)]   // diurnal
    [InlineData("10:00", "10:30", 0.5, false)]   // sub-hour, fractional
    [InlineData("06:00", "03:00", 21.0, true)]   // nesten helt døgn over midnatt
    ...
    [Theory]
    [InlineData("R101", "Møterom 101")]
    [InlineData("R202", "Møterom 202")]
    public void Lookup_RomRegistry_Known_ReturnsName(string kode, string expectedName)
    {
        var tool = new LookupTool(RealRegistryProvider());
        var result = tool.Invoke(Args($$"""{ "registry": "rom", "key": "{{kode}}" }"""), SampleApplication);
        var json = JsonSerializer.SerializeToElement(result);
        json.GetProperty("name").GetString().Should().Be(expectedName);
    }

    [Fact]
    public void Lookup_RomRegistry_Unknown_ReturnsError()
    {
        var tool = new LookupTool(RealRegistryProvider());
        var result = tool.Invoke(Args("""{ "registry": "rom", "key": "X999" }"""), SampleApplication);
        var json = JsonSerializer.SerializeToElement(result);
        json.TryGetProperty("error", out _).Should().BeTrue();
    }
    ...
    private static Altinn.App.Ai.Enrichment.Registries.RegistryProvider RealRegistryProvider()
        => new(Altinn.App.Ai.Enrichment.Tests.Helpers.TestPaths.DemoRegistries);
    ...
    public void PathValue_Present_ReturnsValue()
    {
        ...
        var result = tool.Invoke(Args("""{ "json_path": "Innsender.Navn" }"""), SampleApplication);
        var json = JsonSerializer.SerializeToElement(result);
    ...
    {
        var tool = new PathValueTool();
        var result = tool.Invoke(Args("""{ "json_path": "Booking.Periode[0].StartDato" }"""), SampleApplication);
        var json = JsonSerializer.SerializeToElement(result);
        json.GetProperty("present").GetBoolean().Should().BeTrue();
    ...
    {
        var tool = new PathValueTool();
        var result = tool.Invoke(Args("""{ "json_path": "Innsender.NotAField" }"""), SampleApplication);
        var json = JsonSerializer.SerializeToElement(result);
        json.GetProperty("present").GetBoolean().Should().BeFalse();
        json.GetProperty("missing_at").GetString().Should().Be("Innsender.NotAField");
    }
    ...
    public void TextMatchesAny_ExactMatch_Found()
    {
        ...
        var result = tool.Invoke(
            Args("""{ "haystack": "Styremøte", "needles": ["styremøte", "årsmøte"] }"""),
            SampleApplication);
        var json = JsonSerializer.SerializeToElement(result);
        json.GetProperty("match").GetBoolean().Should().BeTrue();
        json.GetProperty("matched").GetString().Should().Be("styremøte");
    }
    ...
    public void TextMatchesAny_PartialMatch_NotFound()
    {
        var tool = new TextMatchesAnyTool();
        var result = tool.Invoke(
            Args("""{ "haystack": "Styremøte 2026", "needles": ["styremøte"] }"""),
            SampleApplication);
        var json = JsonSerializer.SerializeToElement(result);
    ...
    public void TextContainsAny_Substring_Found()
    {
        ...
        var result = tool.Invoke(
            Args("""{ "haystack": "Storsalen kulturhus", "needles": ["kulturhus", "samfunnshus"] }"""),
            SampleApplication);
        var json = JsonSerializer.SerializeToElement(result);
        json.GetProperty("contains").GetBoolean().Should().BeTrue();
        json.GetProperty("matched").GetString().Should().Be("kulturhus");
    }
    ...
    public void TextContainsAny_NoMatch_ReturnsFalse()
    {
        var tool = new TextContainsAnyTool();
        var result = tool.Invoke(
            Args("""{ "haystack": "Tannlegen", "needles": ["kulturhus", "samfunnshus"] }"""),
            SampleApplication);
        var json = JsonSerializer.SerializeToElement(result);
    ...
    public void ToolDefinition_SerializesParametersAsSchema()
    {
        ...
        // (used by OpenAiCompatibleChatService) from mangling JSON-schema property names
