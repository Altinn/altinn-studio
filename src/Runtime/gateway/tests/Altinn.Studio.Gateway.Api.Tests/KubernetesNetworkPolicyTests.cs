...
/// These tests require a running cluster with the gateway deployed.
...
public sealed class KubernetesNetworkPolicyTests : IAsyncLifetime
{
    private const string InternalServiceUrl =
        "http://gateway-internal.runtime-gateway.svc.cluster.local/api/v1/flux/webhook";
    private const string PublicServiceUrl = "http://gateway.runtime-gateway.svc.cluster.local/api/v1/flux/webhook";
