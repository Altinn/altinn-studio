...
) : ControllerBase
{
    ...
    private const long MaxMutationJsonSize = 1024 * 1024 * 4;
    ...
    )
    {
        ...
        {
            return ProcessStatusConflict(exception);
        }
        ...
        List<(InstanceEventType EventType, DataElement DataElement)> transactionalElementEvents =
        [];
        ...
        try
        {
            ...
            )
            {
                ...
                transactionalElementEvents.Add((InstanceEventType.Created, dataElement));
            }
            ...
            )
            {
                ...
                if (hasContentUpdate)
                {
                    ...
                    transactionalElementEvents.Add((InstanceEventType.Saved, scanElement));
                }
            ...
            // Confirmations are appended, so only the added one may be carried. Whether the
            // stakeholder already has one is settled by the repository under the instance lock, not
            // here against a snapshot read before it.
            ...
            if (mutationRequest.AddCompleteConfirmation)
            {
                ...
                [
                    new CompleteConfirmation
                    {
                        ...
                        ConfirmedOn = mutationLastChanged,
                    },
                ];
                instanceUpdateProperties.Add(nameof(Instance.CompleteConfirmations));
            }
            ...
            foreach (InstanceMutationDataElementDelete delete in deleteDataElements)
            {
                mutationInstanceEvents.Add(
                    _instanceEventService.BuildInstanceEvent(
                        InstanceEventType.Deleted,
    ...
    )> ReadMutationRequest(CancellationToken cancellationToken)
    {
            ...
            try
            {
                ...
                )
                {
                    ...
                    if (name == "mutation")
                    {
                        ...
                        if (mutationStream.Length > MaxMutationJsonSize)
                        {
                            return (
                                null,
                                null,
                                BadRequest("Mutation JSON exceeds maximum allowed size.")
                            );
                        }
        ...
        )
        {
            ...
            if (bodyStream.Length > MaxMutationJsonSize)
            {
                return (null, null, BadRequest("Mutation JSON exceeds maximum allowed size."));
            }

            mutationJson = Encoding.UTF8.GetString(bodyStream.ToArray());
        }
        else
        {
            return (
    ...
    private static JsonResult ProcessStatusConflict(ProcessStatusConflictException exception) =>
        new(
            new ProblemDetails
            {
    ...
    )
    {
        ...
        // A caller without an organization has no stakeholder to confirm as.
