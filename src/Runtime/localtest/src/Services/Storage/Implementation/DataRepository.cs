...
namespace LocalTest.Services.Storage.Implementation
{
    public class DataRepository : IDataRepository
    {
        ...
        {
            return ReadAllCore(instanceGuid, stampBlobVersionIds: true, cancellationToken);
        }

        private Task<List<DataElement>> ReadAllWithoutBlobVersionIds(
            Guid instanceGuid,
            CancellationToken cancellationToken
        )
        {
            return ReadAllCore(instanceGuid, stampBlobVersionIds: false, cancellationToken);
        }

        private async Task<List<DataElement>> ReadAllCore(
            Guid instanceGuid,
            bool stampBlobVersionIds,
            CancellationToken cancellationToken
        )
        {
        ...
        private async Task<DataElement> CreateCore(DataElement dataElement)
        {
            ...
            StampBlobVersionId(dataElement, blobVersionId);
            return dataElement;
        }
        ...
        )
        {
            ...
            List<DataElement> dataElements = await ReadAllWithoutBlobVersionIds(
                instanceGuid,
                cancellationToken
            );
        ...
        )
        {
            ...
            StampBlobVersionId(dataElement, newCurrentBlobVersion ?? currentBlobVersion);
            return dataElement;
        }
        ...
        )
        {
            ...
            StampBlobVersionId(dataElement, currentBlobVersion);
            return new DataElementReadResult(dataElement, currentBlobVersion);
        }

        private static void StampBlobVersionId(DataElement dataElement, string currentBlobVersion)
        {
            dataElement.BlobVersionId = string.IsNullOrEmpty(currentBlobVersion)
                ? null
                : currentBlobVersion;
        }
