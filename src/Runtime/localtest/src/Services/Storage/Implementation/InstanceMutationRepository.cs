...
) : IInstanceMutationRepository
{
    ...
    private async Task EnsureIdleProcessStatusWithoutVersionFence(
        Guid instanceGuid,
        InstanceMutationCommit mutation,
    ...
    )
    {
        if (
            mutation.ExpectedInstanceVersion is not null
            || mutation.ExpectedProcessStateVersion is not null
        )
        {
            return;
        }

        (Instance current, _) = instanceLockHeld
    ...
    {
        await EnsureIdleProcessStatusWithoutVersionFence(
            instanceGuid,
            mutation,
            instanceLockHeld,
            cancellationToken
    ...
    )
    {
        ...
        if (updateProperties.Contains(nameof(Instance.CompleteConfirmations)))
        {
            if (mutation.InstanceUpdates.CompleteConfirmations is { } incomingConfirmations)
            {
                ...
                // The caller decides to confirm from a snapshot read before the instance was locked,
                // so a stakeholder that has confirmed since must not be appended a second time. This
                // runs under that lock, as mergeinstanceupdate does in production storage.
                foreach (CompleteConfirmation incoming in incomingConfirmations)
                {
                    if (
                        !existingInstance.CompleteConfirmations.Exists(existing =>
                            existing.StakeholderId == incoming.StakeholderId
                        )
                    )
                    {
                        existingInstance.CompleteConfirmations.Add(incoming);
                    }
                }
            }
        }
