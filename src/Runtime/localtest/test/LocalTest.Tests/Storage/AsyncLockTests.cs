...
public sealed class AsyncLockTests
{
    ...
    [Fact]
    public async Task Lock_WhenObserverThrowsAfterQueuedAcquisition_ReleasesPermit()
    {
        using var asyncLock = new AsyncLock();
        ...
        try
        {
            ...
            Assert.False(failedAcquisition.IsCompleted);
            heldPermit.Dispose();
            heldPermit = null;
    ...
    public async Task Lock_WhenQueuedObserverThrowsAndWaitIsCanceled_DoesNotReleasePermit()
    {
        ...
        try
        {
            ...
            Assert.IsAssignableFrom<OperationCanceledException>(
                actual.Data["AsyncLockWaitException"]
            );
