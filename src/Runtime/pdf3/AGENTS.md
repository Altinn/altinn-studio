...
**Local environments follow tt02**: `studioctl env up` runs the worker from the `tt02` tag, moved by
the ring-tagging job in `.github/workflows/deploy-runtime-pdf3.yaml`, so a developer picks up a new
build as soon as it reaches tt02 — with whatever studioctl they have installed. In a
