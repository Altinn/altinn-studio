...
- `GET /api/v1/{namespace}/workflows` — cursor-paginated list of workflows. Optional filters: `status` (repeatable, case-insensitive — `Enqueued`, `Processing`, `Requeued`, `Completed`, `Failed`, `Canceled`, `DependencyFailed`, `Abandoned`, `Waiting`, `Held`; omitting it returns all statuses), `label` (repeatable, `key:value`), `collectionKey`, `isHead` (head **visibility**, not directive equality: `true` = directive true or unset, `false` = directive false — deliberately asymmetric with the response field of the same name), `cursor`, `pageSize` (default 25, max 100), `includeState` (default `true`; `false` leaves the initial state and every step's `stateOut` off — they are the app's payload, and a status reader such as Studio's admin panel must not receive instance data). Returns a `PaginatedResponse` (`data`, `pageSize`, `totalCount`, `nextCursor`) or `204 No Content` when nothing matches
- `GET /api/v1/{namespace}/workflows/{workflowId:guid}` — get single workflow with all steps; `includeState=false` as on the list
...
- `GET /api/v1/{namespace}/collections` — cursor-paginated per-collection **health** view (stable collation-defined key order, opaque cursor; heads as bare IDs), every entry carrying a `workflowCounts` rollup (`active`/`failedVisible`/`failedInvisible`/`total`, `Abandoned` excluded from the failed buckets) across all of the collection's workflows including invisible ones. Three mutually exclusive modes: list (`cursor`, `pageSize`), annotate (`key` repeatable, reports `unmatchedKeys`; over-max key counts are rejected 400, never truncated), discover (`failures=any|visible|invisible`). `204 No Content` when nothing matches (annotate always answers 200)
- `GET /api/v1/{namespace}/collections/{key}` — get a single workflow collection by key, including head workflow statuses. A **frontier view by contract**: invisible (`isHead=false`) workflows never appear and there is no rollup — use the list endpoint for health
