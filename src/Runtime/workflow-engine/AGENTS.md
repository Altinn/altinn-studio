...
- **Deferral (durable yield)**: A command may return `ExecutionResult.Defer(delay)` — "ran fine, the awaited outcome isn't available yet, re-execute me after `delay`". The step parks in the non-terminal `Waiting` status (no worker slot held; scheduled via the workflow's `backoff_until`), records no error history, and resets its retry counter. Total waiting is bounded by the step's wait budget (`command.maxWaitDuration`, default `EngineSettings.DefaultStepWaitDuration` = 24h, capped by `MaxStepWaitDuration` = 30d); when the budget is exhausted the step fails with the `wait_expired` failure reason (distinct from `execution` in `engine.workflows.execution.failed`). `POST /dashboard/skip-backoff` (and resume) also apply to `Waiting` — a waiting step can be nudged to re-check immediately, so external signals can accelerate a poll without being load-bearing.
