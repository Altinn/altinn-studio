...
public sealed class ConcurrencyLimiterTests : IDisposable
{
    ...
    {
        // Act
        var slot = await _limiter.AcquireDbSlot(cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        // Act
        var slot = await _limiter.AcquireHttpSlot(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task SlotExhaustion_BlocksUntilReleased()
    {
        ...
        var slot1 = await _limiter.AcquireHttpSlot(cancellationToken: TestContext.Current.CancellationToken);
        var slot2 = await _limiter.AcquireHttpSlot(cancellationToken: TestContext.Current.CancellationToken);
        Assert.Equal(0, _limiter.HttpSlotStatus.Available);

        // Act — try to acquire a third slot, it should block
        var acquireTask = _limiter.AcquireHttpSlot(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AcquireSlot_SupportsCancellation()
    {
        ...
        for (int i = 0; i < 3; i++)
            slots.Add(await _limiter.AcquireDbSlot(cancellationToken: TestContext.Current.CancellationToken));

        using var cts = new CancellationTokenSource();
        await cts.CancelAsync();

        // Act & Assert
        await Assert.ThrowsAsync<TaskCanceledException>(() => _limiter.AcquireDbSlot(cancellationToken: cts.Token));
