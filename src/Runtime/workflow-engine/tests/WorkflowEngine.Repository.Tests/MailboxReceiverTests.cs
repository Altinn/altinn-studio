...
public sealed class MailboxReceiverTests(PostgresFixture fixture) : IAsyncLifetime
{
    ...
    public async Task Enqueue_AgainstAClosedMailboxWithNoDelivery_IsBornRunnableWithTheClosingSignal()
    {
        ...
        // Assert — accepted, not refused: the receiver runs promptly and its handler concludes the
    ...
    {
        // The case that looks wrong and is not: an accepted delivery outranks closure. A saga replaying
        // after the deadline has to drain the backlog it was promised rather than be told the exchange
        // is over — the engine kept the message, so the engine hands it over.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
        await Deliver(repository, mailbox.Id, "msg-1");
        await repository.CloseMailbox(
        ...
        // The distinction that matters is invisible in the status, so read the fact the executor will:
        // there is a delivery at position 0, and the receiver holds position 0.
        await using var context = fixture.CreateDbContext();
        Assert.True(
    ...
    {
        // Positions fold sequentially within a flush, the same rule collection heads follow — both
        // receivers taking the position the batch started at would double-book the rendezvous.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
    ...
    {
        // The property the whole rendezvous rests on, proved rather than asserted: whatever order the
        // mailbox row lock grants, the positions handed out are 0..n-1 with no gap and no repeat.
    ...
    {
        // A receiver for a mailbox the engine does not know is a workflow nothing could ever release, so
        // it is refused rather than created. The key release is what makes the refusal repeatable: the
        // same request may be made again once the mailbox exists.
        var repository = fixture.CreateRepository();
    ...
    {
        // The namespace is re-checked under the lock rather than trusted from the request, so a caller
        // cannot reach another tenant's mailbox by naming its id.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
    ...
    {
        // Namespaces are case-insensitive everywhere in the engine, and the repository layer holds one
        // contract for the parameter: normalize on entry. Without that a mailbox minted through a route
        // spelled one way would be invisible to a workflow enqueued through the same route spelled
        // another — the same namespace by every other rule the engine has.
        var repository = fixture.CreateRepository();
        var minted = Assert
            .IsType<MailboxMintResult.Minted>(
                await repository.MintMailbox(
    ...
    {
        // A request carries one idempotency key, so the positions it would consume are held aside until
        // every receiver in it is known to be born. Here the first receiver fits and the second does
        // not, and the whole request is discarded — including the position the first had reserved.
        // Leaving that one behind would put a gap in the receivers log that no receiver ever fills, and
        // a gap is exactly what the wake's position match cannot survive.
    ...
    {
        // A request carries one idempotency key, so half of it cannot be kept. An ordinary workflow
        // batched beside a doomed receiver goes down with it rather than being created under a key that
        // reports a refusal.
        var repository = fixture.CreateRepository();
    ...
    {
        // Per-request isolation inside one flush: the refusal is a verdict about one caller's request,
        // not a transaction failure, so everything else the batch carried still commits.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
    ...
    {
        // The same number that bounds the deliveries log bounds this one: the two are views of one
        // exchange, and a receiver is as much of a cost as a message.
    ...
    {
        // The accepted-versus-kept rule, on the enqueue path. A cap reached *by* a receiver says nothing
        // about that receiver — and the request that filled the log is exactly the one a crashed saga
        // replays. Answering it with the refusal would strand a relay at its last hop.
        var repository = fixture.CreateRepository(SettingsWithLogLength(1));
        var mailbox = await MintMailbox(repository);
    ...
    {
        // A duplicate normally classifies against the stored idempotency key, which a refusal has just
        // released. Without the pairing this would find nothing to classify against and throw rather than
        // answer.
        var repository = fixture.CreateRepository();
    ...
    {
        // The flush locks a mailbox for requests that are actually new, and only those. Both halves
        // matter and are exercised against the same held lock:
        //
        //   a replay mentioning the mailbox completes without blocking — its verdict comes from
        //   engine.idempotency_keys alone and it will consume no position, so making it wait would
        //   stall every unrelated caller the write buffer batched into the same flush behind a
        //   delivery or a close;
        //
        //   a genuinely new receiver blocks — it is about to be handed a position, so it must decide
        //   against a snapshot of the mailbox that nobody can move underneath it.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
        var created = Assert.Single(await Enqueue(repository, [Receiver(mailbox.Id)], idempotencyKey: "r0"));
    ...
    {
        // The same property from the other side, and the reason the ordering matters at all: the write
        // buffer batches unrelated callers into one flush, so a mailbox held by a delivery or a close
        // must not stall workflows that have nothing to do with it.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
    ...
    {
        // Interleaving one of exactly two. The delivery holds the mailbox row lock, so the enqueue cannot
        // decide anything until it commits — and once it has, the receiver's position is behind next_idx
        // and it is born runnable with a message rather than parked waiting for one.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
    ...
    {
        // Closure versus receiver birth, in the interleaving this test can force: the close holds the
        // row lock, so the enqueue waits and then reads a mailbox that is already closed. The receiver
        // is born runnable with the closing signal, never parked on a closed mailbox — which would be a
        // receiver nothing could ever release.
        //
        // The symmetric interleaving — a close blocking behind an in-flight enqueue — is step 4's, where
        // closure gains a release half that has something to act on.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
    ...
    {
        // The status's whole meaning, against the real gate: a held receiver has not started and no
        // worker may start it. If this ever went green by claiming one, the receiver would run before
        // its delivery existed and the frozen-meaning rule would be gone.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
    ...
    {
        // The harness waits for this to reach zero before truncating. Counting a held receiver as
        // runnable would turn "wait until nothing can start" into a wait that never ends.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
    ...
    {
        // A deliberate choice, recorded rather than inherited: cancellation is *accepted* on a held
        // receiver even though nothing will act on it while it stays held. That pairing — cancel the
        // receiver, then close the mailbox — is the design's hard stop, the way to end an exchange with
        // no conclusion at all, so refusing the cancel would take the ops recipe away. The flag simply
        // waits with the row until whatever releases it does, and the fetch gate then claims it for
        // cancellation like any other flagged workflow.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
        var result = Assert.Single(await Enqueue(repository, [Receiver(mailbox.Id)]));
        var workflowId = Assert.Single(result.WorkflowIds!);
        ...
        // Still unfetchable: a pending cancellation bypasses the *timer* gate, never the status list, so
        // the flag cannot pull a held receiver into a worker before its truth is frozen.
    ...
    {
        // The executor reads a receiver's position by workflow id and expects exactly one answer. This
        // index is what makes that expectation a property of the schema.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
        var result = Assert.Single(await Enqueue(repository, [Receiver(mailbox.Id)]));
        var workflowId = Assert.Single(result.WorkflowIds!);
    ...
    public async Task AMailboxWithRegistrationsCannotBeDeleted()
    {
        ...
        // silently take the rendezvous with it.
        var repository = fixture.CreateRepository();
        var mailbox = await MintMailbox(repository);
        await Enqueue(repository, [Receiver(mailbox.Id)]);
        ...
        // 23001 restrict_violation rather than 23503 foreign_key_violation: the constraint is RESTRICT,
        // so the delete is refused outright instead of being reported as a dangling reference.
    ...
    /// <summary>
    /// A settings instance whose only interesting value is the log cap. Built here rather than mutated on
    /// the fixture's shared instance, which every other test in the collection reads.
    /// </summary>
    ...
    /// <summary>
    /// One buffered request. The body hash is derived from the idempotency key rather than randomized,
    /// because these tests replay requests: a random hash would make every replay a body conflict and
    /// hide the answer being asserted.
    /// </summary>
