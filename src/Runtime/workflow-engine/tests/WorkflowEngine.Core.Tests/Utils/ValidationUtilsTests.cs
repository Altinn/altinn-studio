...
public class ValidationUtilsTests
{
    ...
    [Fact]
    public void Validate_InheritStateFromDependsOnRef_DoesNotThrow()
    {
        ValidationUtils.ValidateWorkflowGraph([
            CreateWorkflowRequest("a"),
            CreateWorkflowRequest("b", ["a"]) with
            {
                InheritStateFrom = (WorkflowRef)"a",
            },
        ]);
    }

    [Fact]
    public void Validate_InheritStateFromExternalIdDependency_DoesNotThrow()
    {
        var externalId = Guid.NewGuid();
        ValidationUtils.ValidateWorkflowGraph([
            CreateWorkflowRequest("a") with
            {
                DependsOn = [externalId],
                InheritStateFrom = (WorkflowRef)externalId,
            },
        ]);
    }

    [Fact]
    public void Validate_InheritStateFromCombinedWithState_Throws()
    {
        var requests = new List<WorkflowRequest>
        {
            CreateWorkflowRequest("a"),
    ...
    [Fact]
    public void Validate_InheritStateFromNonDependency_Throws()
    {
        // "c" exists in the batch, but "b" does not depend on it - inheritance requires the
        // source to be terminal before the workflow starts, which only a dependency guarantees.
        var requests = new List<WorkflowRequest>
        {
            CreateWorkflowRequest("a"),
            CreateWorkflowRequest("c"),
            CreateWorkflowRequest("b", ["a"]) with
            {
                InheritStateFrom = (WorkflowRef)"c",
            },
        };

        var ex = Assert.Throws<ArgumentException>(() => ValidationUtils.ValidateWorkflowGraph(requests));
        Assert.Contains(
            "must reference one of the workflow's own DependsOn entries",
            ex.Message,
    ...
    [Fact]
    public void Validate_InheritStateFromWithoutAnyDependencies_Throws()
    {
        var requests = new List<WorkflowRequest>
        {
            CreateWorkflowRequest("a") with
