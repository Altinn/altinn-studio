...
`throttled_until` predicate is selected at startup, so the schema is fully inert.
...
grown window; a closed breaker lingers for a grace period (5 sweep intervals) during which
stragglers are cleared. Each cycle every replica refreshes an in-memory snapshot of the open
