...
/// </summary>
/// <remarks>
/// The engine, not the caller, owns the mailbox id: a caller identifies its mint attempt by
...
public sealed record MailboxCreateRequest
{
    ...
    /// <see cref="EngineSettings.MaxMailboxTimeout"/>; the engine stamps the absolute deadline from it
    /// at mint, and the deadline — not this value — is what bounds the exchange from then on.
    /// </summary>
    [JsonPropertyName("timeout")]
    public required TimeSpan Timeout { get; init; }

    /// <summary>
    /// Gets the optional workflow-collection key the mailbox belongs to. Grouping only: it places the
    /// mailbox under its collection for operators, and scopes the open-mailboxes cap. Nothing about the
    /// mailbox's lifecycle depends on it.
    /// </summary>
