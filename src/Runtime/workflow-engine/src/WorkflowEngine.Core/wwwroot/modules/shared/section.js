...
import {
    ...
    buildScheduledCardHTML,
    buildCompactScheduledCardHTML,
    setCardFilterData,
} from './cards.js';
...
window.toggleSection = (sectionId) => {
    ...
    try {
        localStorage.setItem(`section:${sectionId}`, collapsed ? '1' : '0');
    } catch {
        /* ignore */
    }
    _syncUrl();
...
window.collapseAll = (section) => {
    ...
    try {
        localStorage.setItem(`compact:${section}`, '1');
    } catch {
        /* ignore */
    }
...
window.fullAll = (section) => {
    ...
    try {
        localStorage.setItem(`compact:${section}`, '0');
    } catch {
        /* ignore */
    }
    rebuildSectionCards(section);
...
const rebuildSectionCards = (section) => {
    ...
    if (section === 'inbox') {
        ...
        for (const wf of state.recentWorkflows) {
            const card = document.getElementById(`wf-recent-${cssId(wf.databaseId)}`);
            if (card) {
                card.className = `workflow-card${compact ? ' compact' : ''}`;
...
const setupCardExpand = (container, section, isStatic, isScheduled) => {
    container.addEventListener('click', (e) => {
        ...
        if (isCompact) {
            ...
            card.style.animation = 'none';
            card.innerHTML = isScheduled
                ? buildCompactScheduledCardHTML(wf)
                : buildCompactCardHTML(wf, isStatic);
        }
        setCardFilterData(card, wf);
        _syncUrl();
    });
