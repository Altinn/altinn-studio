...
const fetchQuery = async (opts) => {
    ...
    const retried = /** @type {HTMLInputElement} */ (document.getElementById('retried-check'))
        .checked;
    ...
    try {
        ...
        if (!isGuid && state.labelFilters.size > 0) {
            ...
            for (const [key, values] of state.labelFilters) {
                if (key === 'namespace') {
                    ...
                    for (const v of values) params.set('collectionKey', v);
                } else {
        ...
        if (workflows.length === 0 && page === 0 && (dbStatus || effectiveRetried) && isGuid) {
            const fallbackRes = await fetch(
                `/dashboard/query?limit=${QUERY_PAGE}&search=${encodeURIComponent(searchTerm)}`,
            );
...
export const loadQuery = () => {
    queryPage = 0;
    queryPageCursors = [null];
    return fetchQuery({ page: 0 });
};
window.loadQuery = loadQuery;
window.queryPrev = () => {
    if (queryPage > 0) fetchQuery({ page: queryPage - 1 });
};
...
document.addEventListener('click', (e) => {
    ...
    if (!target.closest('.time-range-group'))
        document.getElementById('time-dropdown')?.classList.remove('open');
    if (!target.closest('.refresh-group'))
        document.getElementById('refresh-dropdown')?.classList.remove('open');
});

document.getElementById('time-dropdown')?.addEventListener('click', (e) => {
    const opt = /** @type {HTMLElement | null} */ (
        /** @type {HTMLElement} */ (e.target).closest('.time-option')
    );
    if (!opt) return;
    ...
    if (minutes === 'custom') {
        ...
        /** @type {HTMLInputElement} */ (document.getElementById('time-from')).value =
            toLocalDatetime(from);
        /** @type {HTMLInputElement} */ (document.getElementById('time-to')).value =
            toLocalDatetime(now);
        return;
    }
...
window.applyCustomTimeRange = () => {
    ...
    if (dropdown) {
        for (const o of dropdown.querySelectorAll('.time-option')) {
            o.classList.toggle(
                'active',
                /** @type {HTMLElement} */ (o).dataset.minutes === 'custom',
            );
        }
    }
    dropdown?.classList.remove('open');
...
document.getElementById('refresh-dropdown')?.addEventListener('click', (e) => {
    const opt = /** @type {HTMLElement | null} */ (
        /** @type {HTMLElement} */ (e.target).closest('.refresh-option')
    );
    if (!opt) return;
    ...
    if (dropdown)
        for (const o of dropdown.querySelectorAll('.refresh-option'))
            o.classList.toggle('active', o === opt);
    dropdown?.classList.remove('open');

    document
        .getElementById('refresh-dropdown-btn')
        ?.classList.toggle('active-interval', seconds > 0);
