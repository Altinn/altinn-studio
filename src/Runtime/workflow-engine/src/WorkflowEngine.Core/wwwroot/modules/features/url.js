...
export const setCustomTimeRange = (v) => {
    customTimeRange = v;
};
...
/** @param {number} v */
export const setQueryTimeRange = (v) => {
    queryTimeRange = v;
};
...
export const syncUrl = () => {
    ...
    if (customTimeRange) {
        p.set('qtf', customTimeRange.from);
        p.set('qtt', customTimeRange.to);
    ...
    const expanded = [];
    for (const [id, key] of [
        ['scheduled-section', 'sched'],
        ['live-section', 'inbox'],
        ['recent-section', 'recent'],
    ]) {
        const el = document.getElementById(id);
        if (!el) continue;
        if (id === 'scheduled-section') {
            if (!el.classList.contains('collapsed')) expanded.push(key);
        } else {
            if (el.classList.contains('collapsed')) collapsed.push(key);
        }
    }
    if (collapsed.length) p.set('c', collapsed.join(','));
    if (expanded.length) p.set('e', expanded.join(','));
    const cpt = Object.entries(state.compactSections)
        .filter(([, v]) => v)
        .map(([k]) => k);
...
export const restoreUrl = () => {
    ...
    if (isShared) {
        ...
        const rc = /** @type {HTMLInputElement | null} */ (
    ...
    if (lf) {
        for (const pair of lf.split(',').filter(Boolean)) {
            ...
            if (key && value) {
                ...
                if (!set) {
                    set = new Set();
                    state.labelFilters.set(key, set);
                }
                set.add(value);
            }
        }
    ...
    if (expParam) {
        try {
            for (const k of atob(expParam).split(',').filter(Boolean)) state.pendingExpand.add(k);
        } catch {
            /* ignore */
        }
    }
