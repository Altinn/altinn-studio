...
const highlightLine = (/** @type {string} */ line) => {
    ...
    while ((m = re.exec(line)) !== null) {
        ...
        if (/^"/.test(tok)) {
            if (/:$/.test(tok)) {
                result += `<span class="json-key">${escHtml(tok.replace(/:$/, ''))}</span>:`;
                last = re.lastIndex;
                continue;
            }
            cls = 'json-string';
...
const buildDiffHTML = (oldText, newText) => {
    ...
    html +=
        '<div class="diff-header"><div class="diff-col-header">State In</div><div class="diff-col-header">State Out</div></div>';
    let lnL = 0,
        lnR = 0;
    for (const r of rows) {
...
const buildStateContent = (data) => {
    ...
    if (stateIn == null && stateOut == null)
        return '<div class="modal-loading">No state data available</div>';
...
const renderStepDetail = (data) => {
    ...
    if (hasState)
        tabs += `<button class="modal-tab" onclick="switchModalTab(this,'state')">State</button>`;
    if (hasData)
        tabs += `<button class="modal-tab" onclick="switchModalTab(this,'data')">Data</button>`;
    ...
    if (hasState)
        body += `<div class="modal-tab-panel" data-panel="state">${buildStateContent(data)}</div>`;
    if (hasData)
        body += `<div class="modal-tab-panel" data-panel="data">${buildPayloadContent(data)}</div>`;
...
const fetchAndRender = async (wfId, stepKey, stepName, initialTab) => {
    ...
    try {
        ...
        if (stepName === 'ExecuteServiceTask' && data.command?.data) {
            try {
                const d =
                    typeof data.command.data === 'string'
                        ? JSON.parse(data.command.data)
                        : data.command.data;
                if (d.serviceTaskType)
                    dom.modalTitle.textContent = `${stepName}: ${d.serviceTaskType}`;
            } catch {
                /* ignore */
            }
        }
