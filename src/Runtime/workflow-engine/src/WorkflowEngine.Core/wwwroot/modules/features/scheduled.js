...
export const loadScheduled = async () => {
    ...
    try {
        const res = await fetch('/dashboard/scheduled');
        ...
        if (workflows.length === 0) {
            dom.scheduledContainer.innerHTML =
                '<div class="empty-state">No scheduled workflows</div>';
            return;
        }
                ...
                msUntil < 10000
                    ...
                    : msUntil < 60000
                      ...
                      : msUntil < 300000
                        ? '5m'
                        : 'later';
            card.innerHTML = state.compactSections.scheduled
                ? buildCompactScheduledCardHTML(wf)
                : buildScheduledCardHTML(wf);
