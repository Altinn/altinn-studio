...
- **`executionStartedAt`** — On the workflow and on each step: the start of the **most recent attempt** (`Workflow.ExecutionStartedAt` / `Step.ExecutionStartedAt`), stamped by the worker and persisted with that attempt's first write-back, so it survives a round trip through the database. Null until the first attempt, overwritten on every retry, deferral re-execution or reclaim, and cleared by resume. The card, chain and live durations fall back to `createdAt` only for a workflow that has never started; on a settled step, `updatedAt − executionStartedAt` is the last attempt's duration, and the step modal's Processing counter counts up from it.
