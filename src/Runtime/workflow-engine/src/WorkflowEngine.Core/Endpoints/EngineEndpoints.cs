        ...
        workflowGroup
            ...
            .WithDescription(
                "Lists workflows in the namespace, newest first. Optionally filtered by `status` (repeatable, "
                    + "case-insensitive), `label` (`key:value`, repeatable), and `collectionKey`. Cursor-paginated: "
                    + "pass the `nextCursor` from a response back as `cursor`. Returns `204 No Content` when nothing "
                    + "matches, and `400 Bad Request` for an unrecognized `status` value."
            );
