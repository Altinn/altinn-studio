...
1. Fires `ITERATIONS` POST requests to the enqueue endpoint with unique correlation IDs, spread across `VUS` concurrent virtual users.
...
| `BASE_URL`   | `http://localhost:8080/api/v1/workflows` | Workflow engine base URL        |
...
Continuously sends POST requests with a fixed correlation ID at a steady interval. Useful for generating telemetry while building Grafana dashboards or during manual testing. Runs until cancelled with Ctrl+C.
...
| `INSTANCE_GUID` | `f13f515d-17b2-4f86-9c7a-a955583c4a1c`   | GUID used as correlationId/instanceGuid |
| `BASE_URL`      | `http://localhost:8080/api/v1/workflows` | Workflow engine base URL                |
| `API_KEY`       | `0544ba8b-2d8a-4ec9-b93a-47cdbd220293`   | API key for authentication              |
...
All scripts use `process-next-payload.json` in this directory as the POST body template. The `correlationId`, `instanceGuid`, and `idempotencyKey` fields are replaced with unique values per request at runtime. The `org`, `app`, and `instanceOwnerPartyId` fields have defaults in the template (`test-org`, `test-app`, `12345`).
