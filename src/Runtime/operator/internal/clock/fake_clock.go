...
//
//nolint:govet // Field order keeps the mutable clock state grouped; this helper is not size-sensitive.
...
func (f *FakeClock) Advance(d time.Duration) {
	...
	for {
		...
		switch kind {
		case eventNone:
			panic("clock: unexpected empty event kind")
		case eventTimer:
			timer.due = true
...
func (f *FakeClock) nextDueEventLocked(target time.Time) (eventKind, *timerEvent, *FakeTicker, time.Time) {
	...
	for ticker := range f.tickers {
		...
		if bestKind == eventNone ||
			ticker.next.Before(bestTime) ||
			(ticker.next.Equal(bestTime) && ticker.id < bestID) {
