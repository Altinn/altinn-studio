...
func TestStripMultiplexedHeaderBytes(t *testing.T) {
	tests := []struct {
		name  string
		input []byte
		want  []byte
		...
		{
			name:  "stdout header",
			input: []byte{0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x05, 'h', 'e', 'l', 'l', 'o'},
			want:  []byte{'h', 'e', 'l', 'l', 'o'},
		},
		{
			name:  "no header",
			input: []byte("regular text"),
			want:  []byte("regular text"),
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := docker.StripMultiplexedHeaderBytes(tt.input)
			if string(got) != string(tt.want) {
				t.Errorf("StripMultiplexedHeaderBytes() = %v, want %v", got, tt.want)
			}
		})
	}
}
