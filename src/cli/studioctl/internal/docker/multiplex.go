...
func StripMultiplexedHeader(line string) string {
	...
	// Check if first byte is stdout (1) or stderr (2) stream type
	firstByte := line[0]
...
// StripMultiplexedHeaderBytes removes the 8-byte Docker multiplexed log header if present.
// Same as StripMultiplexedHeader but operates on byte slices.
func StripMultiplexedHeaderBytes(data []byte) []byte {
	if len(data) < multiplexHeaderSize {
		return data
	}

	// Check if first byte is stdout (1) or stderr (2) stream type
	if data[0] != streamTypeStdout && data[0] != streamTypeStderr {
		return data
	}

	return data[multiplexHeaderSize:]
}
