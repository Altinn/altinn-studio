...
func (c *ShellCommand) runAlias(_ context.Context, args []string) error {
...
func getPowerShellProfilePath() (string, error) {
	...
	// Or for PowerShell Core: ~\Documents\PowerShell\Microsoft.PowerShell_profile.ps1
	home, err := os.UserHomeDir()
	if err != nil {
		return "", fmt.Errorf("getting home directory: %w", err)
	}

	// Use WindowsPowerShell path as default (most common)
