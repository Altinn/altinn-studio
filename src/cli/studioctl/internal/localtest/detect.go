...
// IsDevModeAvailable checks if dev mode can be used from the given directory.
// This is a convenience function that returns true if dev mode is enabled and source files are available.
func IsDevModeAvailable(cwd string) bool {
	mode, _ := DetectImageMode(cwd)
	return mode == DevMode
}
