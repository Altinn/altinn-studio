...
func TestIsDevModeAvailable(t *testing.T) {
	t.Run("returns true when dev mode available", func(t *testing.T) {
		root := t.TempDir()
		if err := os.Mkdir(filepath.Join(root, ".git"), perm.DirPermDefault); err != nil {
			t.Fatalf("create .git: %v", err)
		}
		...
		if err := os.WriteFile(
			filepath.Join(localtestDir, "Dockerfile"),
			[]byte("FROM alpine"),
			perm.FilePermDefault,
		); err != nil {
			t.Fatalf("create Dockerfile: %v", err)
		}

		t.Setenv(localtest.EnvDevMode, "true")

		if !localtest.IsDevModeAvailable(root) {
			t.Error("IsDevModeAvailable() = false, want true")
		}
	})
	...
	t.Run("returns false when env var not set", func(t *testing.T) {
		...
		// Explicitly unset env var for test isolation
		t.Setenv(localtest.EnvDevMode, "")
		if localtest.IsDevModeAvailable(root) {
			t.Error("IsDevModeAvailable() = true, want false (env var not set)")
		}
	})

	t.Run("returns false when no repo found", func(t *testing.T) {
		t.Setenv(localtest.EnvDevMode, "true")
		if localtest.IsDevModeAvailable(t.TempDir()) {
			t.Error("IsDevModeAvailable() = true, want false")
		}
	})
}
