...
// Uses owner-only permissions for security.
func SaveCredentials(homeDir string, creds *Credentials) error {
	...
	if err := os.WriteFile(credPath, data, perm.FilePermOwnerOnly); err != nil {
...
// Returns the env name itself if not a known environment (for custom hosts).
