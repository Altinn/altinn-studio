...
const (
	// DefaultHome is the default home directory name.
	DefaultHome = ".altinn-studio"
...
type Config struct {
	...
	Debug     bool         // Debug output (-vv or STUDIOCTL_DEBUG=1)
}
...
func New(flags Flags, version string) (*Config, error) {
	...
	socketDir := resolveSocketDir(flags.SocketDir, home)
	...
	cfg := &Config{
		...
		Debug:     flags.Verbose >= 2 || os.Getenv(EnvDebug) == "1",
	}
...
// resolveHome determines the home directory from flags, env, or default.
func resolveHome(flagValue string) (string, error) {
	...
	return filepath.Join(userHome, DefaultHome), nil
}

// resolveSocketDir determines the socket directory from flags, env, or home.
func resolveSocketDir(flagValue, home string) string {
	if flagValue != "" {
		return flagValue
	}
...
func Install(homeDir string, force bool) error {
	...
	if err := os.WriteFile(path, embeddedConfig, perm.FilePermOwnerOnly); err != nil {
