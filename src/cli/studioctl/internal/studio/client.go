...
// NewClientForTesting creates a client configured for test servers (HTTP instead of HTTPS).
func NewClientForTesting(host, token, username string, httpClient *http.Client) *Client {
	if httpClient == nil {
		httpClient = &http.Client{Timeout: httpTimeout}
	}
	return &Client{
...
func (c *Client) CloneRepo(ctx context.Context, org, repo, destPath string) error {
	...
	// Verify repo exists before cloning
