...
type NetworkMetadata struct {
	...
	LocalDNS    string `yaml:"localDns"`    // Resolved IP for local.altinn.cloud
...
type CacheStatus struct {
	IP     string        // cached IP (empty if not cached or invalid)
...
func NewNetworking(client container.ContainerClient, cfg *config.Config, out *ui.Output) *Networking {
	return &Networking{
		client: client,
		cfg:    cfg,
		out:    out,
...
func GetCacheStatus(configDir string) CacheStatus {
	...
	if err != nil {
		return CacheStatus{IP: "", Age: 0, Fresh: false, Exists: false}
	}
	...
	if !ok {
		return CacheStatus{IP: "", Age: age, Fresh: false, Exists: true}
	}
...
// localDNSHostname is the hostname used for DNS resolution testing.
const localDNSHostname = "local.altinn.cloud"
...
func (n *Networking) RefreshNetworkMetadata(ctx context.Context) (NetworkMetadata, error) {
	...
	if err := n.client.ContainerRemove(ctx, containerName, true); err != nil {
		n.out.Debugf("failed to remove existing network probe container: %v", err)
	}
	...
	defer func() {
		if cerr := logs.Close(); cerr != nil {
			n.out.Debugf("failed to close log stream: %v", cerr)
		}
	}()
	...
	if err := writeMetadataCache(cachePath, metadata); err != nil {
		n.out.Debugf("failed to cache network metadata: %v", err)
	}
...
func readMetadataCache(path string, maxAge time.Duration) (NetworkMetadata, bool) {
	...
	//nolint:gosec // G304: path is constructed from trusted configDir, not user input
...
func writeMetadataCache(path string, metadata NetworkMetadata) error {
	...
	if err := os.MkdirAll(dir, perm.DirPermOwnerOnly); err != nil {
