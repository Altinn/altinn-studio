...
var (
	...
	errTestExisting = errors.New("existing error")
)
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(t *testing.T, dataDir string) {
				...
				if err := os.WriteFile(filepath.Join(testdataPath, "dummy.txt"), []byte("test"), 0o644); err != nil {
					t.Fatal(err)
				}
			},
		},
		{
			...
			setup: func(t *testing.T, dataDir string) {
				...
				if err := os.WriteFile(filepath.Join(dataDir, versionFile), []byte("v0.9.0\n"), 0o644); err != nil {
					t.Fatal(err)
				}
			},
		},
		{
			...
			setup: func(t *testing.T, dataDir string) {
				...
				if err := os.WriteFile(filepath.Join(dataDir, versionFile), []byte("v1.0.0\n"), 0o644); err != nil {
					t.Fatal(err)
				}
			},
		},
		{
			...
			setup: func(t *testing.T, dataDir string) {
				...
				if err := os.WriteFile(filepath.Join(testdataPath, "file.json"), []byte("{}"), 0o644); err != nil {
					t.Fatal(err)
				}
				if err := os.WriteFile(filepath.Join(dataDir, versionFile), []byte("  v1.0.0  \n"), 0o644); err != nil {
					t.Fatal(err)
...
func testInstallForceReinstall(t *testing.T) {
	...
	t.Setenv(EnvResourcesTarball, tarball)
...
func testInstallTarballNotFound(t *testing.T) {
	dataDir := t.TempDir()
	t.Setenv(EnvResourcesTarball, "/nonexistent/path.tar.gz")
...
func TestInstallFromRelease(t *testing.T) {
	t.Run("successful download", testInstallFromReleaseSuccess)
	t.Run("404 not found", testInstallFromRelease404)
	t.Run("500 server error", testInstallFromRelease500)
}

func testInstallFromReleaseSuccess(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		tarData := createTestTarGz(t, map[string]string{
			"testdata/remote.txt": "from remote",
		})
		w.WriteHeader(http.StatusOK)
		if _, writeErr := w.Write(tarData); writeErr != nil {
	...
	if extractErr := extractTarGz(resp.Body, dataDir); extractErr != nil {
		t.Fatalf("extractTarGz: %v", extractErr)
	}

	if finishErr := finishInstall(Options{DataDir: dataDir, Version: "v1.0.0"}); finishErr != nil {
		t.Fatalf("finishInstall: %v", finishErr)
	}

	verifyFileContent(t, filepath.Join(dataDir, "testdata/remote.txt"), "from remote")
}

func testInstallFromRelease404(t *testing.T) {
	verifyHTTPErrorStatus(t, http.StatusNotFound)
}

func testInstallFromRelease500(t *testing.T) {
	verifyHTTPErrorStatus(t, http.StatusInternalServerError)
}

// verifyHTTPErrorStatus tests that a server returning the given status code is handled.
func verifyHTTPErrorStatus(t *testing.T, statusCode int) {
	t.Helper()
...
func TestCloseWithError(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "only close error",
			closeErr: errTestClose,
			wantErr:  errTestClose,
		},
		{
			name:        "only existing error",
			existingErr: errTestExisting,
			wantErr:     errTestExisting,
		},
		{
			name:        "both errors - existing takes priority",
			closeErr:    errTestClose,
			existingErr: errTestExisting,
			wantErr:     errTestExisting,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			closer := &mockCloser{err: tt.closeErr}
			got := closeWithError(closer, "test close", tt.existingErr)

			if tt.wantNil {
				if got != nil {
					t.Errorf("closeWithError() = %v, want nil", got)
				}
				return
			}

			if got == nil {
				t.Error("closeWithError() = nil, want error")
				return
			}

			// Check the expected error
			if !errors.Is(got, tt.wantErr) {
				t.Errorf("closeWithError() = %v, want %v", got, tt.wantErr)
			}
		})
	}
}
