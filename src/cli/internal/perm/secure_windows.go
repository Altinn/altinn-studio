...
func SecureFile(path string) error {
	...
	// Create explicit access entry granting owner full control
	...
	// Create ACL from entries (nil oldACL = new ACL with only these entries)
	acl, err := windows.ACLFromEntries(explicitAccess, nil)
	if err != nil {
		return fmt.Errorf("create ACL: %w", err)
	}

	// Apply DACL to file (PROTECTED prevents inheritance from parent)
