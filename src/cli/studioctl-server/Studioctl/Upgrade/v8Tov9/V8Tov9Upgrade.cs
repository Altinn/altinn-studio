...
internal static class V8Tov9Upgrade
{
    ...
    internal static async Task<int> RunAsync(V8Tov9UpgradeOptions options)
    {
        ...
        options.CancellationToken.ThrowIfCancellationRequested();
        returnCode = CombineExitCodes(returnCode, await MigrateEFormidlingClientNamespaces(projectFile));
    ...
    static async Task<int> MigrateEFormidlingClientNamespaces(string projectFile)
    {
        try
        {
            var migration = new UsingNamespaceMigration(projectFile);
        ...
        {
            await UpgradeConsole.WriteErrorAsync("Error migrating eFormidling client namespaces", ex);
            return ExitError;
        }
    }
