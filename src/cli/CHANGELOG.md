...
- Auto-migrate the `PlatformHttpException` changes in `studioctl app upgrade v9`: rename `PlatformHttpException.CreateAsync(...)` to `Create(...)`, and adapt constructor calls to the new response snapshot. Every rewrite is listed for review.
