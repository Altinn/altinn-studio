...
- `studioctl app upgrade v9` awaits `AddAltinnAppServices` in Program.cs, which loads the app's files before the host is built in v9, making the template's `ConfigureServices` function async and awaiting it in turn.
...
- `studioctl app upgrade v9` reports code that injects `IFeatureManager` or calls `AddFeatureManagement()`, which the v9 app libraries no longer register, reads of the removed `AppSettings.AppBasePath` and folder settings, and direct references to the service classes that are internal in v9, with what to use instead.
