...
- `studioctl app maskinporten set|show|remove` stores the Maskinporten client an app uses when it runs locally - for testing a real integration, such as a Fiks Arkiv shipment against the Fiks test environment, with a real client. studioctl provisions the stored client to the app the way Studio does when the app is deployed, so the app never reads Maskinporten credentials from its own configuration and there is no configuration section to get right. `set` reads the client as JSON from a file or standard input - in the provisioned `maskinporten-settings.json` format, as the bare credentials, or as a section written for the `Altinn.ApiClients.Maskinporten` package - and `--from-appsettings` lifts it straight out of an appsettings file from a v8 app (`--section` names the section when the file has several). The private key is stored readable by you only and is never printed; `show` prints the client id, the Maskinporten environment and the key id. A running app picks up a stored client without a restart.
...
- `studioctl app run` and `studioctl app env` hand the app the directory studioctl provisions its secrets into, which is how a stored Maskinporten client reaches it: as the `STUDIOCTL_APP_SECRETS_DIR` environment variable for a native run, as a read-only mount at the deployed location for a container run, and through `studioctl app env` for an app started from an IDE. The app libraries honor the variable on localtest only. When a client is stored, `app run` names it under the app's address.
