...
- The local environment now follows the environments it mirrors instead of image versions frozen into each studioctl release. `studioctl env up` starts the newest localtest build and the workflow engine build deployed to tt02, and checks both for a newer build every time it starts the environment, so a platform change reaches your machine without waiting for a studioctl release. An environment that is already running keeps the build it started with until you stop and start it again. When the registry cannot be reached — you are offline, or it is down — the environment starts on the images you already have instead of failing.
- `studioctl env status` and `studioctl doctor` show the image each container runs and the identifier of the exact build behind it. Quote that identifier when reporting a problem with the local environment: the image tags that follow tt02 point at a new build whenever one is deployed, so the tag alone no longer says which build you ran.
