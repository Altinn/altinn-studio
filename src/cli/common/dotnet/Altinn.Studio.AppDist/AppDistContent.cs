...
public interface IAppDistContent
{
    ...
    /// <exception cref="FileNotFoundException">The layer has no file at <paramref name="path"/>.</exception>
    Task<string> GetFileText(string path, CancellationToken cancellationToken = default);

    /// <summary>
    /// Reads all UTF-8 text files below <paramref name="pathPrefix"/> and returns paths relative to that prefix.
    /// </summary>
    Task<IReadOnlyDictionary<string, string>> GetFiles(
        string pathPrefix = "",
        CancellationToken cancellationToken = default
    );

    /// <summary>Lists every file path relative to the layer root.</summary>
    Task<IReadOnlyList<string>> ListFiles(CancellationToken cancellationToken = default);

    /// <summary>Copies every file to <paramref name="targetDirectory"/>, overwriting matching files.</summary>
    Task CopyToDirectory(string targetDirectory, CancellationToken cancellationToken = default);
}
...
internal abstract class AppDistContent(IAppDistStore store, string version) : IAppDistContent
{
    ...
    protected abstract Task<Stream?> OpenStoredFile(string path, CancellationToken ct);

    public abstract Task<IReadOnlyList<string>> ListFiles(CancellationToken cancellationToken = default);

    public async Task<Stream> OpenFile(string path, CancellationToken cancellationToken = default) =>
        await OpenStoredFile(path, cancellationToken)
        ?? throw new FileNotFoundException($"app-dist {Version} has no file \"{path}\"");

    public async Task<string> GetFileText(string path, CancellationToken cancellationToken = default)
    {
        await using var stream = await OpenFile(path, cancellationToken);
        using var reader = new StreamReader(stream, Encoding.UTF8);
        return await reader.ReadToEndAsync(cancellationToken);
    }

    public async Task<IReadOnlyDictionary<string, string>> GetFiles(
        string pathPrefix = "",
        CancellationToken cancellationToken = default
    )
    {
        ...
        foreach (var path in await ListFiles(cancellationToken))
        {
            if (!path.StartsWith(prefix, StringComparison.Ordinal))
                continue;
            files[path[prefix.Length..]] = await GetFileText(path, cancellationToken);
        }
        return files;
    }

    public async Task CopyToDirectory(string targetDirectory, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var path in await ListFiles(cancellationToken))
        {
            ...
            await using var source = await OpenFile(path, cancellationToken);
...
{
    protected override Task<Stream?> OpenStoredFile(string path, CancellationToken ct) =>
        Store.OpenFile(Version, layer, path, ct);

    public override Task<IReadOnlyList<string>> ListFiles(CancellationToken cancellationToken = default) =>
        Store.ListFiles(Version, layer, cancellationToken);
}
