...
namespace KubernetesWrapper.Controllers
{
    ...
    [ApiController]
    public class ApplicationLogsController(ILogger<ApplicationLogsController> logger, IConfiguration configuration) : ControllerBase
    {
        /// <summary>
        ...
        public ActionResult GetLogs(string app = null, double take = 50, double time = 24)
        {
            try
            {
                ...
                var query = $@"
                    ...
                    | serialize | extend RowNumber = row_number() | order by RowNumber asc
                    | project TimeGenerated, OuterMessage
                    | take {take}";

                var response = client.QueryWorkspace(applicationLawWorkspaceId, query, new QueryTimeRange(TimeSpan.FromHours(time)));
                ...
                return Ok(response.Value.AllTables.Select(table => table.Rows.Select(row => new
                {
                    ...
                    LogMessage = row["OuterMessage"]?.ToString()
                })));
            }
            catch (Exception e)
            {
