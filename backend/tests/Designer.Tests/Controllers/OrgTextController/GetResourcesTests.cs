...
public class GetResourcesTests
    : DesignerEndpointsTestsBase<GetResourcesTests>,
        IClassFixture<WebApplicationFactory<Program>>
{
    public GetResourcesTests(WebApplicationFactory<Program> factory)
        : base(factory) { }

    [Theory]
    [InlineData("ttd", "testUser", "org-content", "nb")]
    public async Task GetResources_Returns200OK_WithValidInput(
        string org,
        string developer,
        string repo,
        string lang
    )
    {
        ...
        Assert.True(
            JsonUtils.DeepEquals(expectedContent, await response.Content.ReadAsStringAsync())
        );
    }

    [Theory]
    [InlineData("ttd", "testUser", "org-content-empty", "sr")]
    public async Task GetResources_Returns204NoContent_WithNonExistingLang(
        string org,
        string developer,
        string repo,
        string languageCode
    ...
    private static string ApiUrl(string org, string languageCode) =>
        $"/designer/api/{org}/text/language/{languageCode}";
