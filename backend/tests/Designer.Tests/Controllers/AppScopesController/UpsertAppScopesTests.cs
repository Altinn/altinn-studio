        ...
        IClassFixture<WebApplicationFactory<Program>>
{
    ...
    private async Task CallUpsertEndpointAndAssertFromDb(
        string org,
        string app,
        AppScopesUpsertRequest payload
    )
    {
        ...
        var dbEntity = await DesignerDbFixture.DbContext.AppScopes.SingleAsync(x =>
            x.App == app && x.Org == org
        );

        Assert.NotNull(dbEntity);

        var scopes = JsonSerializer.Deserialize<ISet<MaskinPortenScopeEntity>>(
            dbEntity.Scopes,
        ...
        foreach (MaskinPortenScopeEntity maskinPortenScopeEntity in scopes)
        {
            Assert.Contains(
                payload.Scopes,
                x =>
                    x.Scope == maskinPortenScopeEntity.Scope
                    && x.Description == maskinPortenScopeEntity.Description
            );
        }
    }
