        ...
        IClassFixture<MockServerFixture>
{
    ...
    {
        // Arrange
        _mockServerFixture.PrepareQueueBuildResponse(
            DecommissionDefinitionId,
            azureDevopsMockQueueBuildResponse
        );

        // Ensure that the deployment entity exists in the database
        var entity = EntityGenerationUtils.Deployment.GenerateDeploymentEntity(
            org,
            app,
            envName: environment
        );
