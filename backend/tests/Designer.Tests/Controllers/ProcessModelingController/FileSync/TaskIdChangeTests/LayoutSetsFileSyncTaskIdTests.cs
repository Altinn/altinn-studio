...
public class LayoutSetsFileSyncTaskIdTests
    : DesignerEndpointsTestsBase<LayoutSetsFileSyncTaskIdTests>,
        IClassFixture<WebApplicationFactory<Program>>
{
    private static string VersionPrefix(string org, string repository) =>
        $"/designer/api/{org}/{repository}/process-modelling/process-definition";

    public LayoutSetsFileSyncTaskIdTests(WebApplicationFactory<Program> factory)
        : base(factory) { }
    ...
    )
    {
        ...
        Assert.DoesNotContain(
            layoutSets.Sets,
            layout => layout.Tasks.Contains(metadata.TaskIdChange.OldId)
        );
        Assert.Contains(
            layoutSets.Sets,
            layout => layout.Tasks.Contains(metadata.TaskIdChange.NewId)
        );
    }
