...
namespace Designer.Tests.Controllers.AppDevelopmentController
{
    public class GetFormLayoutsTests
        : DesignerEndpointsTestsBase<GetFormLayoutsTests>,
            IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        public async Task GetAppDevelopment_ShouldReturnLayouts(
            string org,
            string app,
            string developer,
            string layoutSetName,
            params string[] expectedLayoutPaths
        )
        {
            string targetRepository = TestDataHelper.GenerateTestRepoName();
            await CopyRepositoryForTest(org, app, developer, targetRepository);

            Dictionary<string, string> expectedLayouts = await AddLayoutsToRepo(
                TestRepoPath,
                layoutSetName,
                expectedLayoutPaths
            );
            ...
            string url =
                $"{VersionPrefix(org, targetRepository)}/form-layouts?layoutSetName={layoutSetName}";
            ...
            {
                string actualLayout = responseJson[
                    Path.GetFileNameWithoutExtension(expectedLayoutName)
                ]
                    .ToJsonString();
        ...
        private static async Task<Dictionary<string, string>> AddLayoutsToRepo(
            string repoPath,
            string layoutSetName,
        ...
        )
        {
            ...
            foreach (string layoutPath in expectedLayoutPaths)
            {
                ...
                string layoutName =
                    $"{Guid.NewGuid()}{Path.GetFileNameWithoutExtension(layoutPath)}";
                string layoutFolder = string.IsNullOrEmpty(layoutSetName)
                    ? Path.Combine(repoPath, "App", "ui", "layouts")
                    : Path.Combine(repoPath, "App", "ui", layoutSetName, "layouts");
