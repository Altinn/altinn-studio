...
public class PutDatamodelTests
    : DesignerEndpointsTestsBase<PutDatamodelTests>,
        IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public PutDatamodelTests(WebApplicationFactory<Program> factory)
        : base(factory)
    {
    ...
    public async Task ValidInput_ShouldReturn_NoContent_And_Create_Files(
        string modelPath,
        string org,
        string repo,
        string user
    ...
    [Theory]
    [InlineData("testModel.schema.json", "ttd", "hvem-er-hvem", "testUser")]
    public async Task InvalidInput_ShouldReturn_BadRequest_And_CustomErrorMessages(
        string modelPath,
        string org,
        string repo,
        string user
    )
    {
        ...
        var firstErrorMessage = customErrorMessagesElement
            .EnumerateArray()
            .FirstOrDefault()
            .GetString();
        Assert.Contains(
            "'root': member names cannot be the same as their enclosing type",
            firstErrorMessage
        );
    }
    ...
    [Theory]
    [InlineData(
        "testModel.schema.json",
        "ttd",
        "hvem-er-hvem",
        "testUser",
        "Model/JsonSchema/General/NonXsdContextSchema.json"
    )]
    public async Task ValidSchema_ShouldReturn_NoContent_And_Create_Files(
        string modelPath,
        string org,
        string repo,
        string user,
    ...
    public async Task IncompatibleSchema_ShouldReturn422(
        string modelPath,
        string schemaPath,
        string org,
        string repo,
    ...
    )
    {
        ...
        foreach ((string pointer, string errorCode) in expectedValidationIssues)
        {
            ...
            Assert.Contains(
                errorCode,
                errorResponse.Errors[pointerObject.ToString(JsonPointerStyle.UriEncoded)]
            );
        }
    }
    ...
    public async Task PutDatamodelDataType_ShouldReturnWithoutErrors(
    ...
    {
        string url =
            $"{VersionPrefix(org, TargetTestRepository)}/datamodel/{datamodelName}/dataType";
    ...
    [Theory]
    [InlineData("testmodelname", "ttd", "hvem-er-hvem")]
    public async Task PutDatamodelDataType_FailsIfDatamodelNameMismatchesObjectId(
        string datamodelName,
        string org,
        string repo
    )
    ...
    public static IEnumerable<object[]> IncompatibleSchemasTestData =>
        new List<object[]>
        {
            new object[]
            {
                "testModel.schema.json",
                "Model/JsonSchema/Incompatible/OneOfAndPropertiesSchema.json",
                "ttd",
                "hvem-er-hvem",
                "testUser",
            ...
            {
                "testModel.schema.json",
                "Model/JsonSchema/Incompatible/InvalidReferenceSchema.json",
                "ttd",
                "hvem-er-hvem",
                "testUser",
                new Tuple<string, string>(
