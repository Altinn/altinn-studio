...
public class TextsServiceTest : IDisposable
{
    ...
    {
        (var targetRepository, var altinnGitRepositoryFactory, var textsService) =
        ...
        AltinnAppGitRepository altinnAppGitRepository =
            altinnGitRepositoryFactory.GetAltinnAppGitRepository(Org, targetRepository, Developer);
        JsonNode formLayout = await altinnAppGitRepository.GetLayout(LayoutSetName1, LayoutName1);

        Assert.NotNull(formLayout);
        Assert.Equal(
            "new-id",
            (formLayout["data"]["layout"] as JsonArray)[0]
    ...
    public async Task UpdateRelatedFiles_MultipleKeysExistInLayoutsAcrossLayoutSets_ShouldFindNewIdInMutlipleFiles()
    {
        ...
        Assert.Equal(
            "new-id",
            (formLayout1["data"]["layout"] as JsonArray)[0]
                ["textResourceBindings"]["title"]
                .ToString()
        );
        Assert.Equal(
            "new-id",
            (formLayout2["data"]["layout"] as JsonArray)[0]
                ["textResourceBindings"]["title"]
                .ToString()
        );
    }
    ...
    public async Task UpdateRelatedFiles_Options_ShouldUpdateSourceLabel()
    {
        ...
        Assert.Equal(
            "source-label-new",
            (formLayout["data"]["layout"] as JsonArray)[3]["source"]["label"].ToString()
        );
    }
