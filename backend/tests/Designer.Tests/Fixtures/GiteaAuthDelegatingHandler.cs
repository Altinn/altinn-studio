...
namespace Designer.Tests.Fixtures
{
    ...
    internal class GiteaAuthDelegatingHandler : DelegatingHandler
    {
        ...
        )
        {
            ...
            using var redirectToAuthorizeResponse = await base.SendAsync(
                redirectToAuthorizeRequest,
                cancellationToken
            );

            using var authorizeRedirectedToLoginResponse = await Redirect(
                redirectToAuthorizeResponse.Headers.Location,
                cancellationToken
            );

            using HttpResponseMessage loginToGiteaResponse = await LoginToGitea(
                authorizeRedirectedToLoginResponse,
                redirectToAuthorizeResponse,
                cancellationToken
            );

            using HttpResponseMessage loginToAuthorizeRedirectedResponse = await Redirect(
                loginToGiteaResponse.Headers.Location,
                cancellationToken,
                loginToGiteaResponse.GetGiteaAuthCookies()
            );
            ...
            {
                using HttpResponseMessage grantResponse = await GrantAuthorization(
                    loginToAuthorizeRedirectedResponse,
                    loginToGiteaResponse,
                    cancellationToken
                );
                designerSignInUrl = grantResponse.Headers.Location;
            }

            using HttpResponseMessage designerSignInResponse = await Redirect(
                designerSignInUrl,
                cancellationToken,
                initialResponse.GetCookies(".AspNetCore.")
            );
            ...
            return await RetryInitialRequestAfterSigningIn(
                request,
                designerAuthCookies,
                xsrfToken,
                cancellationToken
        ...
        {
            using var finalRedirectRequest = new HttpRequestMessage(
                request.Method,
                request.RequestUri
            );
        ...
        )
        {
            ...
            using var xsrfResponse = await base.SendAsync(
        ...
        private async Task<HttpResponseMessage> GrantAuthorization(
            ...
            HttpResponseMessage loginToGiteaResponse,
            CancellationToken cancellationToken
        )
        {
            string authorizePageContent =
                await loginToAuthorizeRedirectedResponse.Content.ReadAsStringAsync(
                    cancellationToken
                );
            ...
            List<KeyValuePair<string, string>> grantFormValues = new()
            {
                new KeyValuePair<string, string>(
                    "_csrf",
                    WebScrapingUtils.ExtractTextBetweenMarkers(
            ...
            string grantUrl = WebScrapingUtils.ExtractTextBetweenMarkers(
                authorizePageContent,
        ...
        private async Task<HttpResponseMessage> LoginToGitea(
            HttpResponseMessage loginRedirectResponse,
            HttpResponseMessage redirectResponse,
            CancellationToken cancellationToken
        )
        {
            string loginPageContent = await loginRedirectResponse.Content.ReadAsStringAsync(
                cancellationToken
            );
            ...
            using var giteaPostLoginMessage = new HttpRequestMessage(
                HttpMethod.Post,
                loginRedirectResponse.RequestMessage.RequestUri
            )
            {
                Content = content,
            };

            giteaPostLoginMessage.AddCookies(
        ...
        private async Task<HttpResponseMessage> Redirect(
            Uri redirectUri,
            CancellationToken cancellationToken,
