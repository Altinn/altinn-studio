...
{
    public GetSucceededTests(DesignerDbFixture dbFixture)
        : base(dbFixture) { }

    [Theory]
    [MemberData(nameof(TopAndSortTestData))]
    public async Task GetSucceeded_ShouldReturnCorrectRecordsFromDatabase(
        string org,
        string app,
        string envName,
        int top,
    ...
    )
    {
        ...
        var deploymentEntities = EntityGenerationUtils
            .Deployment.GenerateDeploymentEntities(org, app, DeployedInEnvCount, envName: envName)
            .ToList();
        await DbFixture.PrepareEntitiesInDatabase(deploymentEntities);

        var deploymentEntitiesAnotherEnvs = EntityGenerationUtils
    ...
    public async Task GetSucceeded_Without_TopDefined_ShouldReturnAllRecordsForGivenApp(
        string org,
        string app,
        string envName,
        SortDirection sortDirection
