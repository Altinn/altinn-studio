...
{
    public UpsertAppScopesAsyncIntegrationTests(DesignerDbFixture dbFixture)
        : base(dbFixture) { }

    [Theory]
    [MemberData(nameof(TestData))]
    public async Task UpsertAppScopesAsync_ShouldCreateAppScopes_IfNotExists(
        string org,
        string app,
        int numberOfScopes
    )
    {
        var entity = EntityGenerationUtils.AppScopes.GenerateAppScopesEntity(
            org,
            app,
            numberOfScopes
        );
        var repository =
            new Altinn.Studio.Designer.Repository.ORMImplementation.AppScopesRepository(
                DbFixture.DbContext
            );
        ...
        var dbRecord = await DbFixture
            ...
            .FirstOrDefaultAsync(d => d.Org == org && d.App == app);
    ...
    [Theory]
    [MemberData(nameof(TestData))]
    public async Task UpsertAppScopesAsync_ShouldUpdateAppScopes_IfAlreadyExists(
        string org,
        string app,
        int numberOfScopes
    )
    {
        ...
        var updatedDbRecord = await DbFixture
            .DbContext.AppScopes.AsNoTracking()
            .FirstOrDefaultAsync(d => d.Org == org && d.App == app);
