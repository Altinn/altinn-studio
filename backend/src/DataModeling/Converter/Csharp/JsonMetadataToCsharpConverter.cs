...
namespace Altinn.Studio.DataModeling.Converter.Csharp
{
    ...
    public class JsonMetadataToCsharpConverter : IModelMetadataToCsharpConverter
    {
        ...
        private string Indent(int level = 1) =>
            new string(' ', level * _generationSettings.IndentSize);
        ...
        )
        {
            ...
            CreateModelFromMetadataRecursive(
                ...
                rootElementType,
                serviceMetadata,
                serviceMetadata.TargetNamespace,
        ...
        private void CreateModelFromMetadataRecursive(
            Dictionary<string, string> classes,
            ElementMetadata parentElement,
            ModelMetadata serviceMetadata,
        ...
        )
        {
            ...
            if (parentElement.ParentElement == null && string.IsNullOrWhiteSpace(targetNamespace))
            {
                classBuilder.AppendLine(
                    Indent() + "[XmlRoot(ElementName=\"" + parentElement.Name + "\")]"
                );
            }
            else if (
            ...
            foreach (
                ElementMetadata element in serviceMetadata
                    .Elements.Select(e => e.Value)
                    .Where(ele => ele.ParentElement == parentElement.ID)
            )
            {
        ...
        private void ParseFieldProperty(
        ...
        )
        {
            ...
            if (_generationSettings.XmlTextValueNullableHack && element.IsTagContent && isValueType)
            {
                ...
                classBuilder.AppendLine(
                    $"{Indent(2)}public {dataType}? valueNullable {{ get; set; }}"
                );
                classBuilder.AppendLine();
            ...
            else if (element.IsTagContent)
            {
                ...
                classBuilder.AppendLine(
                    $"{Indent(2)}public {dataType}{nullableReference} value {{ get; set; }}"
                );
                classBuilder.AppendLine();
            }
            else
            {
                ...
                classBuilder.AppendLine(
                    Indent(2) + "[JsonPropertyName(\"" + element.XName + "\")]"
                );

                if (element.MaxOccurs > 1)
                {
                    classBuilder.AppendLine(
                        $"{Indent(2)}public List<{dataType}>{nullableReference} {element.Name} {{ get; set; }}\n"
                    );
                }
                else
                {
                    ...
                    {
                        classBuilder.AppendLine(
                            $"{Indent(2)}public {dataType}? {element.Name} {{ get; set; }}\n"
                        );

                        if (
                            element.Nillable.HasValue
                    ...
                    {
                        classBuilder.AppendLine(
                            $"{Indent(2)}public {dataType}{nullableReference} {element.Name} {{ get; set; }}\n"
                        );
                    }
                }
            }
        ...
        private void ParseGroupProperty(
            ElementMetadata element,
            StringBuilder classBuilder,
            ModelMetadata modelMetadata,
            List<ElementMetadata> referredTypes,
            ref int elementOrder,
            bool useNullableReferenceTypes
        )
        {
        ...
        private void AddXmlElementAnnotation(
            ElementMetadata element,
            StringBuilder classBuilder,
            int elementOrder,
            bool addNillableAttribute = false
        )
        {
            ...
            classBuilder.AppendLine(
                $"""{Indent(2)}[XmlElement("{element.XName}"{additionalAttributeParams})]"""
            );
        }

        private void AddShouldSerializeForTagContent(
            ElementMetadata element,
            StringBuilder classBuilder,
            ModelMetadata modelMetadata
        )
        {
            ...
            if (
                children.Count(metadata => metadata.FixedValue != null) == 1
                && children.Count(metadata => metadata.IsTagContent) == 1
            )
            {
                var taggedContentChild = children.Single(metadata => metadata.IsTagContent);
                var value =
                    _generationSettings.XmlTextValueNullableHack
                    && taggedContentChild.XsdValueType.HasValue
                    && GetPropertyType(taggedContentChild.XsdValueType).IsValueType
                        ? "valueNullable is not null"
                        : "value is not null";

                classBuilder.AppendLine(
                    $"{Indent(2)}public bool ShouldSerialize{element.Name}() => {element.Name}?.{value};"
                );
                classBuilder.AppendLine();
            }
        }

        private void ParseAttributeProperty(
            ElementMetadata element,
            StringBuilder classBuilder,
            bool required,
            bool useNullableReferenceTypes
        ...
        )
        {
            ...
            {
                errorMessage =
                    ", ErrorMessage = \"" + element.Texts[TextCategoryType.Error.ToString()] + "\"";
            }
        ...
        )
        {
            ...
            {
                classBuilder.AppendLine(
                    Indent(2) + "[MinLength(" + minLengthRestriction.Value + errorMessage + ")]"
                );
            }
            ...
            {
                classBuilder.AppendLine(
                    Indent(2) + "[MaxLength(" + maxLengthRestriction.Value + errorMessage + ")]"
                );
            }
        ...
        private void WriteRangeRestriction(
            ...
            ElementMetadata element,
            string errorMessage,
            string leftRestrictionName,
            string rightRestrictionName,
            out bool hasRange
        )
        {
            hasRange = false;
            bool hasMinimum = element.Restrictions.TryGetValue(
                leftRestrictionName,
                out var minRestriction
            );
            bool hasMaximum = element.Restrictions.TryGetValue(
                rightRestrictionName,
                out var maxRestriction
            );

            if (hasMinimum && hasMaximum)
            {
                classBuilder.AppendLine(
                    $"{Indent(2)}[Range({GetRangeValueAsString(element, minRestriction)}, {GetRangeValueAsString(element, maxRestriction)}{errorMessage})]"
                );
                hasRange = true;
            }
            else if (hasMinimum)
            {
                classBuilder.AppendLine(
                    $"{Indent(2)}[Range({GetRangeValueAsString(element, minRestriction)}, {RightRangeLimit(element.XsdValueType ?? BaseValueType.Double)}{errorMessage})]"
                );
                hasRange = true;
            }
            else if (hasMaximum)
            {
                classBuilder.AppendLine(
                    $"{Indent(2)}[Range({LeftRangeLimit(element.XsdValueType ?? BaseValueType.Double)}, {GetRangeValueAsString(element, maxRestriction)}{errorMessage})]"
                );
                hasRange = true;
            }
        }

        private static string GetRangeValueAsString(
            ElementMetadata element,
        ...
        )
        {
            ...
            if (
                !isAlreadyDecimal
                && element.XsdValueType.HasValue
                && !new[] { BaseValueType.Int, BaseValueType.Long }.Contains(
                    element.XsdValueType.Value
                )
            )
            {
                value = $"{value}d";
        ...
        private void WriteTypeRestrictions(
            StringBuilder classBuilder,
            BaseValueType type,
            string errorMessage
        )
        {
            switch (type)
            {
                ...
                case BaseValueType.PositiveInteger:
                    classBuilder.AppendLine(
                        Indent(2)
                            + $"[Range({LeftRangeLimit(type)},{RightRangeLimit(type)}"
                            + errorMessage
                            + ")]"
                    );
                    break;
                case BaseValueType.GYear:
                    classBuilder.AppendLine(
                        Indent(2) + "[RegularExpression(@\"^[0-9]{4}$\"" + errorMessage + ")]"
                    );
                    break;
        ...
        private static string LeftRangeLimit(BaseValueType type) =>
            type switch
            {
        ...
        private static string RightRangeLimit(BaseValueType? type) =>
            type switch
            {
        ...
        {
            classBuilder.AppendLine(
                Indent(2) + $"public bool ShouldSerialize{propName}() => {propName}.HasValue;"
            );
            classBuilder.AppendLine();
        }

        private bool ShouldWriteAltinnRowId(
            ElementMetadata element,
            List<ElementMetadata> allElements
        ) => allElements.Any(e => e.TypeName == element.TypeName && e.MaxOccurs > 1);
        ...
        private void WriteAltinnRowId(StringBuilder classBuilder)
        {
            ...
            classBuilder.AppendLine("");
            classBuilder.AppendLine(
                Indent(2) + "public bool ShouldSerializeAltinnRowId() => AltinnRowId != default;"
            );
            classBuilder.AppendLine();
        }
    }
