...
namespace Altinn.Studio.Designer.Configuration.Extensions
{
    public static class ServiceCollectionExtensions
    {
        ...
        public static IServiceCollection RegisterSettingsByBaseType<TMarker>(
            this IServiceCollection services,
            IConfiguration configuration
        )
        {
            var typesToRegister = AltinnAssembliesScanner
                .GetTypesAssignedFrom<TMarker>()
        ...
        )
        {
            Func<
                ...
                string,
                IServiceCollection
            > registerSettingsMethodObject = RegisterSettings<object>;
            var genericMethodTemplate = registerSettingsMethodObject
        ...
        public static IServiceCollection RegisterSettings<TOption>(
            this IServiceCollection services,
            IConfiguration configuration,
            string section = null
        )
        ...
        {
            string sectionName = string.IsNullOrWhiteSpace(section)
                ? typeof(TOption).Name
                : section;
        ...
        {
            IEnumerable<IConfigurationSection> resourceRegistrySettingSections =
                section.GetChildren();

            foreach (IConfigurationSection settingSection in resourceRegistrySettingSections)
            {
                ResourceRegistryEnvironmentSettings rris =
                    new ResourceRegistryEnvironmentSettings();
                settingSection.Bind(rris);
                rris.Environment = settingSection.Key;
                services.Configure<ResourceRegistryIntegrationSettings>(x =>
                    x.Add(rris.Environment.ToLower(), rris)
                );
            }
        ...
        {
            IEnumerable<IConfigurationSection> maskinportenIntegrationSettingSections =
                section.GetChildren();
            ...
            foreach (IConfigurationSection settingSection in maskinportenIntegrationSettingSections)
            {
                ...
                services.Configure<ResourceRegistryMaskinportenIntegrationSettings>(x =>
                    x.Add(integrationSettings.Environment.ToLower(), integrationSettings)
                );
            }
        ...
        private static void ConfigureSettingsTypeBySection<TOption>(
            this IServiceCollection services,
            IConfiguration configuration,
            string sectionName
        )
            where TOption : class, new()
        {
            ...
            services.TryAddScoped(
                ...
                svc =>
                    (
                        (IOptionsSnapshot<object>)svc.GetService(typeof(IOptionsSnapshot<TOption>))
                    )!.Value
            );
        }

        public static IServiceCollection RegisterSettingsSingleton<TOption>(
            this IServiceCollection services,
            IConfiguration configuration,
            string section = null
        )
        ...
        private static void ConfigureSettingsTypeBySectionSingleton<TOption>(
            this IServiceCollection services,
            IConfiguration configuration,
            string sectionName
        )
        ...
        /// <typeparam name="TMarker">The marker type used to identify the services or settings to be registered.</typeparam>
        /// <returns>A reference to this instance after the operation has completed.</returns>
        public static IServiceCollection RegisterSingletonServicesByBaseType<TMarker>(
            this IServiceCollection services
        )
        {
            var typesToRegister = AltinnAssembliesScanner
