...
public class InstanceService(IDistributedCache distributedCache, IDataService dataService)
    : IInstanceService
{
        ...
        dataTypes.ForEach(dataType =>
        {
            if (dataType.AppLogic?.ClassRef is not null)
            {
                ...
                if (dataType.MaxCount != 1)
                {
                    ...
                    instance.Data.Add(
                        dataService.CreateDataElement(partyId, instanceGuid, dataType.Id)
                    );
                }
            }
        });
    ...
    public Instance AddDataElement(Guid instanceGuid, DataElement dataElement)
    {
        ...
        distributedCache.SetString(
            ...
            JsonSerializer.Serialize(instanceElement),
            _cacheOptions
        );
        return instanceElement;
    }
