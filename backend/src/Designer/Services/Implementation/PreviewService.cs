...
public class PreviewService(IAltinnGitRepositoryFactory altinnGitRepositoryFactory)
    : IPreviewService
{
    ...
    )
    {
        ...
        string task = await GetTaskForLayoutSetName(
            org,
            app,
            developer,
            layoutSetName,
            ...
            InstanceOwner = new InstanceOwner
            {
                PartyId =
                    instanceOwnerPartyId == null
                        ? "undefined"
                        : instanceOwnerPartyId.Value.ToString(),
            },
            Id = $"{instanceOwnerPartyId}/{instanceGuid}",
            AppId = applicationMetadata.Id,
            Data = await GetDataTypesForInstance(
                org,
                app,
                developer,
                layoutSetName,
                shouldProcessActAsReceipt
            ),
    ...
    )
    {
        ...
        try
        {
            ...
            {
                foreach (
                    LayoutSetConfig layoutSet in layoutSets.Sets.Where(ls =>
                        !tasks.Contains(ls.Tasks?[0])
                    )
                )
                {
    ...
    private async Task<string> GetDataTypeForCustomReceipt(
        AltinnAppGitRepository altinnAppGitRepository
    )
    {
        LayoutSets layoutSets = await altinnAppGitRepository.GetLayoutSetsFile();
        string dataType = layoutSets
            ?.Sets?.Find(set => set.Tasks[0] == Constants.General.CustomReceiptId)
            ?.DataType;
