...
namespace Altinn.Studio.Designer.Services.Implementation.ProcessModeling
{
    public class ProcessModelingService : IProcessModelingService
    {
        ...
        {
            using (
                Stream processDefinitionStream = GetProcessDefinitionStream(
            ...
            )
            {
                ...
                Definitions? definitions = (Definitions?)
                    serializer.Deserialize(processDefinitionStream);
                LayoutSetConfig layoutSet = await _appDevelopmentService.GetLayoutSetConfig(
                    altinnRepoEditingContext,
                    layoutSetId
                );
                string? taskId = layoutSet.Tasks?.First();
                ProcessTask? task = definitions?.Process.Tasks.FirstOrDefault(task =>
                    task.Id == taskId
                );
        ...
        private Stream GetTemplateStream(SemanticVersion version, string templateName)
        {
            ...
            Stream? templateStream =
                typeof(ProcessModelingService).Assembly.GetManifestResourceStream(template);
            if (templateStream == null)
            {
                throw new FileNotFoundException(
                    $"Template resource '{template}' not found in the assembly."
                );
            }
