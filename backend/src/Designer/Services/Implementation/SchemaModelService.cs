...
namespace Altinn.Studio.Designer.Services.Implementation
{
    ...
    public class SchemaModelService : ISchemaModelService
    {
        ...
        )
        {
            ...
            return await altinnAppGitRepository.ReadTextByRelativePathAsync(
                relativeFilePath,
                cancellationToken
            );
        }
        ...
        )
        {
            ...
            altinnAppGitRepository.DeleteModelMetadata(
                relativeFilePath.Replace(".schema.json", ".metadata.json")
            );
            ...
            ModelMetadata modelMetadata = GetModelMetadataForCsharpGeneration(
                serializedJsonContent,
                jsonSchema
            );
        ...
        private async Task UpdateXsdFromJsonSchema(
        ...
        private ModelMetadata GetModelMetadataForCsharpGeneration(
            string jsonContent,
            JsonSchema jsonSchema
        )
        {
            ...
            var metamodelConverter = new JsonSchemaToMetamodelConverter(
                jsonSchemaConverterStrategy.GetAnalyzer()
            );
            return metamodelConverter.Convert(jsonContent);
        }

        public async Task<ModelMetadata> GenerateModelMetadataFromJsonSchema(
            AltinnRepoEditingContext altinnRepoEditingContext,
            string relativeFilePath,
            CancellationToken cancellationToken = default
        )
        ...
        )
        {
            ...
            AltinnRepositoryType altinnRepositoryType =
                await altinnAppGitRepository.GetRepositoryType();
            if (altinnRepositoryType == AltinnRepositoryType.Datamodels)
            {
                await altinnAppGitRepository.WriteTextByRelativePathAsync(
                    Path.ChangeExtension(fileNameWithExtension, "schema.json"),
                    serializedJsonContent,
                    true,
                    cancellationToken
                );
        ...
        /// <inheritdoc/>
        public async Task<(string RelativePath, string JsonSchema)> CreateSchemaFromTemplate(
            AltinnRepoEditingContext altinnRepoEditingContext,
        ...
        )
        {
            ...
            {
                var uri = GetSchemaUri(
                    altinnRepoEditingContext.Org,
                    altinnRepoEditingContext.Repo,
                    schemaAndModelName,
                    relativeDirectory
                );
                JsonTemplate jsonTemplate = altinn2Compatible
                    ? new SeresJsonTemplate(uri, schemaAndModelName)
                    : new GeneralJsonTemplate(uri, schemaAndModelName);

                var jsonSchema = jsonTemplate.GetJsonString();

                var relativeFilePath = Path.ChangeExtension(
                    Path.Combine(relativeDirectory, schemaAndModelName),
                    ".schema.json"
                );
                await altinnGitRepository.WriteTextByRelativePathAsync(
                    relativeFilePath,
                    jsonSchema,
                    true,
                    cancellationToken
            ...
            else
            {
                ...
                var relativePath = await altinnAppGitRepository.SaveJsonSchema(
                    jsonSchema,
                    schemaAndModelName
                );
        ...
        )
        {
            ...
            if (await altinnGitRepository.GetRepositoryType() == AltinnRepositoryType.App)
            {
                ...
                var altinnCoreFile = altinnAppGitRepository.GetAltinnCoreFileByRelativePath(
                    relativeFilePath
                );
                var schemaFileName = altinnAppGitRepository.GetSchemaName(relativeFilePath);
                ...
                DeleteRelatedSchemaFiles(
                    altinnAppGitRepository,
                    schemaFileName,
                    altinnCoreFile.Directory
                );
            }
            else
            {
        ...
        )
        {
            ...
            else
            {
                ...
                schemaUri = new Uri(
                    $"{baseUrl}/{org}/{repository}/{relativePath}/{schemaFileName}.schema.json"
                );
            }
        ...
        private async Task UpdateCSharpClasses(
            AltinnAppGitRepository altinnAppGitRepository,
        ...
        private static void DeleteRelatedSchemaFiles(
            AltinnAppGitRepository altinnAppGitRepository,
            string schemaFileName,
            string directory
        )
        ...
        private static IEnumerable<string> GetRelatedSchemaFiles(
            string schemaFileName,
            string directory
        )
        {
        ...
        )
        {
            ...
            if (applicationMetadata.DataTypes != null)
            {
                ...
                if (altinnAppGitRepository.AppUsesLayoutSets())
                {
                    ...
                    List<LayoutSetConfig> layoutSetsWithDataTypeToDelete = layoutSets.Sets.FindAll(
                        set => set.DataType == id
                    );
        ...
        {
            bool separateNamespace = NamespaceNeedsToBeSeparated(application, csharpModelName);
            return separateNamespace
                ? $"Altinn.App.Models.{csharpModelName}.{csharpModelName}"
                : $"Altinn.App.Models.{csharpModelName}";
        }

        private bool NamespaceNeedsToBeSeparated(
            ApplicationMetadata application,
            string csharpModelName
        )
        {
            return application.DataTypes.All(d =>
                d.AppLogic?.ClassRef != $"Altinn.App.Models.{csharpModelName}"
            );
        }
        ...
        public async Task<DataType> GetModelDataType(string org, string app, string modelId)
        {
            ...
            DataType dataType = applicationMetadata.DataTypes.Find(
                (dataType) => dataType.Id == modelId
            );
            return dataType;
        }

        public async Task SetModelDataType(
            string org,
            string app,
            string modelId,
            DataType dataType
        )
        {
            ...
            await _applicationMetadataService.UpdateApplicationMetaDataLocally(
                org,
                app,
                applicationMetadata
            );
