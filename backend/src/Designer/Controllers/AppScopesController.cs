...
[Route(
    "designer/api/{org}/{app:regex(^(?!datamodels$)[[a-z]][[a-z0-9-]]{{1,28}}[[a-z0-9]]$)}/app-scopes"
)]
...
) : ControllerBase
{
    ...
    )
    {
        ...
        {
            Scopes = scopes
                .Select(x => new MaskinPortenScopeDto()
                {
                    Scope = x.Scope,
    ...
    )
    {
        var scopes = appScopesUpsertRequest
            .Scopes.Select(x => new MaskinPortenScopeEntity()
            {
                Scope = x.Scope,
                Description = x.Description,
            })
    ...
    [Authorize]
    [HttpGet]
    public async Task<IActionResult> GetAppScopes(
        string org,
        string app,
        CancellationToken cancellationToken
    )
    {
        ...
        var reponse = new AppScopesResponse()
        {
            Scopes =
                appScopes
                    ?.Scopes.Select(x => new MaskinPortenScopeDto()
                    {
                        Scope = x.Scope,
                        Description = x.Description,
                    })
