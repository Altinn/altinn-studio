...
namespace Altinn.Studio.Designer.Controllers
{
    ...
    [Route(
        "designer/api/{org}/{app:regex(^(?!datamodels$)[[a-z]][[a-z0-9-]]{{1,28}}[[a-z0-9]]$)}/text"
    )]
    [Obsolete(
    ...
    public class TextController : Controller
    {
        ...
        public async Task<ActionResult<TextResource>> GetResource(
            string org,
            string app,
            string languageCode
        )
        {
            try
            {
                ...
                TextResource textResource = await _textsService.GetText(
                    org,
                    app,
                    developer,
                    languageCode
            ...
            {
                return NotFound(
                    $"Text resource, resource.{languageCode}.json, could not be found."
                );
            }
        }
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    TextResource textResourceObject = await _textsService.GetText(
                        org,
                        app,
                        developer,
                        languageCode
                    ...
                    foreach (TextIdMutation m in mutations)
                    {
                        ...
                        int originalEntryIndex = textResourceObject.Resources.FindIndex(
                            textResourceElement => textResourceElement.Id == m.OldId
                        );
                        ...
                        TextResourceElement textEntry = textResourceObject.Resources[
                            originalEntryIndex
                        ];
