...
namespace Altinn.Studio.Designer.TypedHttpClients.AzureDevOps
{
    ...
    public class AzureDevOpsBuildClient : IAzureDevOpsBuildClient
    {
        ...
        public async Task<Build> QueueAsync<T>(T buildParameters, int buildDefinitionId)
            where T : class
        {
            QueueBuildRequest queueBuildRequest = CreateBuildRequest(
                buildParameters,
                buildDefinitionId
            );
        ...
        private static QueueBuildRequest CreateBuildRequest<T>(
            T buildParameters,
