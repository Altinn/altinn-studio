...
} from './utils/generateTimeOptions/generateTimeOptions';
import { handleSegmentValueChange } from './utils/handleSegmentValueChange/handleSegmentValueChange';
...
export const TimePicker: React.FC<TimePickerProps> = ({
  ...
  const navigateUpDown = (direction: 'up' | 'down') => {
    ...
    const newFocus = calculateNextFocusState(
      dropdownFocus,
      action,
      getMaxColumns(),
      getOptionCounts(),
  ...
  const handleDropdownKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    switch (action.type) {
      ...
      case 'ESCAPE':
        setDropdownFocus(
          calculateNextFocusState(dropdownFocus, action, getMaxColumns(), getOptionCounts()),
        );
        closeDropdown();
        break;
    }
    ...
    >
      <div className={styles.segmentContainer}>
        {segments.map((segmentType, index) => {
          const segmentValue =
            segmentType === 'period' ? timeValue.period || 'AM' : timeValue[segmentType];
          const segmentConstraints =
            segmentType !== 'period'
              ? getSegmentConstraints(
                  segmentType as 'hours' | 'minutes' | 'seconds',
                  timeValue,
                  constraints,
                  format,
                )
      ...
      <Popover.TriggerContext>
        ...
        <Popover
          ...
          onOpen={() => {
            ...
            const currentHourIndex = hourOptions.findIndex(
              (option) => option.value === displayHours,
            );
            const initialFocus = {
        ...
        >
          <div className={styles.dropdownColumns}>
            {/* Hours Column */}
            <div className={styles.dropdownColumn} role='group' aria-label='Hours selection'>
              <div className={styles.dropdownLabel}>{segmentLabels.hours}</div>
              <div
                className={`${styles.dropdownList} ${
                  dropdownFocus.isActive && dropdownFocus.column === 0
              ...
              >
                {hourOptions.map((option, optionIndex) => {
                  const normalizedHour = normalizeHour(
                    option.value,
                    is12Hour,
                    timeValue.period || 'AM',
                  );
            ...
            {/* Minutes Column */}
            <div className={styles.dropdownColumn} role='group' aria-label='Minutes selection'>
              <div className={styles.dropdownLabel}>{segmentLabels.minutes}</div>
              <div
                className={`${styles.dropdownList} ${
                  dropdownFocus.isActive && dropdownFocus.column === 1
                    ? styles.dropdownListFocused
                    : ''
                }`}
                ref={minutesListRef}
            ...
            {includesSeconds && (
              <div className={styles.dropdownColumn} role='group' aria-label='Seconds selection'>
                <div className={styles.dropdownLabel}>{segmentLabels.seconds}</div>
                <div
                  className={`${styles.dropdownList} ${
                    dropdownFocus.isActive && dropdownFocus.column === 2
                      ? styles.dropdownListFocused
                      : ''
                  }`}
                  ref={secondsListRef}
            ...
            {is12Hour && (
              <div className={styles.dropdownColumn} role='group' aria-label='AM/PM selection'>
