...
import type { SegmentConstraints, TimeValue } from 'src/app-components/Timepicker/types';
...
describe('handleSegmentValueChange', () => {
  ...
  describe('hours wrapping', () => {
    const hoursConstraints: SegmentConstraints = {
      min: 1,
      max: 12,
      validValues: Array.from({ length: 12 }, (_, i) => i + 1),
    };
    ...
    describe('24-hour format', () => {
      const hours24Constraints: SegmentConstraints = {
        min: 0,
        max: 23,
        validValues: Array.from({ length: 24 }, (_, i) => i),
      };
  ...
  describe('minutes wrapping', () => {
    const minutesConstraints: SegmentConstraints = {
      min: 0,
      max: 59,
      validValues: Array.from({ length: 60 }, (_, i) => i),
    };
  ...
  describe('constraint validation', () => {
    it('should find nearest valid value when wrapped value is not in constraints', () => {
      const result = handleSegmentValueChange('minutes', 22, mockTimeValue, mockConstraints, true);

      expect(result.updatedTimeValue).toEqual({ minutes: 15 });
    });

    it('should find nearest valid value on the higher side', () => {
      const result = handleSegmentValueChange('minutes', 37, mockTimeValue, mockConstraints, true);

      expect(result.updatedTimeValue).toEqual({ minutes: 30 });
    });

    it('should find nearest valid value on the lower side', () => {
      const result = handleSegmentValueChange('minutes', 38, mockTimeValue, mockConstraints, true);
  ...
  describe('error handling', () => {
    it('should throw error for invalid segment type and value type combination', () => {
      expect(() => {
        handleSegmentValueChange('period', 123, mockTimeValue, mockConstraints, true);
    ...
    it('should throw error for numeric segment with string value', () => {
      expect(() => {
        handleSegmentValueChange('hours', 'invalid', mockTimeValue, mockConstraints, true);
