...
test.describe('test', async () => {
  ...
  test('that the user can drag a new task in to the data model, and assign a data model to it', async ({
    ...
    await processEditorPage.dataModelConfig.waitForDataModelButtonToBeVisibleWithValue(
      newDataModel,
    );
    await processEditorPage.dataModelConfig.verifyDataModelButtonTextIsSelectedDataModel(
      newDataModel,
  ...
  test('that the user can add a sequence arrow between two tasks and that the changes are uploaded to Gitea', async ({
    page,
    testAppName,
  }) => {
    const processEditorPage = await setupAndVerifyProcessEditorPage(page, testAppName);
  ...
  test('test2', async ({ page, testAppName }) => {
