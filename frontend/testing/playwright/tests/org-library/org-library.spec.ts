...
test.beforeAll(async ({ testAppName, request, storageState }) => {
  const designerApi = new DesignerApi({ app: testAppName, org: 'ttd' });
...
test.afterAll(async ({ request, testAppName }) => {
  ...
  const response = await request.delete(
    gitea.getDeleteAppEndpoint({ app: testAppName, org: 'ttd' }),
  );
  expect(response.ok()).toBeTruthy();
});
...
const setupAndVerifyOrgLibraryPage = async (
  ...
  const orgLibraryPage = new OrgLibraryPage(page, { app: testAppName, org: 'ttd' });
