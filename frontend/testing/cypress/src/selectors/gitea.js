...
export const gitea = {
  ...
  getConfirmDeletionButton: () =>
    getDeleteRepositoryModal().findByRole("button", {
      name: /confirm deletion/i,
    }),
  getDashboardLink: () => cy.findByRole("link", { name: /dashboard/i }),
  getDeleteButton: () =>
    cy.findByRole("button", { name: /delete this repository/i }),
  ...
  getLanguageMenuItem: (language) =>
    cy.findByRole("menuitem", { name: language }),
  getLoginButton: () => cy.findByRole("button", { name: /logg inn/i }),
  getLoginErrorMessage: () =>
    cy.findByText(/ugyldig brukernavn eller passord./i),
