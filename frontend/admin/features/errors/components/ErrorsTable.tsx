...
import { useContainerLogsQuery } from 'admin/hooks/queries/useContainerLogsQuery';
...
export const ErrorsTable = ({ org, env, app }: InstancesTableProps) => {
  ...
  return (
    <div>
      ...
      <ErrorsTable1 org={org} env={env} app={app} time={time} />
    </div>
  );
};
...
export const ErrorsTable1 = ({ org, env, app, time }: InstancesTableProps1) => {
  const { data, status } = useContainerLogsQuery(org, env, app, time);
...
const ErrorsTableWithData = ({ errors }: InstancesTableWithDataProps) => {
  ...
  return (
    <StudioTable zebra data-color='brand1'>
      ...
      <StudioTable.Body>
        {errors.map((error) => (
          <StudioTable.Row key={error.timeGenerated + error.logMessage}>
            <StudioTable.Cell>{formatDateAndTime(error.timeGenerated)}</StudioTable.Cell>
            <StudioTable.Cell>{error.logMessage}</StudioTable.Cell>
          </StudioTable.Row>
        ))}
      </StudioTable.Body>
    </StudioTable>
