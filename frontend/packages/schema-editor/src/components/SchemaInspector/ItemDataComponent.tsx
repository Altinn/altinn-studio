...
export function ItemDataComponent({ schemaNode }: IItemDataComponentProps) {
  ...
  const [itemDescription, setItemItemDescription] = useState<string>(
    description || "",
  );

  const getChildNodes = () =>
    pointer && pointer.endsWith(nodeName)
      ? getChildNodesByPointer(data, pointer)
      : [];

  const onChangeRef = (path: string, ref: string) =>
    mutate(setRef(data, { path, ref }));

  const onChangeFieldType = (type: FieldType) =>
    mutate(setType(data, { path: pointer, type }));
  ...
  const onChangeTitle = () =>
    mutate(setTitle(data, { path: pointer, title: itemTitle }));

  const onChangeDescription = () =>
    mutate(
      setDescription(data, { path: pointer, description: itemDescription }),
    );
  ...
  const handleArrayPropertyToggle = () =>
    mutate(toggleArrayField(data, pointer));
  ...
  return (
    <div className={classes.root}>
      ...
      <Divider marginless />
      <LegacyFieldSet
        legend={t("schema_editor.descriptive_fields")}
        ...
        <div>
          <TextField
            ...
            onChange={(e: ChangeEvent) =>
              setItemItemTitle((e.target as HTMLInputElement)?.value)
            }
            value={itemTitle}
          />
        </div>
