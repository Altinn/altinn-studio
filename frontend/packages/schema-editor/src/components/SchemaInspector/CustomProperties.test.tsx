...
describe("CustomProperties", () => {
  ...
  it("Renders legend", () => {
    render();
    expect(
      screen.getByText(textMock("schema_editor.custom_props"))
    ).toBeInTheDocument();
  });
  ...
  it("Renders a number input with correct value for number properties", () => {
    render();
    expect(screen.getByLabelText(numberPropKey)).toHaveValue(
      numberPropValue.toString()
    );
  });
  ...
  it('Renders an "unsupported property" message for unsupported properties', () => {
    render();
    expect(
      screen.getAllByText(textMock("schema_editor.custom_props_unknown_format"))
    ).toHaveLength(1);
  });
  ...
  it("Saves model without deleted property when the delete button is clicked", async () => {
    ...
    await act(() =>
      user.click(
        screen.getAllByRole("button", { name: textMock("general.delete") })[0]
      )
    );
  ...
  it("Saves model correctly when a string property is changed", async () => {
    ...
    const updatedModel = getSavedModel(saveDatamodel);
    const updatedNode = getNodeByPointer(updatedModel, defaultPath);
    expect(updatedNode.custom[stringPropKey]).toEqual(
      stringPropValue + newLetter
    );
  });
  ...
  it("Saves model correctly when a number property is changed", async () => {
    ...
    const newDigit = 4;
    await act(() =>
      user.type(screen.getByLabelText(numberPropKey), newDigit.toString())
    );
    expect(saveDatamodel).toHaveBeenCalledTimes(1);
    const updatedModel = getSavedModel(saveDatamodel);
    const updatedNode = getNodeByPointer(updatedModel, defaultPath);
    expect(updatedNode.custom[numberPropKey]).toEqual(
      numberPropValue * 10 + newDigit
    );
  });
  ...
  it("Saves model correctly when a boolean property is changed from false to true", async () => {
    render();
    await act(() =>
      user.click(screen.getByLabelText(initiallyFalseBoolPropKey))
    );
    expect(saveDatamodel).toHaveBeenCalledTimes(1);
    const updatedModel = getSavedModel(saveDatamodel);
    const updatedNode = getNodeByPointer(updatedModel, defaultPath);
  ...
  it("Saves model correctly when a boolean property is changed from true to false", async () => {
    render();
    await act(() =>
      user.click(screen.getByLabelText(initiallyTrueBoolPropKey))
    );
    expect(saveDatamodel).toHaveBeenCalledTimes(1);
    const updatedModel = getSavedModel(saveDatamodel);
    const updatedNode = getNodeByPointer(updatedModel, defaultPath);
...
const render = (
  ...
  schemaState: Partial<SchemaState> = {}
) => {
