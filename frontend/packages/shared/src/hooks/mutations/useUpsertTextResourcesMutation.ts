...
export const useUpsertTextResourcesMutation = (
  org: string,
  app: string,
): UseMutationResult<ITextResourcesWithLanguage, DefaultError, UpsertTextResourcesMutationArgs> => {
  ...
  return useMutation({
    ...
    onSuccess: (response: ITextResourcesWithLanguage): void => {
      if (previewConnection && previewConnection.state === 'Connected') {
        previewConnection.send('sendMessage', 'reload-layouts').catch(function (err) {
