...
export function convertFromLayoutToInternalFormat(
  ...
  for (const element of topLevelComponents(layout)) {
    if (element.type !== ComponentType.Group) {
      ...
      rest.propertyPath =
        formItemConfigs[rest.type].defaultProperties.propertyPath;
...
export function convertInternalToLayoutFormat(
  internalFormat: IInternalLayout
): ExternalFormLayout {
...
export function extractChildrenFromGroup(
  ...
  children?.forEach((componentId: string) => {
    const component: ExternalComponent = components.find(
      (candidate: ExternalComponent) => candidate.id === componentId
    );
    if (component.type === "Group") {
      ...
      convertedLayout.components[componentId] = {
        ...
        itemType: "COMPONENT",
        propertyPath:
          formItemConfigs[component.type].defaultProperties.propertyPath,
      } as FormComponent;
    }
  });
}
...
export const updateContainer = (
  ...
  if (currentId !== newId) {
    ...
    const parentContainer = Object.keys(oldLayout.order).find(
      (containerId: string) => {
...
export const removeComponent = (
  ...
  if (containerId) {
    newLayout.order[containerId] = removeItemByValue(
 ...
 * @returns The new layout.
 */
export const addNavigationButtons = (
  layout: IInternalLayout,
  id: string
): IInternalLayout => {
...
export const moveLayoutItem = (
  ...
  if (oldContainerId) {
    newLayout.order[oldContainerId] = removeItemByValue(
...
export const addItemOfType = <T extends ComponentType>(
  ...
  if (newItem.itemType === "COMPONENT")
    return addComponent(layout, newItem, parentId, position);
 ...
 */
export const hasSubContainers = (
  layout: IInternalLayout,
  itemId: string
): boolean =>
  isContainer(layout, itemId) &&
  layout.order[itemId].some((id) => isContainer(layout, id));
 ...
 */
const numberOfContainerLevels = (
  layout: IInternalLayout,
  itemId: string
): number => {
  if (!isContainer(layout, itemId) || !hasSubContainers(layout, itemId))
    return 0;
  else
    return (
      1 +
      Math.max(
        ...layout.order[itemId].map((id) => numberOfContainerLevels(layout, id))
      )
    );
};
...
export const getDepth = (layout: IInternalLayout): number => {
  ...
  if (containers.length <= 1) return 0;
    ...
    return Math.max(
      ...containers.map((id) => numberOfContainerLevels(layout, id))
    );
};
