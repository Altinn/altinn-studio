...
export const changeTitleBinding = (
  component: FormComponent,
  resourceKey: string
): FormComponent => changeTextResourceBinding(component, "title", resourceKey);
...
export const addOptionToComponent = <
 ...
 */
export const generateFormItem = <T extends ComponentType>(
  type: T,
  id: string
): FormItem<T> => {
  const { defaultProperties } = formItemConfigs[type];
  return type === ComponentType.Group
    ? defaultProperties
...
export const getUnsupportedPropertyTypes = (
  ...
  let unsupportedPropertyKeys = propertyKeys
    .filter((key) =>
      knownUnsupportedPropertyKeys
        ? !knownUnsupportedPropertyKeys.includes(key)
        : true
    )
    .filter((key) => {
  ...
  if (knownUnsupportedPropertyKeys) {
    unsupportedPropertyKeys = unsupportedPropertyKeys.concat(
      knownUnsupportedPropertyKeys
    );
  }
