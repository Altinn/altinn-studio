...
export const Dynamics = ({
    ...
    if (form) {
      const propertiesWithDynamics:
        ...
        Object.keys(form)
          ...
          .map(
            (property) =>
              property as ExpressionPropertyBase | ExpressionPropertyForGroup
          );
      const potentialConvertedExternalDynamics: Dynamic[] =
        propertiesWithDynamics
          ?.filter((property) => typeof form[property] !== "boolean")
          ?.map((property) =>
            convertExternalDynamicToInternal(property, form[property])
          );
      ...
      const defaultDynamics =
        potentialConvertedExternalDynamics?.length === 0
          ? [defaultDynamic]
          : potentialConvertedExternalDynamics;
  ...
  const addDynamic = async () => {
    ...
    const nonEditableDynamics: Dynamic[] = await Promise.all(
      [
        ...dynamics.filter(
          (prevDynamic) =>
            (prevDynamic.expressionElements &&
              prevDynamic.expressionElements.length > 0) ||
            complexExpressionIsSet(prevDynamic.complexExpression)
        ),
      ].map(async (prevDynamic) => {
        if (complexExpressionIsSet(prevDynamic.complexExpression)) {
          const newDynamic = tryParseString(
            prevDynamic,
            prevDynamic.complexExpression
          );
        ...
        if (prevDynamic.property && prevDynamic.editMode) {
          ...
          form[prevDynamic.property] =
            convertDynamicToExternalFormat(prevDynamic);
          try {
            await updateFormComponent({
              updatedComponent: form as FormComponent,
              id: formId,
            });
  ...
  const removeDynamic = async (dynamic: Dynamic) => {
    ...
    const newDynamics =
      dynamics.length === 1
        ? dynamics
            .filter((prevDynamic) => prevDynamic !== dynamic)
            .concat(defaultDynamic)
        : dynamics.filter((prevDynamic) => prevDynamic !== dynamic);
    setDynamics(newDynamics);
  };
  ...
  const getProperties = (dynamic: Dynamic) => {
    ...
    const availableProperties = expressionProperties.filter(
      (expressionProperty) =>
        !Object.values(alreadyUsedProperties).includes(expressionProperty)
    );
