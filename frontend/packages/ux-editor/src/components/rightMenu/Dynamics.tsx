...
export const Dynamics = ({
  ...
  const [showRemoveDynamicButton, setShowRemoveDynamicButton] =
  ...
  const addDynamic = () => {
    ...
    const nonEditableDynamics: Dynamic[] = [
      ...dynamics.filter(
        (prevDynamic) => prevDynamic.expressionElements.length > 0,
      ),
  ...
  const removeDynamic = (dynamic: Dynamic) => {
    ...
    if (dynamics.length === 1) {
      ...
      setDynamics((prevDynamics) =>
        prevDynamics.filter((prevDynamic) => prevDynamic !== dynamic),
      );
    }
  };
