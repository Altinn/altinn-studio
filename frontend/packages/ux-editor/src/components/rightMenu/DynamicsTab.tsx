...
export const DynamicsTab = () => {
  ...
  const editDynamic = (dynamic: Dynamic) => {
    ...
    setDynamics(prevDynamics => prevDynamics.filter(prevDynamic => prevDynamic !== dynamic).map(prevDynamic => ({ ...prevDynamic, editMode: false })).concat({...dynamic, editMode: true}));
  };
  ...
  // Show dynamics add button only if there are existing dynamics from before
  // Need to collect all existing expressions and list them here - or send a state prop to all the mapped expressions

  // Should only get button for adding new dynamic if there are existing dynamics and they are valid - send callback to addDynamic to DynamicContent
