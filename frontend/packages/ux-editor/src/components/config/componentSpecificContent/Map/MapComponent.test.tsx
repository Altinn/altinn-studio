...
describe("MapComponent", () => {
  ...
  test("should render titles", async () => {
    ...
    expect(
      screen.getByRole("heading", { level: 2, name: "Sentrum av kartet" }),
    );
...
describe("AddMapLayer", () => {
  ...
  it("renders correctly when layers are not empty", async () => {
    ...
    expect(
      screen.getByLabelText(texts["ux_editor.url_label"]),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(texts["ux_editor.attribution_label"]),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(texts["ux_editor.subdomains_label"]),
    ).toBeInTheDocument();
  });
