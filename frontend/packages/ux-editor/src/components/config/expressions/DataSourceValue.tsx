...
export const DataSourceValue = ({
  ...
  const getDataModelElementNames = (
  ...
  const getCorrespondingDataSourceValues = (dataSource: DataSource) => {
    switch (dataSource) {
      ...
      case DataSource.DataModel:
        return getDataModelElementNames(
          dataModelElementsData as DatamodelFieldElement[]
        );
      case DataSource.InstanceContext:
        return ["instanceOwnerPartyId", "instanceId", "appId"].map(
          (dsv: string) => ({ label: dsv, value: dsv })
        );
  ...
  switch (currentDataSource) {
      ...
      return (
        <Select
          ...
          options={[{ label: "Velg...", value: "default" }].concat(
            getCorrespondingDataSourceValues(currentDataSource)
          )}
          value={
            isComparableValue
              ? expressionElement.comparableValue
              : expressionElement.value || "default"
          }
        />
      );
    ...
    case DataSource.Number:
      return (
        <TextField
          formatting={
            currentDataSource === DataSource.Number ? { number: {} } : {}
          }
          onChange={(e) =>
