...
export const DynamicContent = ({
  ...
  const [selectedAction, setSelectedAction] = React.useState<string>(
    dynamic.property || "default",
  );
  const [expressionElements, setExpressionElements] = React.useState<
    ExpressionElement[]
  >([...dynamic.expressionElements]); // default state should be already existing expressions
  ...
  const addExpressionElement = () => {
    const newExpressionElement: ExpressionElement = { id: uuidv4() };
    const updatedExpressionElements = [
  ...
  const removeExpressionElement = (expressionElement: ExpressionElement) => {
    if (dynamic.expressionElements.length < 2) {
      ...
      const lastExpressionElement =
        updatedExpressionElements[updatedExpressionElements.length - 1];
      dynamic.expressionElements = [
        ...updatedExpressionElements.filter(
          (expEl) => expEl !== lastExpressionElement,
        ),
        {
          ...lastExpressionElement,
          expressionOperatorForNextExpression: undefined,
        },
      ];
            ...
            {expressionElements.map((expEl: ExpressionElement) => (
              <div key={expEl.id}>
                ...
                <p>
                  {" "}
                  <ArrowRightIcon fontSize="1.5rem" />
                  {expEl.comparableDataSource}{" "}
                  <span>{expEl.comparableValue}</span>
                </p>
                <p className={classes.bold}>
                  {expEl.expressionOperatorForNextExpression}
                </p>
