...
export function App() {
  ...
  const selectedLayoutSetInPreviewFromLocalStorage =
    typedLocalStorage.getItem<string>("layoutSet" + app);
  ...
  const { isSuccess: areWidgetsFetched, isError: widgetFetchedError } =
    useWidgetsQuery(org, app);
  const { isSuccess: isDatamodelFetched, isError: dataModelFetchedError } =
    useDatamodelMetadataQuery(org, app);
  const { isSuccess: areTextResourcesFetched } = useTextResourcesQuery(
    org,
    app
  );

  const componentIsReady =
    areWidgetsFetched && isDatamodelFetched && areTextResourcesFetched;
  ...
  useEffect(() => {
    if (selectedLayoutSet === null && layoutSets) {
      ...
      dispatch(
        FormLayoutActions.updateSelectedLayoutSet(layoutSets.sets[0].id)
      );
      typedLocalStorage.setItem<string>(
        "layoutSet" + app,
        layoutSets.sets[0].id
      );
    }
  ...
  if (componentHasError) {
    ...
    return (
      <ErrorPage title={mappedError.title} message={mappedError.message} />
    );
  }
