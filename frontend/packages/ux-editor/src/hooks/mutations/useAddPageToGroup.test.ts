...
describe('useAddPageToGroup', () => {
  ...
  describe('useAddPageToGroup loop coverage', () => {
    ...
    it('should increment exactly once when first attempt is taken', () => {
      ...
      // Should try page1 (taken), then use page2
      expect(mockMutate).toHaveBeenCalledWith(
        expect.objectContaining({
          groups: [
            expect.objectContaining({
    ...
    it('should increment multiple times with gap in numbering', () => {
      ...
      // The hook starts numbering from allPageNames.length + 1 (4 in this case)
      // and increments until it finds an available name:
      // 4 -> 'ux_editor.page4' (taken)
      // 5 -> 'ux_editor.page5' (available)
      expect(mockMutate).toHaveBeenCalledWith(
        expect.objectContaining({
          groups: [
            expect.objectContaining({
