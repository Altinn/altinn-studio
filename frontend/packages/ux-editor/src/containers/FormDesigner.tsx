...
export const FormDesigner = ({
  ...
  const { data: formLayouts, isError: layoutFetchedError } =
  ...
  useEffect(() => {
    if (!searchParams.has("layout") && layoutPagesOrder?.[0]) {
      setSearchParams({
        ...deepCopy(searchParams),
        layout: layoutPagesOrder[0],
      });
      dispatch(FormLayoutActions.updateSelectedLayout(layoutPagesOrder[0]));
    } else if (searchParams.has("layout")) {
      dispatch(
        FormLayoutActions.updateSelectedLayout(searchParams.get("layout")),
      );
  ...
  useEffect((): void => {
    ...
    const layoutsWithContentExist =
      layoutOrder && !Object.keys(layoutOrder).length;
      ...
      <DndProvider backend={HTML5Backend}>
        <div className={classes.root}>
          <div className={classes.container}>
            ...
            <FormContextProvider>
              <DesignView
                className={classes.mainContent + " " + classes.item}
              />
              <RightMenu
