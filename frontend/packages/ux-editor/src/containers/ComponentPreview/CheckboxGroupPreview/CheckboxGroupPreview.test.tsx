...
describe("CheckboxGroupPreview", () => {
  ...
  it("Renders button to add option by default", async () => {
    await render();
    expect(
      screen.getByRole("button", { name: addOptionText })
    ).toBeInTheDocument();
  });
  ...
  it("Calls handleComponentChange with new option when clicking the add option button", async () => {
    await renderAndOpenAddSection();
    const addOptionButton = last(
      screen.getAllByRole("button", { name: addText })
    );
...
const waitForData = async () => {
  const { result } = renderHookWithMockStore(
    ...
    {
      getTextResources: () =>
        Promise.resolve({ language: "nb", resources: nbTextResources }),
    }
...
const render = async (props: Partial<CheckboxGroupPreviewProps> = {}) => {
  ...
  return renderWithMockStore()(
    <CheckboxGroupPreview {...defaultProps} {...props} />
  );
};

const renderAndOpenAddSection = async (
  props?: Partial<CheckboxGroupPreviewProps>
) => {
  await render(props);
  const openAddSectionButton = screen.getByRole("button", {
    name: addOptionText,
  });
