...
const mockUseUpdateFormContainerMutation =
  useUpdateFormContainerMutation as jest.MockedFunction<
...
const mockUseUpdateFormComponentMutation =
  useUpdateFormComponentMutation as jest.MockedFunction<
...
describe("FormContext", () => {
  ...
  it("should update the form when calling handleUpdate", async () => {
    ...
    await waitFor(async () =>
      expect((await screen.findByTestId("form.id")).textContent).toEqual(
        mockForm.id,
      ),
    );
    await waitFor(async () =>
      expect((await screen.findByTestId("form.itemType")).textContent).toEqual(
        mockForm.itemType,
      ),
    );
  });
  ...
  it("should edit the form when calling handleEdit", async () => {
    ...
    await waitFor(async () =>
      expect((await screen.findByTestId("formId")).textContent).toEqual(
        mockForm.id,
      ),
    );
    await waitFor(async () =>
  ...
  it("should discard the form when calling handleDiscard", async () => {
    ...
    expect(state?.appData?.textResources?.currentEditId).toBeUndefined();
    await waitFor(async () =>
      expect((await screen.findByTestId("formId")).textContent).toBe(""),
    );
    await waitFor(async () =>
      expect((await screen.findByTestId("form.id")).textContent).toBe(""),
    );
    await waitFor(async () =>
  ...
  it("should save the container when calling handleSave", async () => {
    ...
    render(() => {
      ...
      return (
        <button
          data-testid="button"
          onClick={() => handleSave(mockForm.id, mockForm)}
        />
      );
    });
    ...
    render(() => {
      ...
      return (
        <>
          <button
            data-testid="button"
            onClick={async () => await handleSave("old-id", mockForm)}
          />
          <div data-testid="formId">{formId}</div>
        </>
      );
  ...
  it("should save the container when calling debounceSave", async () => {
    ...
    render(() => {
      ...
      return (
        <button
          data-testid="button"
          onClick={() => debounceSave(mockForm.id, mockForm)}
        />
      );
    });
