...
<Paragraph>
  StudioTableLocalPagination brings together Digdir Designsystemet's `Table` and `Pagination` components. It dynamically renders a table by passing an array of objects to the column and row props.

  * Use this component when you have a complete set of data, and want the component to handle pagination internally. Otherwise, use `StudioTableRemotePagination`.
...
```
<Heading level={2} size='xsmall'>Row format</Heading>
<Paragraph>
