...
describe('ResourceDashBoardPage', () => {
  ...
  it('does not show the spinner when the resource list is present', async () => {
    ...
    render();
    await waitForElementToBeRemoved(() => screen.queryByTitle(textMock('resourceadm.dashboard_spinner')));
    expect(screen.queryByRole(textMock('resourceadm.dashboard_spinner'))).not.toBeInTheDocument();
    expect(screen.getByRole('heading', { name: textMock('resourceadm.dashboard_num_resources', { num: mockResourceList.length }), level: 2 } )).toBeInTheDocument();
  ...
  it('opens the import resource from altinn 2 modal on click', async () => {
    ...
    const modalTitle = screen.queryByRole('heading', { name: textMock('resourceadm.dashboard_import_modal_title'), level: 2 })
    expect(modalTitle).not.toBeInTheDocument();

    const importButton = screen.getByRole('button', { name: textMock('resourceadm.dashboard_import_resource') })
    await act(() => user.click(importButton));

    expect(screen.getByRole('heading', { name: textMock('resourceadm.dashboard_import_modal_title'), level: 2 })).toBeInTheDocument();
  })
  ...
  it('opens the create new resource modal on click', async () => {
    ...
    const modalTitle = screen.queryByRole('heading', { name: textMock('resourceadm.dashboard_create_modal_title'), level: 2 })
    expect(modalTitle).not.toBeInTheDocument();

    const createButton = screen.getByRole('button', { name: textMock('resourceadm.dashboard_create_resource') })
    await act(() => user.click(createButton));

    expect(screen.getByRole('heading', { name: textMock('resourceadm.dashboard_create_modal_title'), level: 2 })).toBeInTheDocument();
  })
  ...
  it ('does not display the error message when the list is not empty', async () => {
    ...
    expect(screen.queryByText(textMock('resourceadm.dashboard_empty_list'))).not.toBeInTheDocument();
  })
