...
const mockResourceList: ResourceListItem[] = [mockResourceListItem1, mockResourceListItem2, mockResourceListItem3];
...
describe('ResourceTable', () => {
  ...
  it('toggles sort order when header is clicked', async () => {
    ...
    const sortButton = screen.getByRole('button', { name: textMock('resourceadm.dashboard_table_header_last_changed') });
  ...
  it('navigates to the clicked resource', async () => {
    ...
    const [editButton] = screen.getAllByRole('button', { name: textMock('resourceadm.dashboard_table_row_edit') })
