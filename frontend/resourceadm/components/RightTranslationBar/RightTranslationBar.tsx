...
>(
  (
    ...
    const handleChange = (lang: "nn" | "en", val: string) => {
      const obj: LanguageStringType =
        lang === "nn" ? { ...value, nn: val } : { ...value, en: val };
      onChangeValue(obj);
    };
        ...
        <div className={classes.bodyWrapper}>
          <Alert severity="info">
            <Paragraph size="small">
              For å kunne publisere ressursen må du legge til nynorsk og engelsk
              oversettelse.
            </Paragraph>
          </Alert>
          <div className={classes.inputWrapper}>
            {displayNField("nn", false)}
          </div>
          <div className={classes.inputWrapper}>
            {displayNField("en", true)}
          </div>
        </div>
      </div>
    );
