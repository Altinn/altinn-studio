...
describe("getTopBarMenu", () => {
  ...
  it('should return all items when provided repository type is "App" which is not hidden behind feature-flags', () => {
    const menuLength = menu.filter(
      (menuItem) => !menuItem.featureFlagName
    ).length;
    expect(
      getTopBarMenu("test-org", "test-app", RepositoryType.App, t)
    ).toHaveLength(menuLength);
  });
  ...
  it('should return only menu items relevant for datamodelling repo when provided repo type is "Datamodels"', () => {
    ...
    expect(
      getTopBarMenu("test-org", "test-app", RepositoryType.Datamodels, t)
    ).toEqual(expected);
  });
  ...
  it('should return empty list when provided repo type is "Unknown"', () => {
    ...
    expect(
      getTopBarMenu("test-org", "test-app", RepositoryType.Unknown, t)
    ).toEqual(expected);
  });
