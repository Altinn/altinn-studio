...
describe('ActionLinks', () => {
  it('should render the three buttons', () => {
    ...
    const giteaButton = screen.getByRole('link', { name: textMock('dashboard.show_repo') });
    const editButton = screen.getByRole('link', { name: textMock('dashboard.edit_app') });
    const dropdownButton = screen.getByRole('button', { name: textMock('dashboard.app_dropdown') });
  ...
  it('should open MakeCopyModal when clicking "Make copy" in the dropdown menu', async () => {
    ...
    const dropdownButton = screen.getByLabelText(textMock('dashboard.app_dropdown'));
