...
const config = {
  transform: {
    "\\.(ts|tsx|js)": "@swc/jest",
    [`node_modules(\\\\|/)(${packagesToTransform})(\\\\|/).+\\.(j|t)sx?$`]:
      "@swc/jest",
  },
  ...
  moduleNameMapper: {
    ...
    "^@altinn/schema-editor/(.*)": path.join(
      __dirname,
      "packages/schema-editor/src/$1"
    ),
    "^@altinn/schema-model/(.*)": path.join(
      __dirname,
      "packages/schema-model/src/$1"
    ),
    "^@altinn/ux-editor/(.*)": path.join(
      __dirname,
      "packages/ux-editor/src/$1"
    ),
    "^@altinn/process-editor/(.*)": path.join(
      __dirname,
      "packages/process-editor/src/$1"
    ),
    "^@altinn/policy-editor/(.*)": path.join(
      __dirname,
      "packages/policy-editor/src/$1"
    ),
  },
