...
| --------------------- | ----------------------------------------------------------------------------------------- |
| `Input`               | Text, search, numeric and pattern-formatted input field, with label, help, prefix/suffix. |
| `Paragraph`           | Renders a block of text (plain, HTML or markdown), with optional help text.               |
| `DemoLayoutComponent` | Example component demonstrating text translation and parsing. Not a real form component.  |
...
A component with no runtime props (e.g. `Paragraph`) classifies everything as `config`; the
"Runtime (injected)" section is then omitted automatically.
...
controls tables and the remaining stories. See `Input/` and `Paragraph/` for working examples.
